...
namespace Openize.Accord.Tests.IO
{
    ...
    public class SparseWriterTest
    {
        ...
        public void WriteSamplesTest()
        {
            ...
            MemoryStream file = new MemoryStream(Encoding.Default.GetBytes(Resources.iris_scale));

            int sampleSize = 4;

            SparseReader src = new SparseReader(stream: file, encoding: Encoding.Default, sampleSize: sampleSize);
            SparseWriter dst = new SparseWriter(this.test_txt, Encoding.Default);
            ...
            file = new MemoryStream(Encoding.Default.GetBytes(Resources.iris_scale));
            using (SparseReader orig = new SparseReader(stream: file, encoding: Encoding.Default, sampleSize: sampleSize))
            using (SparseReader copy = new SparseReader(this.test_txt, Encoding.Default, sampleSize))
            {
            ...
            file = new MemoryStream(Encoding.Default.GetBytes(Resources.iris_scale));
            using (SparseReader orig = new SparseReader(stream: file, encoding: Encoding.Default, sampleSize: sampleSize))
            using (SparseReader copy = new SparseReader(this.test_txt, Encoding.Default))
            {
                while (!orig.EndOfStream)
                {
        ...
        public void WriteBoolTest()
        {
            ...
            SparseFormat.Save(samples, outputs, this.test_txt);

            string actual = File.ReadAllText(this.test_txt);
...
";
        ...
        public void WriteBoolTest_compressed()
        {
            ...
            SparseFormat.Save(samples, outputs, this.test_txt_gz, compression: SerializerCompression.GZip);

            Sparse<double>[] newSamples;
            bool[] newOutput;
            SparseFormat.Load(this.test_txt_gz, out newSamples, out newOutput, compression: SerializerCompression.GZip);


            SparseFormat.Save(newSamples, newOutput, Path.Combine(TestContext.CurrentContext.TestDirectory, "test2.txt"));

            string actual = File.ReadAllText(this.test2_txt);
            string expected = @"-1 1:1 2:2 4:3
1 1:6 3:4 4:2
-1 
";
