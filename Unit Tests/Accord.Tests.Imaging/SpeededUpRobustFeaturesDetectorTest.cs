...
namespace FileFormat.Accord.Tests.Imaging
{
    ...
    using FileFormat.Accord.Imaging.Interest_Points.SURF;
    ...
    public class SpeededUpRobustFeaturesDetectorTest
    {
        ...
        {
            // Load an Image
            Bitmap img = Image.Clone(Resources.sample_trans);
        ...
        {
            Bitmap img = Image.Clone(Resources.surf_bug_1);

            var iimg = IntegralImage.FromImage(img);
            var expected = FastHessian.getIpoints(0.0002f, 5, 2, iimg);
            SurfDescriptor.DecribeInterestPoints(expected, false, false, iimg);
        ...
        public void ProcessImageTest4()
        {
            ...
            foreach (Bitmap img in bitmaps)
            {
                ...
                // Create OpenSURF detector by Chris Evans
                {
                    ...
                    // Extract the interest points
                    var pts = FastHessian.getIpoints(0.0002f, 5, 2, iimg);

                    // Describe the interest points
                    SurfDescriptor.DecribeInterestPoints(pts, false, false, iimg);
