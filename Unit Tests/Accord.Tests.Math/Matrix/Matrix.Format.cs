...
namespace Openize.Accord.Tests.Math.Matrix
{
    ...
    public class MatrixFormatTest
    {
        ...
        {
            double[][] ex = new CsvReader(new StringReader(Resources.data16), hasHeaders: false).ToJagged<double>();
            int[] ey = new CsvReader(new StringReader(Resources.labels16), hasHeaders: false).ToJagged<int>().GetColumn(0);

            double[][] ax = Jagged.Parse(Resources.data16);
            double[] ay = Vector.Parse(Resources.labels16);
