...
namespace FileFormat.Accord.Tests.Statistics.Distributions.Univariate.Continuous
{
    ...
    public class NormalDistributionTest
    {
        ...
        public void ConstructorTest5()
        {
            ...
            Assert.AreEqual(Normal.Function(normal.ZScore(4.2)), normal.DistributionFunction(4.2));
            Assert.AreEqual(Normal.Derivative(normal.ZScore(4.2)) / normal.StandardDeviation, normal.ProbabilityDensityFunction(4.2), 1e-16);
            Assert.AreEqual(Normal.LogDerivative(normal.ZScore(4.2)) - Math.Log(normal.StandardDeviation), normal.LogProbabilityDensityFunction(4.2), 1e-15);
        ...
        public void StandardDensityFunctionTest()
        {
            for (int i = -100; i < 100; i++)
            {
                ...
                double expected = Normal.Derivative(x);
        ...
        public void LogStandardDensityFunctionTest()
        {
            for (int i = -100; i < 100; i++)
            {
                ...
                double expected = Normal.LogDerivative(x);
