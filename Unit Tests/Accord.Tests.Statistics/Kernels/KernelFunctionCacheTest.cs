...
namespace FileFormat.Accord.Tests.Statistics.Kernels
{
    ...
    public class KernelFunctionCacheTest
    {
        ...
        public void KernelFunctionCacheConstructorTest()
        {
            ...
            KernelFunctionCache target = new KernelFunctionCache(kernel, this.inputs, cacheSize);
        ...
        public void KernelFunctionCacheConstructorTest2()
        {
            ...
            int cacheSize = this.inputs.Length - 1;
        ...
        public void KernelFunctionCacheConstructorTest6()
        {
            ...
            int cacheSize = this.inputs.Length;
            ...
            for (int i = 0; i < this.inputs.Length; i++)
            {
                double expected = kernel.Function(this.inputs[i], this.inputs[i]);
                double actual = target.GetOrCompute(i);
            ...
            for (int i = 0; i < this.inputs.Length; i++)
            {
                for (int j = 0; j < this.inputs.Length; j++)
                {
                    double expected = kernel.Function(this.inputs[i], this.inputs[j]);
                    double actual = target.GetOrCompute(i, j);
        ...
        public void KernelFunctionCacheConstructorTest4()
        {
            ...
            Assert.AreEqual(this.inputs.Length, target.Size);
        }
