...
namespace Openize.Accord.Math.AForge.Math.Geometry
{
    ...
    public class CoplanarPosit
    {
        ...
        {
            get { return this.bestTranslation; }
        }
        ...
        {
            get { return this.alternateRotation; }
        }
        ...
        {
            get { return this.alternateTranslation; }
        }
        ...
        {
            get { return this.alternatePoseError; }
        }
        ...
        {
            get { return (Vector3[]) this.modelPoints.Clone( ); }
        }
        ...
        public CoplanarPosit( Vector3[] model, float focalLength )
        {
            ...
            this.modelPoints = (Vector3[]) model.Clone( );

            // compute model vectors
            this.modelVectors = Matrix3x3.CreateFromRows(
                model[1] - model[0],
                model[2] - model[0],
                model[3] - model[0] );
            ...
            this.modelVectors.SVD( out u, out e, out v );
            this.modelPseudoInverse = v * Matrix3x3.CreateDiagonal( e.Inverse( ) ) * u.Transpose( );

            // computer unit vector normal to the model
            this.modelNormal = v.GetColumn( e.MinIndex );
        }
        ...
        public void EstimatePose( Point[] points, out Matrix3x3 rotation, out Vector3 translation )
        {
            ...
            this.POS( points, new Vector3( 1 ), out rotation1, out rotation2, out translation1, out translation2 );

            // iterate further and fine tune the solution
            float error1 = this.Iterate( points, ref rotation1, ref translation1 );
            float error2 = this.Iterate( points, ref rotation2, ref translation2 );
            ...
            if ( error1 < error2 )
            {
                ...
                this.bestPoseError   = error1;

                this.alternateRotation    = rotation2;
                this.alternateTranslation = translation2;
                this.alternatePoseError   = error2;
            }
            else
            {
                this.bestRotation    = rotation2;
                this.bestTranslation = translation2;
                this.bestPoseError   = error2;

                this.alternateRotation    = rotation1;
                this.alternateTranslation = translation1;
                this.alternatePoseError   = error1;
            }

            rotation    = this.bestRotation;
            translation = this.bestTranslation;
        }
        ...
        private float Iterate( Point[] points, ref Matrix3x3 rotation, ref Vector3 translation )
        {
            ...
            for ( int count = 0; count < 100; count++ )
            {
                ...
                Vector3 eps = ( this.modelVectors * rotation.GetRow( 2 ) ) / translation.Z + 1;
                // and new pose
                this.POS( points, eps, out rotation1, out rotation2, out translation1, out translation2 );

                // calculate error for both new poses
                float error1 = this.GetError( points, rotation1, translation1 );
                float error2 = this.GetError( points, rotation2, translation2 );
        ...
        private void POS( Point[] imagePoints, Vector3 eps, out Matrix3x3 rotation1, out Matrix3x3 rotation2, out Vector3 translation1, out Vector3 translation2 )
        {
            ...
            Vector3 I0Vector = this.modelPseudoInverse * imageXs;
            Vector3 J0Vector = this.modelPseudoInverse * imageYs;
            ...
            iVector = I0Vector + ( this.modelNormal * lambda );
            jVector = J0Vector + ( this.modelNormal * mu );
            ...
            Vector3 temp = rotation1 * this.modelPoints[0];
            translation1 = new Vector3( imagePoints[0].X / scale - temp.X, imagePoints[0].Y / scale - temp.Y, this.focalLength / scale );
            ...
            temp = rotation2 * this.modelPoints[0];
            translation2 = new Vector3( imagePoints[0].X / scale - temp.X, imagePoints[0].Y / scale - temp.Y, this.focalLength / scale );
        }
        ...
        {
            Vector3 v1 = rotation * this.modelPoints[0] + translation;
            v1.X = v1.X * this.focalLength / v1.Z;
            v1.Y = v1.Y * this.focalLength / v1.Z;

            Vector3 v2 = rotation * this.modelPoints[1] + translation;
            v2.X = v2.X * this.focalLength / v2.Z;
            v2.Y = v2.Y * this.focalLength / v2.Z;

            Vector3 v3 = rotation * this.modelPoints[2] + translation;
            v3.X = v3.X * this.focalLength / v3.Z;
            v3.Y = v3.Y * this.focalLength / v3.Z;

            Vector3 v4 = rotation * this.modelPoints[3] + translation;
            v4.X = v4.X * this.focalLength / v4.Z;
            v4.Y = v4.Y * this.focalLength / v4.Z;
