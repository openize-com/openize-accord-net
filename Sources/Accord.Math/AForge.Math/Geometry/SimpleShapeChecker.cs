...
namespace Openize.Accord.Math.AForge.Math.Geometry
{
    ...
    public class SimpleShapeChecker
    {
        ...
        {
            get { return this.minAcceptableDistortion; }
            set { this.minAcceptableDistortion = Math.Max( 0, value ); }
        }
        ...
        {
            get { return this.relativeDistortionLimit; }
            set { this.relativeDistortionLimit = Math.Max( 0, Math.Min( 1, value ) ); }
        }
        ...
        {
            get { return this.angleError; }
            set { this.angleError = Math.Max( 0, Math.Min( 20, value ) ); }
        }
        ...
        {
            get { return this.lengthError; }
            set { this.lengthError = Math.Max( 0, Math.Min( 1, value ) ); }
        }
        ...
        public ShapeType CheckShapeType( List<IntPoint> edgePoints )
        {
            ...
            if ( this.IsConvexPolygon( edgePoints, out corners ) )
            {
        ...
        public bool IsCircle( List<IntPoint> edgePoints )
        {
            ...
            return this.IsCircle( edgePoints, out center, out radius );
        }
        ...
        public bool IsCircle( List<IntPoint> edgePoints, out Point center, out float radius )
        {
            ...
            float maxDitance = Math.Max( this.minAcceptableDistortion,
                ( (float) cloudSize.X + cloudSize.Y ) / 2 * this.relativeDistortionLimit );
        ...
        {
            List<IntPoint> corners;
            return this.IsQuadrilateral( edgePoints, out corners );
        }
        ...
        {
            corners = this.GetShapeCorners( edgePoints );

            if ( corners.Count != 4 )
                return false;

            return this.CheckIfPointsFitShape( edgePoints, corners );
        }
        ...
        {
            List<IntPoint> corners;
            return this.IsTriangle( edgePoints, out corners );
        }
        ...
        public PolygonSubType CheckPolygonSubType( List<IntPoint> corners )
        {
            ...
            float maxLengthDiff = this.lengthError * ( cloudSize.X + cloudSize.Y ) / 2;
            ...
            if ( corners.Count == 3 )
            {
                ...
                if ( ( Math.Abs( angle1 - 60 ) <= this.angleError ) &&
                     ( Math.Abs( angle2 - 60 ) <= this.angleError ) &&
                     ( Math.Abs( angle3 - 60 ) <= this.angleError ) )
                {
                ...
                else
                {
                    ...
                    if ( ( Math.Abs( angle1 - angle2 ) <= this.angleError ) ||
                         ( Math.Abs( angle2 - angle3 ) <= this.angleError ) ||
                         ( Math.Abs( angle3 - angle1 ) <= this.angleError ) )
                    {
                        subType = PolygonSubType.IsoscelesTriangle;
                    }

                    // check for rectangled triangle
                    if ( ( Math.Abs( angle1 - 90 ) <= this.angleError ) ||
                         ( Math.Abs( angle2 - 90 ) <= this.angleError ) ||
                         ( Math.Abs( angle3 - 90 ) <= this.angleError ) )
                    {
            ...
            else if ( corners.Count == 4 )
            {
                ...
                if ( angleBetween1stPair <= this.angleError )
                {
                    subType = PolygonSubType.Trapezoid;

                    // check 2nd pair for parallelism
                    if ( angleBetween2ndPair <= this.angleError )
                    {
                        subType = PolygonSubType.Parallelogram;

                        // check angle between adjacent sides
                        if ( Math.Abs( GeometryTools.GetAngleBetweenVectors( corners[1], corners[0], corners[2] ) - 90 ) <= this.angleError )
                        {
        ...
        public bool CheckIfPointsFitShape( List<IntPoint> edgePoints, List<IntPoint> corners )
        {
            ...
            float maxDitance = Math.Max( this.minAcceptableDistortion,
                ( (float) rectSize.X + rectSize.Y ) / 2 * this.relativeDistortionLimit );
        ...
        {
            return this.shapeOptimizer.OptimizeShape( PointsCloud.FindQuadrilateralCorners( edgePoints ) );
        }
    }
}
