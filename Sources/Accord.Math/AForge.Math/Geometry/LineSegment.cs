...
namespace FileFormat.Accord.Math.AForge.Math.Geometry
{
    ...
    public sealed class LineSegment
    {
        ...
        public float DistanceToPoint(Point point)
        {
            ...
            switch (this.LocateProjection(point))
            {
                case ProjectionLocation.RayA:
                    segmentDistance = point.DistanceTo(this.start);
                    break;
                case ProjectionLocation.RayB:
                    segmentDistance = point.DistanceTo(this.end);
                    break;
                default:
                    segmentDistance = this.line.DistanceToPoint(point);
                    break;
            };
        ...
        public Point? GetIntersectionWith(LineSegment other)
        {
            ...
            if ((this.line.Slope == other.line.Slope) || (this.line.IsVertical && other.line.IsVertical))
            {
                if (this.line.Intercept == other.line.Intercept)
                {
                    ...
                    ProjectionLocation projC = this.LocateProjection(other.start), projD = this.LocateProjection(other.end);
                    ...
                    else if (((this.start == other.start) && (projD == ProjectionLocation.RayA)) ||
                              ((this.start == other.end) && (projC == ProjectionLocation.RayA)))
                    {
                        // shared start point
                        result = this.start;
                    }
                    else if (((this.end == other.start) && (projD == ProjectionLocation.RayB)) ||
                              ((this.end == other.end) && (projC == ProjectionLocation.RayB)))
                    {
                        // shared end point
                        result = this.end;
                    }
            ...
            {
                result = this.GetIntersectionWith(other.line);
        ...
        public Point? GetIntersectionWith(Line other)
        {
            ...
            if ((this.line.Slope == other.Slope) || (this.line.IsVertical && other.IsVertical))
            {
                if (this.line.Intercept == other.Intercept) 
            ...
            {
                result = this.line.GetIntersectionWith(other);
            }

            if ((result.HasValue) && (this.LocateProjection(result.Value) != ProjectionLocation.SegmentAB))
            {
        ...
        private ProjectionLocation LocateProjection(Point point)
        {
            ...
            */
            ...
            Point abDelta = this.end - this.start;
            Point acDelta = point - this.start;
