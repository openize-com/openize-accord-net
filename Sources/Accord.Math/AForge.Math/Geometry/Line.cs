...
namespace Openize.Accord.Math.AForge.Math.Geometry
{
    ...
    public sealed class Line
    {
        ...
        {
            get { return float.IsInfinity(this.k); }
        }
        ...
        private Line(float radius, float theta, bool unused)
        {
            ...
            if (!float.IsInfinity(this.k))
            {
                this.b = pt1.Y - this.k * pt1.X;
            }
            else
            {
        ...
        private Line(Point point, float theta)
        {
            ...
            this.k = (float)(-1.0f / Math.Tan(theta));
        ...
        public float GetAngleBetweenLines(Line secondLine)
        {
            ...
            bool isVertical1 = this.IsVertical;
            ...
            {
                float tanPhi = ((k2 > this.k) ? (k2 - this.k) : (this.k - k2)) / (1 + this.k * k2);
            ...
            else
            {
                ...
                {
                    angle = (float)(Math.PI / 2 - Math.Atan(this.k) * Math.Sign(this.k));
                }
            }
        ...
        public Point? GetIntersectionWith(Line secondLine)
        {
            ...
            else
            {
                if (isVertical1)
                {
                    intersection = new Point(this.b, k2 * this.b + b2);
                }
                else if (isVertical2)
                {
                    intersection = new Point(b2, this.k * b2 + this.b);
                }
                else
                {
                    // the intersection is at x=(b2-b1)/(k1-k2), and y=k1*x+b1
                    float x = (b2 - this.b) / (this.k - k2);
                    intersection = new Point(x, this.k * x + this.b);
                }
            }
        ...
        public float DistanceToPoint(Point point)
        {
            ...
            if (!this.IsVertical)
            {
                float div = (float)Math.Sqrt(this.k * this.k + 1);
                distance = Math.Abs((this.k * point.X + this.b - point.Y) / div);
            }
            else
            {
                distance = Math.Abs(this.b - point.X);
            }
        ...
        {
            return this.k.GetHashCode() + this.b.GetHashCode();
        }
        ...
        {
            return string.Format(System.Globalization.CultureInfo.InvariantCulture, "k = {0}, b = {1}", this.k, this.b);
        }
    }
}
