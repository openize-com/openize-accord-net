...
namespace Openize.Accord.Math.Integration
{
    ...
    public class RombergMethod : IUnivariateIntegration, INumericalIntegration
    {
        ...
        /// </summary>
        /// 
        public int Steps { get { return this.s.Length; } }
        ...
        public bool Compute()
        {
            ...
            {
                sum = this.s[0];
                this.s[0] = TrapezoidalRule.Integrate(this.Function, a, b, 1 << k);

                for (int i = 1; i <= k; i++)
                {
                    int p = (int)Math.Pow(4, i);
                    this.s[k] = (p * this.s[i - 1] - sum) / (p - 1);
            ...
            this.Area = this.s[this.s.Length - 1];
