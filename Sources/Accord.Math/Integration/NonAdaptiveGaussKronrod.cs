...
namespace Openize.Accord.Math.Integration
{
        ...
        INumericalIntegration<NonAdaptiveGaussKronrodStatus>
    {
        ...
        public DoubleRange Range
        {
            ...
            set
            {
                if (Double.IsInfinity(this.range.Min) || Double.IsNaN(this.range.Min))
                    throw new ArgumentOutOfRangeException("value", "Minimum is out of range.");

                if (Double.IsInfinity(this.range.Max) || Double.IsNaN(this.range.Max))
        ...
        public bool Compute()
        {
            ...
            qng_(this.Function, this.Range.Min, this.Range.Max, this.ToleranceAbsolute, this.ToleranceRelative,
                out this.result, out this.error, out this.evaluations, out errorCode);

            this.Status = (NonAdaptiveGaussKronrodStatus)errorCode;

            return this.Status == NonAdaptiveGaussKronrodStatus.Success;
        }
