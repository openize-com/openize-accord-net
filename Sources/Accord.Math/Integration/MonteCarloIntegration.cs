...
namespace Openize.Accord.Math.Integration
{
    ...
    public class MonteCarloIntegration : INumericalIntegration, IMultidimensionalIntegration
    {
        ...
        public bool Compute()
        {
            double[] sample = new double[this.NumberOfParameters];

            for (int j = 0; j < this.Iterations; j++)
            {
                for (int i = 0; i < sample.Length; i++)
                    sample[i] = this.Random.NextDouble() * this.Range[i].Length + this.Range[i].Min;

                double f = this.Function(sample);
            ...
            double volume = 1;
            for (int i = 0; i < this.Range.Length; i++)
                volume *= this.Range[i].Length;

            double avg = this.sum / this.count;
            double avg2 = this.sum2 / this.count;

            this.Area = volume * avg;
            this.Error = volume * Math.Sqrt((avg2 - avg * avg) / this.count);
        ...
        public object Clone()
        {
            ...
            clone.Range = (DoubleRange[])this.Range.Clone();
