...
namespace FileFormat.Accord.Math.Differentiation
{
    ...
    public class FiniteDifferences
    {
        ...
        {
            get { return this.stepSizes.Max(); }
            set
            {
                if (this.stepSizes == null)
                    this.stepSizes = new double[this.numberOfVariables];
                for (int i = 0; i < this.stepSizes.Length; i++)
                    this.stepSizes[i] = value;
            }
        }
        ...
        public int Order
        {
            get { return this.derivativeOrder; }
            set
            {
        ...
        public int NumberOfPoints
        {
            ...
            set
            {
                ...
                this.points = new ThreadLocal<double[][]>(() => Jagged.Zeros(2, this.coefficients.Length));
                this.center = (this.coefficients.Length - 1) / 2;
            }
        }
        ...
        {
            this.init(variables: variables, order: order);
        }
        ...
        {
            this.init(variables: variables, order: order, stepSize: stepSize);
        }
        ...
        {
            this.init(variables, function: function);
        }
        ...
        {
            this.init(variables: variables, function: function, order: order);
        }
        ...
        {
            this.init(variables: variables, function: function, order: order, stepSize: stepSize);
        }
        ...
        public double[] Gradient(double[] x, double[] result)
        {
            ...
            if (x.Length != this.numberOfVariables)
            ...
            double centerValue = this.Function(x);

            for (int i = 0; i < result.Length; i++)
                result[i] = this.derivative(this.Function, x, i, pointCache, centerValue);
            return result;
        }
        ...
        {
            return this.Hessian(x, Jagged.Zeros(x.Length, x.Length));
        }
        ...
        public double[][] Hessian(double[] x, double[][] result)
        {
            ...
            {
                Func<double[], double> f = (double[] x0) => this.derivative(this.Function, x0, i, pointCache2, this.Function(x0));

                // Process elements at the diagonal
                result[i][i] = this.derivative(f, x, i, pointCache1, centerValue);

                // Process off-diagonal elements
                for (int j = 0; j < i; j++)
                {
                    double d = this.derivative(f, x, j, pointCache1, centerValue);
        ...
        private double derivative(Func<double[], double> func, double[] x, int index, double[] points, double centerValue)
        {
            ...
            double step = this.GetUniformlySampledPoints(func, x, index, centerValue, points);

            return Interpolate(this.coefficients, points, this.derivativeOrder, this.center, step);
        }
        ...
        private double GetUniformlySampledPoints(Func<double[], double> func, double[] x, int index, double centerValue, double[] points)
        {
            ...
            for (int i = 0; i < points.Length; i++)
            {
                if (i != this.center)
                {
                    ...
                    x[index] = original + (i - this.center) * step;
