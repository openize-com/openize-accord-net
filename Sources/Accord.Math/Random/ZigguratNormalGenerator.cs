...
namespace FileFormat.Accord.Math.Random
{
    ...
    public class ZigguratNormalGenerator : IRandomNumberGenerator<double>
    {
        ...
        public double Generate()
        {
            ...
            int hz = unchecked((int)this.u.Next());
            uint iz = (uint)(hz & 127);
            ...
            for (; ; )
            {
                ...
                if (iz == 0)
                {
                    for (; ; )
                    {
                        x = -0.2904764 * Math.Log(this.u.Generate());
                        y = -Math.Log(this.u.Generate());
                        if (x * x <= y + y)
                        {
                            break;
                        }
                ...
                if (this.fn[iz] + this.u.Generate() * (this.fn[iz - 1] - this.fn[iz]) < Math.Exp(-0.5 * x * x))
                    return x;

                hz = unchecked((int)this.u.Next());
                iz = (uint)(hz & 127);
