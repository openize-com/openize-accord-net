...
namespace Openize.Accord.Math.Optimization
{
    ...
    public class QuadraticObjectiveFunction : NonlinearObjectiveFunction, IObjectiveFunction
    {
        ...
        /// </summary>
        /// 
        public double[,] QuadraticTerms { get { return this.Q; } }

        /// <summary>
        ///   Gets the vector of linear terms of the quadratic function.
        /// </summary>
        /// 
        public double[] LinearTerms { get { return this.d; } }
        ...
        public QuadraticObjectiveFunction(double[,] quadraticTerms, double[] linearTerms, params string[] variables)
        {
            ...
            this.d = linearTerms;
            base.NumberOfVariables = this.d.Length;
        ...
        private void initialize(Dictionary<Tuple<string, string>, double> terms)
        {
            ...
            {
                this.InnerVariables.Add(variable, i);
                this.InnerIndices.Add(i, variable);
                i++;
            }

            this.NumberOfVariables = this.Variables.Count;
            this.Q = this.createQuadraticTermsMatrix(quadratic);
            this.d = this.createLinearTermsVector(linear);
        ...
        {
            int n = this.Variables.Count;

            double[,] Q = new double[n, n];
            for (int i = 0; i < n; i++)
            {
                var x = this.Indices[i];
                for (int j = 0; j < n; j++)
                {
                    var y = this.Indices[j];
        ...
        {
            double a = 0.5 * input.DotAndDot(this.Q, input);
            double b = input.Dot(this.d);
            return a + b + this.ConstantTerm;
        }

        private double[] gradient(double[] input)
        {
            double[] g = this.Q.Dot(input);
            g.Add(this.d, g);
        ...
        {
            var simpleString = string.Format("{0}-dimensional quadratic objective function", this.NumberOfVariables);
            ...
            for (int i = 0; i < this.NumberOfVariables; i++)
            {
                ...
                sb.AppendFormat("{0:+#;-#}{1}² ", 0.5 * this.Q[i, i], this.InnerIndices[i]);
            }
            ...
            for (int i = 0; i < this.NumberOfVariables; i++)
            {
                for (int j = i + 1; j < this.NumberOfVariables; j++)
                {
                    ...
                    sb.AppendFormat("{0:+#;-#}{1}{2} ", this.Q[i, j], this.InnerIndices[i], this.InnerIndices[j]);
                }
            }
            ...
            for (int i = 0; i < this.NumberOfVariables; i++)
            {
                ...
                sb.AppendFormat("{0:+#;-#}{1} ", this.d[i], this.InnerIndices[i]);
            }


            if (this.ConstantTerm != 0)
                sb.AppendFormat("{0:+#;-#} ", this.ConstantTerm);
