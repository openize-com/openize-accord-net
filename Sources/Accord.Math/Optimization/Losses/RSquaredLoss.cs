...
namespace Openize.Accord.Math.Optimization.Losses
{
    ...
    public class RSquaredLoss : LossBase<double[][], double[][], double[]>, ILoss<double[]>
    {
        ...
        public double Loss(double[] actual)
        {
            ...
            double avg = this.Expected.WeightedMean(this.Weights).Mean();
            ...
            for (int i = 0; i < this.Expected.Length; i++)
            {
                ...
                double d = this.Expected[i][0] - actual[i];
                SSe += w * d * d;

                d = this.Expected[i][0] - avg;
                SSt += w * d * d;
            }
        ...
        public override double[] Loss(double[][] actual)
        {
            ...
            double[] avg = this.Expected.WeightedMean(this.Weights);
            ...
            for (int i = 0; i < N; i++)
            {
                for (int c = 0; c < M; c++)
                {
                    ...
                    double d;
                    d = this.Expected[i][c] - actual[i][c];
                    SSe[c] += w * d * d;

                    d = this.Expected[i][c] - avg[c];
                    SSt[c] += w * d * d;
                }
            }
