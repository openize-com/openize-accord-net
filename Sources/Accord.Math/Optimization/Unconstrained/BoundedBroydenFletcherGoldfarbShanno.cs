...
namespace FileFormat.Accord.Math.Optimization.Unconstrained
{
        ...
        ISupportsCancellation
    {
        ...
        public double[] UpperBounds
        {
            ...
            set
            {
                if (value.Length != this.upperBound.Length)
                    throw new DimensionMismatchException("value", "The bounds vector should have the same length as the number of variables to be optimized ({0}.".Format(this.NumberOfVariables));
        ...
        public double[] LowerBounds
        {
            ...
            set
            {
                if (value.Length != this.lowerBound.Length)
                    throw new DimensionMismatchException("value", "The bounds vector should have the same length as the number of variables to be optimized ({0}.".Format(this.NumberOfVariables));
        ...
        {
            get { return this.pgtol; }
            set { this.pgtol = value; }
        }
            ...
            : base(numberOfVariables)
        {
            ...
            for (int i = 0; i < this.upperBound.Length; i++)
                this.lowerBound[i] = Double.NegativeInfinity;

            for (int i = 0; i < this.upperBound.Length; i++)
                this.upperBound[i] = Double.PositiveInfinity;
        }
        ...
        protected override bool Optimize()
        {
            ...
            NonlinearObjectiveFunction.CheckGradient(this.Gradient, this.Solution);


            int n = this.NumberOfVariables;
            int m = this.corrections;
            ...
            if (this.work == null || this.work.Length < totalSize)
                this.work = new double[totalSize];
...

            {
                ...
                {
                    bool hasUpper = !Double.IsInfinity(this.UpperBounds[i]);
                    bool hasLower = !Double.IsInfinity(this.LowerBounds[i]);
                    ...
                    if (hasLower)
                        l[i] = this.LowerBounds[i];
                    if (hasUpper)
                        u[i] = this.UpperBounds[i];
                }
            }
            ...
            {
                this.exit(csave, lsave, isave, x, dsave, out newF, out newG);
                return true;
            }
            ...
            setulb(n, m, x, 0, l, 0, u, 0, nbd, 0, ref f, g, 0,
                this.factr, this.pgtol, this.work, 0, iwa, 0, ref task, iprint, ref csave,
            ...
            {
                newF = this.Function(x);
                newG = this.Gradient(x);
                this.evaluations++;
            ...
            else
            {
                if (task == "ABNORMAL_TERMINATION_IN_LNSRCH")
                    this.Status = BoundedBroydenFletcherGoldfarbShannoStatus.LineSearchFailed;
                else if (task == "CONVERGENCE: REL_REDUCTION_OF_F_<=_FACTR*EPSMCH")
                    this.Status = BoundedBroydenFletcherGoldfarbShannoStatus.FunctionConvergence;
                else if (task == "CONVERGENCE: NORM_OF_PROJECTED_GRADIENT_<=_PGTOL")
                    this.Status = BoundedBroydenFletcherGoldfarbShannoStatus.GradientConvergence;
            ...
            {
                this.Progress(this, new OptimizationProgressEventArgs(this.iterations, 0, newG, 0, null, 0, f, 0, false)
                {
                    Tag = new BoundedBroydenFletcherGoldfarbShannoInnerStatus(
                            isave, dsave, lsave, csave, this.work)
                });
            }
        ...
        private void exit(string csave, bool[] lsave, int[] isave, double[] x, double[] dsave, out double newF, out double[] newG)
        {
            ...
            if (this.Progress != null)
            {
                this.Progress(this, new OptimizationProgressEventArgs(this.iterations, 0, newG, 0, null, 0, newF, 0, true)
                {
                    Tag = new BoundedBroydenFletcherGoldfarbShannoInnerStatus(
                        isave, dsave, lsave, csave, this.work)
                });
