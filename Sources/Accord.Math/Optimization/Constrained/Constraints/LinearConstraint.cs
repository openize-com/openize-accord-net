...
namespace Openize.Accord.Math.Optimization.Constrained.Constraints
{
    ...
    public class LinearConstraint : IConstraint
    {
        ...
        public LinearConstraint(IObjectiveFunction function, string constraint, CultureInfo format)
            : this()
        {
            this.parseString(function, constraint, format);
        }
        ...
        public LinearConstraint(IObjectiveFunction function, Expression<Func<bool>> constraint)
            : this()
        {
            this.parseExpression(function, constraint);
        }
        ...
        public double[] Gradient(double[] x)
        {
            if (this.grad == null)
            {
                if (x.Length == this.indices.Length && this.indices.IsEqual(Vector.Range(x.Length)))
                {
                    this.grad = this.combinedAs;
                }
                else
                {
        ...
        private void parseString(IObjectiveFunction function, string constraint, CultureInfo culture)
        {
            ...
            this.NumberOfVariables = indices.Count;
            this.VariablesAtIndices = indices.ToArray();
            this.CombinedAs = scalars.ToArray();
            this.ShouldBe = type;
            this.Value = value;
        }
