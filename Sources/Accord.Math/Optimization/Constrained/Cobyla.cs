...
namespace FileFormat.Accord.Math.Optimization.Constrained
{
        ...
        IOptimizationMethod<CobylaStatus>
    {
        ...
        */
        ...
        {
            get { return this.rhobeg; }
            set { this.rhobeg = value; }
        }
        ...
        {
            get { return this.rhoend; }
            set { this.rhoend = value; }
        }
        ...
        {
            get { return this.maxfun; }
            set { this.maxfun = value; }
        }
            ...
            for (int i = 0; i < constraints.Length; i++)
            {
                if (constraints[i].NumberOfVariables != function.NumberOfVariables)
                    throw new DimensionMismatchException("constraints", "The constraint at position " + i +
                        " contains an unexpected number of variables. Expected value would be " + this.NumberOfVariables + ".");
        ...
        {
            this.Status = this.cobyla();
            return this.Status == CobylaStatus.Success;
        }
        ...
        L_40:
            if (this.iterations >= this.maxfun && this.iterations > 0)
            {
            ...
            // Compute constraints
            for (int i = 1; i <= m; i++)
                con[i] = this.constraints[i - 1].GetViolation(x);
