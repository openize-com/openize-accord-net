...
namespace FileFormat.Accord.Math.Decompositions
{
    ...
    public sealed class LuDecompositionD : ICloneable, ISolverMatrixDecomposition<Decimal>
    {
        ...
        public LuDecompositionD(Decimal[,] value, bool transpose, bool inPlace)
        {
            ...
            var LUcolj = new Decimal[this.rows];


            unsafe
            {
                fixed (Decimal* LU = this.lu)
                {
                    // Outer loop.
        ...
        public Decimal[,] LowerTriangularFactor
        {
            get
            {
                ...
                {
                    var L = new Decimal[this.rows, this.rows];
        ...
        public Decimal[,] UpperTriangularFactor
        {
            get
            {
                ...
                {
                    var U = new Decimal[this.rows, this.cols];
                    for (int i = 0; i < this.rows; i++)
                    {
                        for (int j = 0; j < this.cols; j++)
                        {
        ...
        public Decimal[,] Inverse()
        {
            ...
            var X = new Decimal[this.rows, this.rows];
            for (int i = 0; i < this.rows; i++)
            {
        ...
        public Decimal[,] Solve(Decimal[,] value)
        {
            ...
            Decimal[,] X = value.Get(this.pivotVector, null);
