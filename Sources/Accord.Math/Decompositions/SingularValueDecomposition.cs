...
namespace Openize.Accord.Math.Decompositions
{
    ...
    public sealed class SingularValueDecomposition : ICloneable, ISolverMatrixDecomposition<Double>
    {
        ...
        {
            get { return Constants.DoubleEpsilon * System.Math.Max(this.m, this.n) * this.s[0]; }
        }
        ...
        public int Rank
        {
            get
            {
                ...
                Double tol = System.Math.Max(this.m, this.n) * this.s[0] * eps;
        ...
        public Double LogPseudoDeterminant
        {
            get
            {
                if (!this.lnpseudoDeterminant.HasValue)
                {
                    ...
                    this.lnpseudoDeterminant = (Double)det;
                }
                ...
                else
                {
                 ...
                 switch (kase)
                 {
                    ...
                    case 1:
                    {
                       ...
                       for (int j = p - 2; j >= k; j--) 
                       {
                          Double t = Tools.Hypotenuse(this.s[j],f);
                    ...
                    case 4:
                        {
                            ...
                            if (this.s[k] <= 0)
                            {
                                this.s[k] = (this.s[k] < 0 ? -this.s[k] : (Double)0);
        ...
        public Double[,] SolveForDiagonal(Double[] value)
        {
            ...
            //(V x L*) x Ut x Y
            Double[,] VL = Matrix.Dot(this.v, Ls);
        ...
        public Double[,] Inverse()
        {
            ...
            var X = new Double[vrows, this.s.Rows()];
            for (int i = 0; i < vrows; i++)
            {
        ...
        public object Clone()
        {
            ...
            svd.m = this.m;
            svd.n = this.n;
            svd.s = (Double[])this.s.Clone();
            svd.si = (int[])this.si.Clone();
            svd.swapped = this.swapped;
            if (this.u != null) svd.u = (Double[,])this.u.MemberwiseClone();
            if (this.v != null) svd.v = (Double[,])this.v.MemberwiseClone();
