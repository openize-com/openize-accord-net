...
namespace FileFormat.Accord.Math.Decompositions
{
    ...
    public sealed class SingularValueDecompositionF : ICloneable, ISolverMatrixDecomposition<Single>
    {
        ...
        {
            get { return Constants.SingleEpsilon * System.Math.Max(this.m, this.n) * this.s[0]; }
        }
        ...
        public int Rank
        {
            get
            {
                ...
                Single tol = System.Math.Max(this.m, this.n) * this.s[0] * eps;
        ...
        public Single[,] DiagonalMatrix
        {
			...
			{	
				...
				return this.diagonalMatrix = Matrix.Diagonal(this.u.Columns(), this.v.Columns(), this.s);
			}
        }
        ...
        public Single LogPseudoDeterminant
        {
            get
            {
                if (!this.lnpseudoDeterminant.HasValue)
                {
                    ...
                    this.lnpseudoDeterminant = (Single)det;
                }
           ...
           bool computeLeftSingularVectors, bool computeRightSingularVectors, bool autoTranspose, bool inPlace)
        {
            ...
            if (this.m < this.n) // Check if we are violating JAMA's assumption
            {
                ...
                else
                {
                    ...
                    this.n = value.Rows();    // rows
                    this.m = value.Columns(); // cols
                    this.swapped = true;
            ...
            this.s = new Single[ni];
            this.u = new Single[this.m, nu];
            this.v = new Single[this.n, this.n];

            Single[] e = new Single[this.n];
            Single[] work = new Single[this.m];
                ...
                else
                {
                 ...
                 switch (kase)
                 {
                    ...
                    case 1:
                    {
                       ...
                       for (int j = p - 2; j >= k; j--) 
                       {
                          Single t = Tools.Hypotenuse(this.s[j],f);
                          Single cs = this.s[j] / t;
                          Single sn = f / t;
                          this.s[j] = t;
                          if (j != k) 
                          {
                    ...
                    case 3:
                        {
                           ...
                           Single sp = this.s[p-1] / scale;
                           Single spm1 = this.s[p-2] / scale;
                           Single epm1 = e[p-2] / scale;
                           Single sk = this.s[k] / scale;
                           Single ek = e[k] / scale;
                    ...
                    case 4:
                        {
                            ...
                            if (this.s[k] <= 0)
                            {
                                this.s[k] = (this.s[k] < 0 ? -this.s[k] : (Single)0);
        ...
        public Single[,] SolveForDiagonal(Single[] value)
        {
            ...
            //(V x L*) x Ut x Y
            Single[,] VL = Matrix.Dot(this.v, Ls);
        ...
        public Single[,] Inverse()
        {
            ...
            // X = V*S^-1
            int vrows = this.v.Rows();
            int vcols = this.v.Columns();
            var X = new Single[vrows, this.s.Rows()];
            for (int i = 0; i < vrows; i++)
            {
        ...
        public object Clone()
        {
            ...
            svd.m = this.m;
            svd.n = this.n;
            svd.s = (Single[])this.s.Clone();
            svd.si = (int[])this.si.Clone();
            svd.swapped = this.swapped;
            if (this.u != null) svd.u = (Single[,])this.u.MemberwiseClone();
            if (this.v != null) svd.v = (Single[,])this.v.MemberwiseClone();
