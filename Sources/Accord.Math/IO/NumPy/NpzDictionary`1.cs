...
namespace FileFormat.Accord.Math.IO.NumPy
{
        ...
        IList, ICollection, IEnumerable, IStructuralComparable, IStructuralEquatable
    {
        ...
        {
            get { return this.entries.Values.Select(this.OpenEntry); }
        }
        ...
        {
            get { return ((ICollection)this.entries).SyncRoot; }
        }
        ...
        {
            get { return ((ICollection)this.entries).IsSynchronized; }
        }
        ...
        {
            get { return this.OpenEntry(this.entries[key]); }
        }
        ...
        {
            T array;
            if (this.arrays.TryGetValue(entry.Name, out array))
                return array;

            Stream s = entry.Open();
            array = this.Load(s);
            this.arrays[entry.Name] = array;
            return array;
        }
        ...
        {
            foreach (var entry in this.archive.Entries)
                yield return new KeyValuePair<string, T>(entry.Name, this.OpenEntry(entry));
        }
        ...
        {
            foreach (var entry in this.archive.Entries)
                yield return this.OpenEntry(entry);
        }
