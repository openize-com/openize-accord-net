...
namespace Openize.Accord.IO
{
    ...
    public class UnipenReader : IDisposable
    {
        ...
        private void init(Stream input, bool compressed = true)
        {
            ...
            this.Lexicon = new string[parts.Length - 1];
            for (int i = 0; i < this.Lexicon.Length; i++)
                this.Lexicon[i] = parts[i + 1].Trim().TrimStart('"').TrimEnd('"');
        ...
        {
            double[][] coordinates;
            if (this.TryRead(out coordinates, out label, out comment))
                return coordinates;
            throw new FormatException();
        }
        ...
        private bool TryRead(out double[][] coordinates, out string label, out string comment)
        {
            ...
            if (!this.Lexicon.Contains(label))
                return false;

            // Read .COMMENT line
            comment = this.reader.ReadLine().Replace(".COMMENT", "").Trim();
            ...
            coordinates = this.buffer.ToArray();
            return true;
        }
        ...
        public Tuple<double[][][], string[]> ReadToEnd()
        {
            ...
            while (this.TryRead(out coordinates, out label, out comment))
            {
