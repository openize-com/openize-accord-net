...
namespace FileFormat.Accord.IO
{
    ...
    public class SparseReader : IDisposable
    {
        ...
        /// </summary>
        /// 
        public List<string> SampleDescriptions { get { return this.descriptions; } }
        ...
        public int NumberOfInputs
        {
            get
            {
                if (this.sampleSize <= 0)
                    this.sampleSize = this.guessSampleSize();

                return this.Intercept.HasValue ? this.sampleSize + 1 : this.sampleSize;
            }
        }
        ...
        {
            this.createReader(path, encoding);
            this.sampleSize = sampleSize;
        }
        ...
        public Tuple<string[], string> ReadLine()
        {
            ...
            this.SampleDescriptions.Add(description);
        ...
        public Tuple<Sparse<double>, double> ReadSparse()
        {
            ...
            double output;
            this.Read(out sample, out output);
        ...
        public Tuple<Sparse<double>[], double[]> ReadSparse(int count)
        {
            ...
            double[] outputs;
            this.Read(count, out samples, out outputs);
        ...
        {
            this.Read(-1, out samples, out outputs);
        }
        ...
        {
            var sparse = this.ReadSparse();
            return Tuple.Create(sparse.Item1.ToDense(this.NumberOfInputs), sparse.Item2);
        }
        ...
        {
            var sparse = this.ReadSparse(count);
            var dense = new double[sparse.Item1.Length][];
            for (int i = 0; i < dense.Length; i++)
                dense[i] = sparse.Item1[i].ToDense(this.NumberOfInputs);
