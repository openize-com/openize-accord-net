...
namespace Openize.Accord.IO
{
    ...
    public class IdxReader : IDisposable
    {
        ...
        {
            this.init(new FileStream(path, FileMode.Open, FileAccess.Read), compressed);
        }
        ...
        {
            this.init(new MemoryStream(file), compressed);
        }
        ...
        {
            this.init(input, compressed);
        }

        private void init(Stream input, bool compressed)
        {
            if (compressed)
                this.reader = new BinaryReader(new GZipStream(input, CompressionMode.Decompress));
            else this.reader = new BinaryReader(input);

            this.Magic = this.reader.ReadInt32();
            ...
            this.DataType = (IdxDataType)type;
            this.type = Translate(this.DataType);

            this.Samples = reverseBytes(this.reader.ReadInt32());

            this.Dimensions = new int[dimensions - 1];
            for (int i = 0; i < this.Dimensions.Length; i++)
                this.Dimensions[i] = reverseBytes(this.reader.ReadInt32());

            this.arrayLength = 1;
            for (int i = 0; i < this.Dimensions.Length; i++)
                this.arrayLength *= this.Dimensions[i];

            this.dataSize = Marshal.SizeOf(type);
            this.buffer = new byte[this.dataSize * this.arrayLength];
        }
        ...
        {
            int readBytes = this.reader.Read(this.buffer, 0, this.buffer.Length);
            Buffer.BlockCopy(this.buffer, 0, array, 0, array.Length);
        ...
        {
            Array array = Array.CreateInstance(this.type, this.arrayLength);
        ...
        public T[][,] ReadToEndAsMatrices<T>()
        {
            ...
            T[,] current = this.ReadMatrix<T>();

            while (current != null)
            {
                matrices.Add(current);
                current = this.ReadMatrix<T>();
            }
        ...
        public T[][] ReadToEndAsVectors<T>()
        {
            ...
            T[] current = this.ReadVector<T>();

            while (current != null)
            {
                vectors.Add(current);
                current = this.ReadVector<T>();
            }
        ...
        public T[] ReadToEndAsValues<T>()
        {
            ...
            while (this.TryReadValue(out current))
            {
                vectors.Add(current);
            }
