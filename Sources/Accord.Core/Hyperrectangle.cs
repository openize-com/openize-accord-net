...
namespace FileFormat.Accord.Core
{
    using System;
    using Exceptions;
    ...
    public struct Hyperrectangle : ICloneable, IEquatable<Hyperrectangle>, IFormattable
    {
        ...
        {
            double[] length = new double[this.min.Length];
            for (int i = 0; i < length.Length; i++)
                length[i] = this.max[i] - this.min[i];
            return length;
        }
        ...
        public bool IntersectsWith(Hyperrectangle rect)
        {
            ...
            {
                double amini = this.min[i];
                double amaxi = this.max[i];
        ...
        public bool Contains(params double[] point)
        {
            ...
            {
                double mini = this.min[i];
                double maxi = this.max[i];
        ...
        public object Clone()
        {
            return new Hyperrectangle((double[])this.min.Clone(), (double[])this.max.Clone());
        }
        ...
        public string ToString(string format, IFormatProvider formatProvider = null)
        {
            ...
            if (this.NumberOfDimensions == 2)
            {
                return String.Format(formatProvider, format, 
                    ...
                    this.min[0], this.min[1], this.max[0] - this.min[0], this.max[1] - this.min[1]);
            }

            return String.Format(formatProvider, format,
                "Min = {0} Max = {1} (Length = {2})", 
                this.min, this.max, this.GetLength());
        }
    }
}