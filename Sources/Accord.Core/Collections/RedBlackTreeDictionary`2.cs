...
namespace Openize.Accord.Core.Collections
{
    ...
    public class RedBlackTreeDictionary<TKey, TValue> : IDictionary<TKey, TValue>
    {
        ...
        private void init(IComparer<TKey> comparer)
        {
            ...
            this.values = new ValueCollection(this.tree);
            this.keys = new KeyCollection(this.tree);
        }
        ...
        {
            var node = this.tree.Find(item);

            if (node == null)
                return false;

            var result = this.tree.Remove(node);
        ...
        {
            var result = this.tree.Find(item);
        ...
        public bool TryGetValue(TKey key, out TValue value)
        {
            ...
            var result = this.tree.Find(new KeyValuePair<TKey, TValue>(key, value));
        ...
        public TValue this[TKey key]
        {
            get
            {
                var result = this.tree.Find(new KeyValuePair<TKey, TValue>(key, default(TValue)));
            ...
            set
            {
                ...
                var result = this.tree.Find(pair);
        ...
        {
            var node = this.tree.Find(new KeyValuePair<TKey, TValue>(key, default(TValue)));
            var prevNode = this.tree.GetPreviousNode(node);
        ...
        {
            var node = this.tree.Find(new KeyValuePair<TKey, TValue>(key, default(TValue)));
            var nextNode = this.tree.GetNextNode(node);
        ...
        internal class ValueCollection : ICollection<TValue>
        {
            ...
            {
                foreach (var node in this.owner)
