...
namespace Openize.Accord.Statistics.Distributions.Univariate.Base
{
        ...
        IFormattable
    {
        ...
        public virtual double Mode
        {
            ...
            {
                if (this.mode == null)
                    this.mode = BrentSearch.Maximize(this.ProbabilityDensityFunction, this.Quartiles.Min, this.Quartiles.Max, 1e-10);
        ...
        public virtual DoubleRange GetRange(double percentile)
        {
            ...
            double a = this.InverseDistributionFunction(1.0 - percentile);
            double b = this.InverseDistributionFunction(percentile);
        ...
        {
            return this.DistributionFunction(x[0]);
        }
        ...
        {
            return this.ComplementaryDistributionFunction(x[0]);
        }
        ...
        {
            return this.ProbabilityDensityFunction(x[0]);
        }
        ...
        {
            return this.LogProbabilityDensityFunction(x[0]);
        }
        ...
        double DistributionFunction(double x)
        {
            ...
            if (Double.IsNegativeInfinity(this.Support.Min) && Double.IsNegativeInfinity(x))
                return 0;

            if (x < this.Support.Min) // Needed by: InverseChiSquare
                return 0;
            if (x >= this.Support.Max) // Needed by: GrubbDistribution
                return 1;

            double result = this.InnerDistributionFunction(x);
        ...
        double DistributionFunction(double a, double b)
        {
            ...
            return this.DistributionFunction(b) - this.DistributionFunction(a);
        }
        ...
        double ComplementaryDistributionFunction(double x)
        {
            ...
            if (x < this.Support.Min)
                return 1;

            if (x >= this.Support.Max)
                return 0;

            double result = this.InnerComplementaryDistributionFunction(x);
...
[Range(0, 1)]
            ...
            double p)
        {
            ...
            double result = this.InnerInverseDistributionFunction(p);
        ...
        protected internal virtual double InnerInverseDistributionFunction(double p)
        {
            ...
            {
                lower = this.Support.Min;
                upper = this.Support.Max;
                f = 0.5;
            }
        ...
        {
            return 1.0 / this.ProbabilityDensityFunction(this.InverseDistributionFunction(p));
        }
        ...
        double ProbabilityDensityFunction(double x)
        {
            ...
            if (x < this.Support.Min || x > this.Support.Max)
                return 0;

            double result = this.InnerProbabilityDensityFunction(x);
        ...
        double LogProbabilityDensityFunction(double x)
        {
            ...
            double result = this.InnerLogProbabilityDensityFunction(x);
        ...
        {
            double f = this.ProbabilityDensityFunction(x);
            if (f == 0)
                return 0;

            double s = this.ComplementaryDistributionFunction(x);
            return f / s;
        }
        ...
        {
            double result = Math.Log(-Math.Log(this.ComplementaryDistributionFunction(x)));
#if DEBUG
            double expected = Math.Log(this.CumulativeHazardFunction(x));
