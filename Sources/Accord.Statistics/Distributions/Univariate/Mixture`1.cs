...
#pragma warning restore 612, 618
        ...
        public override double Mean
        {
            get
            {
                if (this.mean == null)
                {
                    double mu = 0.0;
                    for (int i = 0; i < this.coefficients.Length; i++)
                        mu += this.coefficients[i] * this.components[i].Mean;
                    this.mean = mu;
                }
        ...
        public override double Variance
        {
            get
            {
                if (this.variance == null)
                {
                    ...
                    for (int i = 0; i < this.coefficients.Length; i++)
                    {
                        double w = this.coefficients[i];
                        double m = this.components[i].Mean;
                        double v = this.components[i].Variance;
                        var += w * (v + m * m);
                    }
        ...
        public override DoubleRange Support
        {
            get
            {
                if (this.range == null)
                {
                    double min = this.Components.Min(p => p.Support.Min);
                    double max = this.Components.Max(p => p.Support.Max);
        ...
        public override double Generate(Random source)
        {
            ...
            // Sample from the chosen coefficient
            return this.sampleable[j].Generate();
        }
