...
namespace FileFormat.Accord.Statistics.Distributions.Univariate.Discrete
{
        ...
        IFittableDistribution<double>, IFittableDistribution<int>
    {
        ...
        {
            get { return this.numberOfTrials; }
        }
        ...
        {
            get { return this.numberOfTrials * this.probability; }
        }
        ...
        {
            get { return this.numberOfTrials * this.probability * (1 - this.probability); }
        }
        ...
        public override double Mode
        {
            get
            {
                double test = (this.numberOfTrials + 1) * this.probability;

                if (test <= 0 || (int)test != test)
                    return Math.Floor(test);

                if (test <= this.numberOfTrials)
                    return test; // TODO: should return test and test - 1 (multimodal)

                if (test == this.numberOfTrials + 1)
        ...
        {
            get { return new IntRange(0, this.numberOfTrials); }
        }
        ...
        protected internal override double InnerDistributionFunction(int k)
        {
            if (k < 0)
                return 0;
            if (k >= this.numberOfTrials)
                return 1;

            double x = 1.0 - this.probability;
            double a = this.numberOfTrials - k;
            double b = k + 1;
        ...
        {
            int result = this.numberOfTrials;

            for (int i = 0; i < this.numberOfTrials; i++)
            {
                if (this.DistributionFunction(i) >= p)
                {
                    result = i;
                    break;
                }
        ...
        {
            double a = Special.LogBinomial(this.numberOfTrials, k);
            double b = k == 0 ? 0 : k * Math.Log(this.probability);
            double c = (this.numberOfTrials - k);
            double d = Math.Log(1 - this.probability);
            double log = a + b + c * d;
        ...
        public override void Fit(double[] observations, double[] weights, IFittingOptions options)
        {
            ...
            for (int i = 0; i < observations.Length; i++)
                sum += observations[i] / (double)this.numberOfTrials;
        ...
        {
            return new BinomialDistribution(this.numberOfTrials, this.probability);
        }
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            return String.Format(formatProvider, "Binomial(x; n = {0}, p = {1})",
                this.numberOfTrials.ToString(format, formatProvider),
                this.probability.ToString(format, formatProvider));
        }
    }
