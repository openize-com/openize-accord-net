...
namespace FileFormat.Accord.Statistics.Distributions.Univariate.Discrete
{
        ...
        IFittableDistribution<double, HypergeometricOptions>
    {
        ...
        {
            get { return this.n * (this.m / (double)this.N); }
        }
        ...
        {
            get { return (double)this.n * (this.m / (double)this.N) * ((this.N - this.m) / (double)this.N) * ((this.N - this.n) / (this.N - 1.0)); }
        }
        ...
        {
            get { return new IntRange(Math.Max(0, this.n + this.m - this.N), Math.Min(this.m, this.n)); }
        }
        ...
        {
            double a = Special.Binomial(this.m, k);
            double b = Special.Binomial(this.N - this.m, this.n - k);
            double c = Special.Binomial(this.N, this.n);
        ...
        {
            double a = Special.LogBinomial(this.m, k);
            double b = Special.LogBinomial(this.N - this.m, this.n - k);
            double c = Special.LogBinomial(this.N, this.n);
        ...
        public override void Fit(double[] observations, double[] weights, IFittingOptions options)
        {
            ...
            this.Fit(observations, weights, geometricOptions);
        }
        ...
        public void Fit(double[] observations, double[] weights, HypergeometricOptions options)
        {
            ...
            if (options.Parameter == HypergeometricParameter.PopulationSize)
            {
                ...
                int newN = (int)Math.Truncate(this.n * this.m / (double)successes);
                this.N = newN;
            }
            else if (options.Parameter == HypergeometricParameter.PopulationSuccesses)
            {
                // Estimate m (number of successes in population)
                int newM = (int)Math.Truncate(successes * (this.N + 1.0) / (double)this.n);
                this.m = newM;
            }
            else
            {
        ...
        {
            return new HypergeometricDistribution(this.N, this.m, this.n);
        }
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            return String.Format(formatProvider, "HyperGeometric(x; N = {0}, m = {1}, n = {2})",
                this.N.ToString(format, formatProvider),
                this.m.ToString(format, formatProvider),
                this.n.ToString(format, formatProvider));
        }
