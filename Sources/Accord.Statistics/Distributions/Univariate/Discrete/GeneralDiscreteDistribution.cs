...
namespace FileFormat.Accord.Statistics.Distributions.Univariate.Discrete
{
        ...
        ISampleableDistribution<int>, ISampleableDistribution<double>
    {
        ...
        public GeneralDiscreteDistribution(bool logarithm, params double[] probabilities)
        {
            ...
            this.initialize(0, probabilities, logarithm);
        }
        ...
        public GeneralDiscreteDistribution(int start, params double[] probabilities)
        {
            ...
            this.initialize(start, probabilities, false);
        }
        ...
        public GeneralDiscreteDistribution(int start, int symbols, bool logarithm = false)
        {
            ...
            this.initialize(start, symbols, logarithm);
        }
        ...
        {
            get { return this.probabilities[i]; }
            set { this.probabilities[i] = value; }
        }
        ...
        public int Maximum
        {
            get { return this.start + this.probabilities.Length; }
        }
        ...
        public int Length
        {
            get { return this.probabilities.Length; }
        }
        ...
        public double[] Frequencies
        {
            ...
            set
            {
                if (!this.probabilities.DimensionEquals(value))
        ...
        public override double Mean
        {
            get
            {
                if (!this.mean.HasValue)
                {
                    if (this.log)
                    {
                        this.mean = this.start;
                        for (int i = 0; i < this.probabilities.Length; i++)
                            this.mean += i * Math.Exp(this.probabilities[i]);
                    }
                    else
                    {
                        this.mean = this.start;
                        for (int i = 0; i < this.probabilities.Length; i++)
                            this.mean += i * this.probabilities[i];
                    }
                }
            ...
            get
            {
                if (!this.variance.HasValue)
                {
                    ...
                    if (this.log)
                    {
                        for (int i = 0; i < this.probabilities.Length; i++)
                        {
                            double d = i + this.start - m;
                            v += Math.Exp(this.probabilities[i]) * (d * d);
                        }
                    }
                    else
                    {
        ...
        public override double Mode
        {
            get
            {
                if (!this.mode.HasValue)
                    this.mode = this.probabilities.ArgMax();
        ...
        public override double Entropy
        {
            get
            {
                if (!this.entropy.HasValue)
                {
                    ...
                    {
                        for (int i = 0; i < this.probabilities.Length; i++)
                            this.entropy -= Math.Exp(this.probabilities[i]) * System.Math.Log(this.probabilities[i]);
                    }
                    else
                    {
                        for (int i = 0; i < this.probabilities.Length; i++)
                            this.entropy -= this.probabilities[i] * System.Math.Log(this.probabilities[i]);
                    }
                }
        ...
        {
            get { return new IntRange(this.start, this.start + this.probabilities.Length - 1); }
        }
        ...
        {
            int value = k - this.start;

            if (this.log)
            {
                double sum = Double.NegativeInfinity;
                for (int i = 0; i <= value; i++)
                    sum = Special.LogSum(this.probabilities[i], sum);
                return sum;
            }
            else
            {
        ...
        protected internal override double InnerProbabilityMassFunction(int k)
        {
            ...
            if (this.log)
                return Math.Exp(this.probabilities[value]);
            return this.probabilities[value];
        }
        ...
        protected internal override double InnerLogProbabilityMassFunction(int k)
        {
            ...
            if (this.log)
                return this.probabilities[value];
            return Math.Log(this.probabilities[value]);
        }
        ...
        {
            return Random(this.probabilities, source, log: this.log);
        }
        ...
        {
            return Random(this.probabilities, samples, result, source, log: this.log);
        }
        ...
        public override void Fit(double[] observations, double[] weights, IFittingOptions options)
        {
            ...
            this.Fit(observations, weights, discreteOptions);
        }
        ...
        {
            this.Fit(observations.ToInt32(), weights, options);
        }
        ...
        {
            this.Fit(observations, weights, null);
        }
        ...
        public override object Clone()
        {
            ...
            c.probabilities = (double[])this.probabilities.Clone();
            c.start = this.start;
            c.mean = this.mean;
            c.entropy = this.entropy;
        ...
        private void initialize(int s, int symbols, bool logarithm)
        {
            ...
            for (int i = 0; i < symbols; i++)
                this.probabilities[i] = 1.0 / symbols;

            if (logarithm)
                this.probabilities.Log(result: this.probabilities);
        ...
        {
            return this.Generate(samples).ToDouble();
        }
        ...
        {
            return Random(probabilities, samples, global::FileFormat.Accord.Math.Random.Generator.Random);
        }
        ...
        {
            return Random(probabilities, samples, result, global::FileFormat.Accord.Math.Random.Generator.Random, log);
        }
        ...
        {
            return Random(probabilities, global::FileFormat.Accord.Math.Random.Generator.Random, log);
        }
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            ...
            if (this.log)
                return String.Format("Categorical(x; p = log({0}))", this.probabilities.ToString(format, provider));
            return String.Format("Categorical(x; p = {0})", this.probabilities.ToString(format, provider));
        }
        ...
        {
            var p = new double[this.probabilities.Length];
            ...
            if (weights == null)
            {
                ...
                {
                    int j = observations[i] - this.start;
                    p[j]++;
                }
            }
            else
            ...
            if (regularization > 0)
            {
                ...
                {
                    double prev = this.log ? Math.Exp(this.probabilities[i]) : this.probabilities[i];
            ...
            Debug.Assert(!p.HasNaN());

            if (this.log)
                p.Log(result: p);

            this.initialize(0, p, this.log);
        }
        ...
        {
            this.Fit(observations, null, null);
        }
