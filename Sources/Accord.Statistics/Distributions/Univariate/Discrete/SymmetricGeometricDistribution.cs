...
namespace FileFormat.Accord.Statistics.Distributions.Univariate.Discrete
{
    ...
    public class SymmetricGeometricDistribution : UnivariateDiscreteDistribution
    {
        ...
        public SymmetricGeometricDistribution([Unit] double probabilityOfSuccess)
        {
            ...
            this.lnconstant = Math.Log(this.p) - Math.Log(2.0 * (1.0 - this.p));
        }
        ...
        {
            get { return ((2 - this.p) * (1 - this.p)) / (this.p * this.p); }
        }
        ...
        {
            if (k == 0)
                return this.p; // need special case otherwise will be double at center
            return Math.Exp(this.lnconstant) * Math.Pow(1 - this.p, Math.Abs(k) - 1);
        }
        ...
        {
            if (k == 0)
                return Math.Log(this.p); // need special case otherwise will be double at center
            return this.lnconstant + (Math.Abs(k) - 1) * Math.Log(1 - this.p);
        }
        ...
        public override int Generate(Random source)
        {
            ...
            return Math.Sign(u - 0.5) * (int)GeometricDistribution.Random(this.p, source);
        }
        ...
        {
            GeometricDistribution.Random(this.p, samples, result, source);
        ...
        {
            return new SymmetricGeometricDistribution(this.p);
        }
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            return String.Format(formatProvider, "SymmetricGeometric(x; p = {0})",
                this.p.ToString(format, formatProvider));
        }
