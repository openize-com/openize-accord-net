...
namespace Openize.Accord.Statistics.Distributions.Univariate.Continuous
{
        ...
        ISampleableDistribution<double>, IFormattable
    {
        ...
        {
            get { return this.sigma * Math.Sqrt(Math.PI / 2.0); }
        }
        ...
        {
            get { return (4.0 - Math.PI) / 2.0 * this.sigma * this.sigma; }
        }
        ...
        {
            get { return 1 + Math.Log(this.sigma / Math.Sqrt(2)) + Constants.EulerGamma / 2.0; ; }
        }
        ...
        {
            return 1.0 - Math.Exp(-x * x / (2 * this.sigma * this.sigma));
        }
        ...
        {
            return x / (this.sigma * this.sigma) * Math.Exp(-x * x / (2 * this.sigma * this.sigma));
        }
        ...
        {
            return Math.Log(x / (this.sigma * this.sigma)) + (-x * x / (2 * this.sigma * this.sigma));
        }
        ...
        {
            return new RayleighDistribution(this.sigma);
        }
        ...
        {
            return Random(this.sigma, samples, result, source);
        }
        ...
        {
            return Random(this.sigma, source);
        }
