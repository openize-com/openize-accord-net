...
namespace FileFormat.Accord.Statistics.Distributions.Univariate.Continuous
{
    ...
    /// </example>
    /// 
    /// <seealso cref="SkewNormalDistribution"/>
        ...
        IUnivariateFittableDistribution
    {
        ...
        {
            this.initialize(this.mean, this.stdDev, this.stdDev * this.stdDev);
        }
        ...
        {
            this.initialize(mean, this.stdDev, this.stdDev * this.stdDev);
        }
        ...
        public NormalDistribution([Real] double mean, [Positive] double stdDev)
        {
            ...
            this.initialize(mean, stdDev, stdDev * stdDev);
        }
        ...
        public override double Median
        {
            get
            {
                Debug.Assert(this.mean.IsEqual(base.Median, 1e-10));
                return this.mean;
            }
        }
        ...
        public override double Entropy
        {
            get
            {
                if (!this.entropy.HasValue)
                    this.entropy = 0.5 * (Math.Log(2.0 * Math.PI * this.variance) + 1);
        ...
        ///  as <see cref="Special.Erfc">erfc</see>(-z/sqrt(2)) / 2.</para>  
        ///  
        /// <para>    
        ///   References:
        ///   <list type="bullet">
        ...
        {
            return Normal.Function((x - this.mean) / this.stdDev);
        }
        ...
        {
            return Normal.Complemented((x - this.mean) / this.stdDev);
        }
        ...
        protected internal override double InnerInverseDistributionFunction(double p)
        {
            ...
            double icdf = this.mean + this.stdDev * inv;
            ...
            if (p == 0)
                baseValue = this.Support.Min;

            if (p == 1)
                baseValue = this.Support.Max;

            baseValue = base.InnerInverseDistributionFunction(p);
            double r1 = this.DistributionFunction(baseValue);
            double r2 = this.DistributionFunction(icdf);
        ...
        public void Fit(double[] observations, double[] weights, NormalOptions options)
        {
            ...
            if (options != null)
            {
                if (options.Robust)
                {
                    this.initialize(mu, Math.Sqrt(var), var);
                    return;
                }
        ...
        {
            return new NormalDistribution(this.mean, this.stdDev);
        }
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            return String.Format(formatProvider, "N(x; μ = {0}, σ² = {1})",
                this.mean.ToString(format, formatProvider),
                this.variance.ToString(format, formatProvider));
        }
        ...
        public MultivariateNormalDistribution ToMultivariateDistribution()
        {
            return new MultivariateNormalDistribution(
                new double[] { this.mean }, new double[,] { { this.variance } });
        }
        ...
        {
            return Random(this.mean, this.stdDev, samples, result, source);
        }
        ...
        {
            return Random(this.mean, this.stdDev, source);
        }
        ...
        {
            return Random(mean, stdDev, global::FileFormat.Accord.Math.Random.Generator.Random);
        }
        ...
        {
            return Random(mean, stdDev, samples, new double[samples], global::FileFormat.Accord.Math.Random.Generator.Random);
        }
