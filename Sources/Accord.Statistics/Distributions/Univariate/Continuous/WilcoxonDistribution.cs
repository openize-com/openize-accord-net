...
namespace Openize.Accord.Statistics.Distributions.Univariate.Continuous
{
    ...
    ///   positive <see cref="Measures.Tools.Rank(double[],bool,bool)">signed ranks 
    ...
    public class WilcoxonDistribution : UnivariateContinuousDistribution
    {
        ...
        /// </summary>
        /// 
        public int NumberOfSamples { get { return this.n; } }
        ...
        {
            this.init(ranks.Length, ranks, exact);
        }
        ...
        {
            ranks = Matrix.Get(ranks, Matrix.Find(ranks, x => x != 0));
            ...
            Parallel.ForEach(seq, item =>
            {
                ...
                // Compute W+
                this.table[i] = WPositive(c, ranks);
            });
        ...
        {
            var clone = new WilcoxonDistribution(this.n);
        ...
        public override DoubleRange Support
        {
            get
            {
                ...
                return this.approximation.Support;
            }
        }
        ...
        {
            if (this.exact)
                return exactMethod(x, this.table);
        ...
        {
            if (this.exact)
                return exactComplement(x, this.table);
        ...
        {
            if (this.exact)
                return count(x, this.table) / (double)this.table.Length;
        ...
        {
            if (this.exact)
                return Math.Log(count(x, this.table)) - Math.Log(this.table.Length);

            return this.approximation.LogProbabilityDensityFunction(x);
        }
