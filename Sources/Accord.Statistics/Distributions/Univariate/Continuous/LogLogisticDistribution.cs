    ...
    public class LogLogisticDistribution : UnivariateContinuousDistribution
    {
        ...
        public override double Mean
        {
            get
            {
                ...
                double b = Math.PI / this.beta;
                return (this.alpha * b) / Math.Sin(b);
            }
        }
        ...
        public override double Median
        {
            get
            {
                Debug.Assert(this.alpha == base.Median);
                return this.alpha;
            }
        }
        ...
        public override double Variance
        {
            get
            {
                ...
                double pb = Math.PI / this.beta;
                double a = this.alpha * this.alpha;
        ...
        {
            double xb = Math.Pow(x, this.beta);
            double ab = Math.Pow(this.alpha, this.beta);
        ...
        {
            double ba = this.beta / this.alpha;
            double xa = x / this.alpha;

            double num = ba * Math.Pow(xa, this.beta - 1);
            double den = 1 + Math.Pow(xa, this.beta);
        ...
        {
            double qdf = (this.alpha / this.beta) * Math.Pow(p / (1 - p), 1 / this.beta - 1);

            Debug.Assert(qdf == base.QuantileDensityFunction(p));
        ...
        protected internal override double InnerComplementaryDistributionFunction(double x)
        {
            ...
            Debug.Assert(icdf.IsEqual(base.InnerComplementaryDistributionFunction(x), 1e-10));
        ...
        public override double HazardFunction(double x)
        {
            ...
            Debug.Assert(h.IsEqual(base.HazardFunction(x), 1e-10));
        ...
        {
            return new LogLogisticDistribution(this.alpha, this.beta);
        }
