...
namespace FileFormat.Accord.Statistics.Distributions.Univariate.Continuous
{
    ...
    ///   sample <see cref="Measures.Tools.Rank(double[],bool,bool)"/> statistics.</para>
    /// <para>
    ...
    /// <seealso cref="Measures.Tools.Rank(double[],bool,bool)"/>
    ...
    public class MannWhitneyDistribution : UnivariateContinuousDistribution
    {
        ...
        /// </summary>
        /// 
        public int NumberOfSamples1 { get { return this.n1; } }

        /// <summary>
        ///   Gets the number of observations in the second sample.
        /// </summary>
        /// 
        public int NumberOfSamples2 { get { return this.n2; } }
        ...
        /// 
        public bool Exact { get { return this.exact; } }
        ...
        /// </summary>
        /// 
        public double[] Table { get { return this.table; } }
        ...
        {
            double[] ranks = Matrix.Concatenate(ranks1, ranks2);
            this.init(ranks1.Length, ranks2.Length, ranks, exact);
        }
        ...
        {
            int min = Math.Min(this.n1, this.n2);
            long combinations = (long)Special.Binomial(this.n1 + this.n2, min);
        ...
        {
            if (this.NumberOfSamples1 <= this.NumberOfSamples2)
            ...
            return this.complementaryDistributionFunction(x);
        }
        ...
        private double distributionFunction(double x)
        {
            if (this.exact)
            {
                ...
                return WilcoxonDistribution.exactMethod(x, this.table);
            }

            if (this.Correction == ContinuityCorrection.Midpoint)
            {
                if (x > this.Mean)
                {
                    x = x - 0.5;
            ...
            else if (this.Correction == ContinuityCorrection.KeepInside)
            {
                x = x + 0.5;
            }

            return this.approximation.DistributionFunction(x);
        }
        ...
        private double complementaryDistributionFunction(double x)
        {
            if (this.exact)
            {
                // For small samples (< 30) and if there are not very large
                // differences in samples sizes, this distribution is exact.
                return WilcoxonDistribution.exactComplement(x, this.table);
            }
            ...
            return this.approximation.ComplementaryDistributionFunction(x);
        }
        ...
        {
            var clone = new MannWhitneyDistribution(this.NumberOfSamples1, this.NumberOfSamples2);
            clone.exact = this.exact;
            clone.table = this.table;
            clone.n1 = this.n1;
            clone.n2 = this.n2;
            clone.Correction = this.Correction;
            clone.approximation = (NormalDistribution)this.approximation.Clone();
            return clone;
        }
        ...
        {
            get { return this.approximation.Mean; }
        }
        ...
        {
            get { return this.approximation.Variance; }
        }
        ...
        {
            get { return this.approximation.Mode; }
        }
        ...
        {
            get { return this.approximation.Entropy; }
        }
        ...
        {
            if (this.exact)
                return WilcoxonDistribution.count(x, this.table) / (double)this.table.Length;

            return this.approximation.ProbabilityDensityFunction(x);
        }
        ...
        {
            if (this.exact)
                return Math.Log(WilcoxonDistribution.count(x, this.table)) - Math.Log(this.table.Length);
        ...
        protected internal override double InnerInverseDistributionFunction(double p)
        {
            if (this.exact)
            {
                if (this.n1 > this.n2)
                {
                    ...
                    double f = this.DistributionFunction(0);
                    ...
                    double value = BrentSearch.Find(this.DistributionFunction, p, lower, upper);
            ...
            return this.approximation.InverseDistributionFunction(p);
        }
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            return String.Format(formatProvider, "MannWhitney(u; n1 = {0}, n2 = {1})",
                this.NumberOfSamples1.ToString(format, formatProvider),
                this.NumberOfSamples2.ToString(format, formatProvider));
        }
    }
}
