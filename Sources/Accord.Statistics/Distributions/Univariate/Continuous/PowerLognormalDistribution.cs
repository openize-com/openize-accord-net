...
namespace FileFormat.Accord.Statistics.Distributions.Univariate.Continuous
{
    ...
    public class PowerLognormalDistribution : UnivariateContinuousDistribution
    {
        ...
        {
            double z = Math.Log(x) / this.sigma;
            return 1.0 - Math.Pow(Normal.Function(-z), this.power);
        }
        ...
        {
            return Math.Exp(Normal.Inverse(1.0 - Math.Pow(1.0 - p, 1.0 / this.power)) * this.sigma);
        }
        ...
        {
            double a = this.power / (x * this.sigma);
            double z = Math.Log(x) / this.sigma;

            return a * Normal.Derivative(z) * Math.Pow(Normal.Function(-z), this.power - 1);
        }
        ...
        public override double HazardFunction(double x)
        {
            ...
            double num = this.power * a * Normal.Derivative(z);
        ...
        {
            double z = Math.Log(x) / this.sigma;
            return -Math.Log(Math.Pow(Normal.Function(-z), this.power));
        }
        ...
        {
            double z = Math.Log(x) / this.sigma;
            return Math.Pow(Normal.Function(-z), this.power);
        }
        ...
        {
            return new PowerLognormalDistribution(this.power, this.sigma);
        }
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            return String.Format(formatProvider, "PLD(x; p = {0}, σ = {1})",
                this.power.ToString(format, formatProvider),
