...
namespace FileFormat.Accord.Statistics.Distributions.Univariate.Continuous
{
    ...
    ///   using or creating <see cref=" ProportionalHazards">
        ...
        IFittableDistribution<double, EmpiricalHazardOptions>
    {
        ...
        /// </summary>
        /// 
        public double[] Times { get { return this.times; } }

        /// <summary>
        ///   Gets the hazard rate values at each time step.
        /// </summary>
        /// 
        public double[] Hazards { get { return this.hazards; } }

        /// <summary>
        ///   Gets the survival values at each time step.
        /// </summary>
        /// 
        public double[] Survivals { get { return this.survivals; } }
        ...
        {
            this.init(null, null, SurvivalOptions.DefaultSurvival);
        }
        ...
        {
            this.init(time, lambdas, SurvivalOptions.DefaultSurvival);
        }
        ...
        public EmpiricalHazardDistribution(double[] times, double[] lambdas, SurvivalEstimator estimator)
        {
            ...
            this.init(times, lambdas, estimator);
        }
        ...
        void init(double[] times, double[] hazards, SurvivalEstimator estimator)
        {
            ...
            if (times.Length > 0)
            {
                ...
                this.survivals[0] = 1;
                for (int i = 1; i < this.survivals.Length; i++)
                    this.survivals[i] = this.survivals[i - 1] * (1.0 - hazards[i - 1]);
            }
        ...
        public override double Mean
        {
            get
            {
                if (!this.mean.HasValue)
                {
                    ...
                    double m = 0;
                    foreach (var t in this.times.Distinct())
                        m += t * this.ProbabilityDensityFunction(t);
                    this.mean = m;
                }
        ...
        public override double Variance
        {
            get
            {
                if (!this.variance.HasValue)
                {
                    ...
                    double v = 0;
                    foreach (var t in this.times.Distinct())
                        v += t * t * this.ProbabilityDensityFunction(t);
        ...
        {
            if (this.estimator == SurvivalEstimator.KaplanMeier)
            {
                // Kaplan-Meier estimator
                for (int i = 0; i < this.survivals.Length; i++)
                {
                    if (this.times[i] > x)
                        return this.survivals[i];
                }

                return this.survivals[this.survivals.Length - 1];
            }
            ...
            else // (estimator == SurvivalEstimator.FlemingHarrington)
            {
                ...
                // H(x)   = -ln(1-F(x))
                // 1-F(x) = exp(-H(x))
                double chf = this.CumulativeHazardFunction(x);
        ...
        {
            if (x < this.range.Min || x > this.range.Max)
                return 0;
        ...
        {
            return this.HazardFunction(x) * this.ComplementaryDistributionFunction(x);
        }
        ...
        {
            return 1.0 - this.ComplementaryDistributionFunction(x);
        }
        ...
        {
            return new EmpiricalHazardDistribution((double[])this.Times.Clone(), (double[])this.Hazards.Clone(), this.estimator);
        }
        ...
        {
            this.Fit(observations, weights, options as SurvivalOptions);
        }
