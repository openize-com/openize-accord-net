...
namespace Openize.Accord.Statistics.Distributions.Univariate.Continuous
{
    ...
    public class BirnbaumSaundersDistribution : UnivariateContinuousDistribution
    {
        ...
        {
            get { return 1 + 0.5 * this.shape * this.shape; }
        }
        ...
        {
            get { return this.shape * this.shape * (1 + (5 * this.shape * this.shape) / 4); }
        }
        ...
        {
            double c = x - this.location;

            double a = Math.Sqrt(c);
            double b = Math.Sqrt(1.0 / c);
            double z = (a - b) / this.shape;
        ...
        protected internal override double InnerProbabilityDensityFunction(double x)
        {
            ...
            double a = Math.Sqrt(c / this.scale);
            double b = Math.Sqrt(this.scale / c);

            double alpha = (a + b) / (2 * this.shape * c);
            double z = (a - b) / this.shape;
        ...
        protected internal override double InnerInverseDistributionFunction(double p)
        {
            ...
            double a = z + Math.Sqrt(4 + this.shape * (z * z));

            return (1.0 / 4.0) * a * a + this.location;
        }
        ...
        {
            return new BirnbaumSaundersDistribution(this.location, this.scale, this.shape);
        }
