...
namespace FileFormat.Accord.Statistics.Distributions.Univariate.Continuous
{
        ...
        IFittableDistribution<double>
    {
        ...
        {
            get { return this.mean + this.beta * Constants.EulerGamma; }
        }
        ...
        {
            get { return ((Math.PI * Math.PI) / 6.0) * this.beta * this.beta; }
        }
        ...
        public override double Median
        {
            get
            {
                double median = this.mean - this.beta * Math.Log(Math.Log(2));
        ...
        {
            get { return Math.Log(this.beta) + Constants.EulerGamma + 1; }
        }
        ...
        {
            double z = (x - this.mean) / this.beta;
        ...
        {
            double z = (x - this.mean) / this.beta;
            return (1 / this.beta) * Math.Exp(-(z + Math.Exp(-z)));
        }
        ...
        {
            double z = (x - this.mean) / this.beta;
            return Math.Log(1 / this.beta) - (z + Math.Exp(-z));
        }
        ...
        {
            return this.mean - this.beta * Math.Log(-Math.Log(p));
        }
        ...
        {
            return new GumbelDistribution(this.mean, this.beta);
        }
