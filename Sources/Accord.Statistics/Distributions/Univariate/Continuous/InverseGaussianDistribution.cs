...
namespace Openize.Accord.Statistics.Distributions.Univariate.Continuous
{
        ...
        ISampleableDistribution<double>, IFormattable
    {
        ...
        {
            get { return (this.mean * this.mean * this.mean) / this.lambda; }
        }
        ...
        public override double Mode
        {
            get
            {
                double a = 1 + (9 * this.mean * this.mean) / (4 * this.lambda * this.lambda * this.lambda * this.lambda);
                double b = (3 * this.mean) / (2 * this.lambda);
                return this.mean * (Math.Sqrt(a) - b);
            }
        }
        ...
        protected internal override double InnerDistributionFunction(double x)
        {
            ...
            double a = 0.5 * Special.Erfc(sqrt * (this.mean - x) / (Constants.Sqrt2 * this.mean));
            double b = 0.5 * Special.Erfc(sqrt * (this.mean + x) / (Constants.Sqrt2 * this.mean));
        ...
        {
            double a = Math.Sqrt(this.lambda / (2.0 * Math.PI * x * x * x));
        ...
        {
            return new InverseGaussianDistribution(this.mean, this.lambda);
        }
        ...
        {
            return Random(this.mean, this.lambda, samples, result, source);
        }
        ...
        {
            return Random(this.mean, this.lambda, source);
        }
