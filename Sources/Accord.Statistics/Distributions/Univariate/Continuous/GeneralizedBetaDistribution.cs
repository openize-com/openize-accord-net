...
namespace FileFormat.Accord.Statistics.Distributions.Univariate.Continuous
{
        ...
        ISampleableDistribution<double>, IFittableDistribution<double, GeneralizedBetaOptions>
    {
            ...
            [Real, DefaultValue(0)] double min, [Real, DefaultValue(1)] double max)
        {
            ...
            this.initialize(min, max, alpha, beta);
        }
        ...
        public override double Mean
        {
            get
            {
                double mu = this.alpha / (this.alpha + this.beta);
                return mu * (this.max - this.min) + this.min;
            }
        }
        ...
        public override double Variance
        {
            get
            {
                double var = (this.alpha * this.beta) / ((this.alpha + this.beta) * (this.alpha + this.beta) * (this.alpha + this.beta + 1));
                return var * (this.max - this.min);
            }
        }
        ...
        public override double Mode
        {
            get
            {
                double mode = (this.alpha - 1.0) / (this.alpha + this.beta - 2.0);
                return mode * (this.max - this.min) + this.min;
            }
        }
        ...
        {
            get { return new DoubleRange(this.min, this.max); }
        }
        ...
        protected internal override double InnerProbabilityDensityFunction(double x)
        {
            ...
            double z = (x - this.min) / length;

            double a = Math.Pow(z, this.alpha - 1);
            double b = Math.Pow(1 - z, this.beta - 1);
            return this.constant * a * b / length;
        }
        ...
        protected internal override double InnerLogProbabilityDensityFunction(double x)
        {
            ...
            double a = (this.alpha - 1) * Math.Log(z);
            double b = (this.beta - 1) * Math.Log(1 - z);
            return Math.Log(this.constant) + a + b - Math.Log(length);
        }
        ...
        {
            return new GeneralizedBetaDistribution(this.alpha, this.beta, this.min, this.max);
        }
        ...
        {
            this.Fit(observations, weights, options as GeneralizedBetaOptions);
        }
        ...
        public void Fit(double[] observations, double[] weights, GeneralizedBetaOptions options)
        {
            ...
            if (!fixMin)
                this.min = TriangularDistribution.GetMin(observations, weights, out imin);

            if (!fixMax)
                this.max = TriangularDistribution.GetMax(observations, weights, out imax);

            if (imin == -1)
                imin = TriangularDistribution.FindMin(observations, this.min);

            if (imax == -1)
                imax = TriangularDistribution.FindMax(observations, this.max);


            double mean = GetMean(observations, weights, imin, imax);
            double var = GetVariance(observations, weights, mean, imin, imax);

            this.fitMoments(this.min, this.max, mean, var);
        ...
        {
            return Random(this.alpha, this.beta, this.min, this.max, samples, result, source);
        }
        ...
        {
            return Random(this.alpha, this.beta, this.min, this.max, source);
        }
