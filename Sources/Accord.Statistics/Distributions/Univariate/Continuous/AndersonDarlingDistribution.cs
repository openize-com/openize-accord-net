...
namespace Openize.Accord.Statistics.Distributions.Univariate.Continuous
{
    ...
    public class AndersonDarlingDistribution : UnivariateContinuousDistribution, IFormattable
    {
        ...
        protected internal override double InnerDistributionFunction(double x)
        {
            ...
            switch (this.DistributionType)
            {
                case AndersonDarlingDistributionType.Uniform:
                    result = ad_uniform(x, this.NumberOfSamples);
                    break;

                case AndersonDarlingDistributionType.Normal:
                    result = 1.0 - ad_normal(x, this.NumberOfSamples);
                    break;
        ...
        protected internal override double InnerComplementaryDistributionFunction(double x)
        {
            ...
            {
                case AndersonDarlingDistributionType.Uniform:
                    return 1.0 - ad_uniform(x, this.NumberOfSamples);

                case AndersonDarlingDistributionType.Normal:
                    return ad_normal(x, this.NumberOfSamples);
            }
        ...
        {
            return new AndersonDarlingDistribution(this.DistributionType, this.NumberOfSamples);
        }
