...
namespace FileFormat.Accord.Statistics.Distributions.Univariate.Continuous
{
        ...
        IFittableDistribution<double, BetaOptions>, ISampleableDistribution<double>
    {
        ...
        public BetaDistribution([NonnegativeInteger(maximum: Int32.MaxValue - 1)] int successes, [PositiveInteger] int trials)
        {
            ...
            this.initialize(successes + 1, trials - successes + 1);
        }
        ...
        {
            get { return this.beta + this.Successes - 1; }
        }
        ...
        {
            get { return (this.alpha * this.beta) / ((this.alpha + this.beta) * (this.alpha + this.beta) * (this.alpha + this.beta + 1)); }
        }
        ...
        public override double Entropy
        {
            get
            {
                if (this.entropy == null)
                {
                    ...
                    this.entropy = lnBab - (this.alpha - 1) * da - (this.beta - 1) * db + (this.alpha + this.beta - 2) * dab;
                }
        ...
        {
            get { return (this.alpha - 1.0) / (this.alpha + this.beta - 2.0); }
        }
        ...
        ///   
        /// <para>
        ///   The Beta's CDF is computed using the <see cref="Accord.Math.Beta.Incomplete(double,double,double)">Incomplete 
        ///   (regularized) Beta function I_x(a,b)</see> as CDF(x) = I_x(a,b)</para>
        /// </remarks>
        /// 
        protected internal override double InnerDistributionFunction(double x)
        {
            return global::FileFormat.Accord.Math.Functions.Beta.Incomplete(this.alpha, this.beta, x);
        }
        ...
        {
            return global::FileFormat.Accord.Math.Functions.Beta.IncompleteInverse(this.alpha, this.beta, p);
        }
        ...
        ///   where constant c is c = 1.0 / <see cref="Accord.Math.Beta.Function(double,double)">Beta.Function(a, b)</see></para>
        /// </remarks>
        /// 
        protected internal override double InnerProbabilityDensityFunction(double x)
        {
            return this.constant * Math.Pow(x, this.alpha - 1) * Math.Pow(1 - x, this.beta - 1);
        }
        ...
        {
            return Math.Log(this.constant) + (this.alpha - 1) * Math.Log(x) + (this.beta - 1) * Math.Log(1 - x);
        }
        ...
        {
            this.Fit(observations, weights, options as BetaOptions);
        }
        ...
        public void Fit(double[] observations, double[] weights, BetaOptions options)
        {
            ...
            if (useMLE)
            {
                if (weights == null)
                {
                    ...
                    this.fitMLE(sum1, sum2, observations.Length);
                }
                else
                {
        ...
        public static double LogLikelihood(double sum1, double sum2, double n, double alpha, double beta)
        {
            ...
            double t3 = global::FileFormat.Accord.Math.Functions.Beta.Log(alpha, beta);
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            return String.Format(formatProvider, "B(x; α = {0}, β = {1})",
                ...
                this.beta.ToString(format, formatProvider));
        }
        ...
        {
            return Random(this.alpha, this.beta, samples, result, source);
        }
        ...
        {
            return Random(this.alpha, this.beta, source);
        }
        ...
        {
            return Random(alpha, beta, samples, new double[samples], global::FileFormat.Accord.Math.Random.Generator.Random);
        }
        ...
        {
            return Random(alpha, beta, samples, result, global::FileFormat.Accord.Math.Random.Generator.Random);
        }
        ...
        {
            return Random(alpha, beta, global::FileFormat.Accord.Math.Random.Generator.Random);
        }
