...
namespace Openize.Accord.Statistics.Distributions.Univariate.Continuous
{
    ...
    public class LevyDistribution : UnivariateContinuousDistribution
    {
        ...
        public override double Median
        {
            get
            {
                if (!this.median.HasValue)
                {
                    ...
                    double m = this.location + this.scale / (a * a);
        ...
        public override double Entropy
        {
            get
            {
                return (1.0 + 3.0 * Constants.EulerGamma + Math.Log(16 * Math.PI * this.scale * this.scale)) / 2.0;
            }
        }
        ...
        {
            double cdf = Special.Erfc(Math.Sqrt(this.scale / (2 * (x - this.location))));
        ...
        {
            double z = x - this.location;
            double a = Math.Sqrt(this.scale / (2.0 * Math.PI));
            double b = Math.Exp(-(this.scale / (2 * z)));
        ...
        protected internal override double InnerInverseDistributionFunction(double p)
        {
            ...
            double icdf = this.location + this.scale / (a * a);
        ...
        {
            return new LevyDistribution(this.location, this.scale);
        }
