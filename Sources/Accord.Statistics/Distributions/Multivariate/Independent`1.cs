...
namespace FileFormat.Accord.Statistics.Distributions.Multivariate
{
        ...
        where TDistribution : IUnivariateDistribution
    {
            ...
            : base(dimensions)
        {
            try
            {
                ...
                for (int i = 0; i < this.components.Length; i++)
                    this.components[i] = Activator.CreateInstance<TDistribution>();
            }
            catch
            {
        ...
        {
            get { return this.components[i]; }
            set { this.components[i] = value; }
        }
        ...
        public override double[] Mean
        {
            get
            {
                if (this.mean == null)
                {
                    this.mean = new double[this.components.Length];
                    for (int i = 0; i < this.components.Length; i++)
                        this.mean[i] = this.components[i].Mean;
                }
                return this.mean;
            }
        }
        ...
        public override double[] Variance
        {
            get
            {
                ...
                {
                    this.variance = new double[this.components.Length];
                    for (int i = 0; i < this.components.Length; i++)
                        this.variance[i] = this.components[i].Variance;
                }
        ...
        public override double[,] Covariance
        {
            get
            {
                if (this.covariance == null)
                    this.covariance = Matrix.Diagonal(this.Variance);
        ...
        public override void Fit(double[][] observations, double[] weights, IFittingOptions options)
        {
            ...
            this.Fit(observations, weights, independentOptions);
        }
        ...
        public void Fit(double[][] observations, double[] weights, IndependentOptions options)
        {
            if (options != null)
            {
                ...
                {
                    for (int i = 0; i < this.components.Length; i++)
                        this.components[i].Fit(observations[i], weights, options.InnerOptions[i]);
                }
                else
                {
                    for (int i = 0; i < this.components.Length; i++)
                        this.components[i].Fit(observations[i], weights, options.InnerOption);
                }
            }
            else
            {
                observations = observations.Transpose();
                for (int i = 0; i < this.components.Length; i++)
                    this.components[i].Fit(observations[i], weights, null);
            }
        ...
        {
            var gen = this.components.Apply(x => (ISampleableDistribution<double>)x);
        ...
        {
            TDistribution[] clone = new TDistribution[this.components.Length];
