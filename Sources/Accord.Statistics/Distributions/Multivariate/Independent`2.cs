                              ...
                              IUnivariateDistribution
    {
        ...
        public double ProbabilityFunction(TObservation[] x)
        {
            ...
            for (int i = 0; i < this.Components.Length; i++)
                p *= this.Components[i].ProbabilityFunction(x[i]);
            return p;
        }
        ...
        public double DistributionFunction(TObservation[] x)
        {
            double p = 1;
            for (int i = 0; i < this.Components.Length; i++)
                p *= this.Components[i].DistributionFunction(x[i]);
            return p;
        }
        ...
        public double ComplementaryDistributionFunction(TObservation[] x)
        {
            double p = 1;
            for (int i = 0; i < this.Components.Length; i++)
                p *= this.Components[i].ComplementaryDistributionFunction(x[i]);
            return p;
        }
        ...
        public double LogProbabilityFunction(TObservation[] x)
        {
            double p = 0;
            for (int i = 0; i < this.Components.Length; i++)
                p += this.Components[i].LogProbabilityFunction(x[i]);
//#if DEBUG
...
//#endif
        ...
        {
            this.Fit(observations, null, (IndependentOptions)null);
        }
        ...
        {
            this.Fit(observations, weights, (IndependentOptions)null);
        }
        ...
        public void Fit(TObservation[][] observations, double[] weights, IndependentOptions options)
        {
            if (options != null)
            {
                ...
                if (options.InnerOptions != null)
                {
                    for (int i = 0; i < this.Components.Length; i++)
                    {
                        var c = ((IFittableDistribution<TObservation>)this.Components[i]);
            ...
            else
            {
                observations = observations.Transpose();
                for (int i = 0; i < this.Components.Length; i++)
                    ((IFittableDistribution<TObservation>)this.Components[i]).Fit(observations[i], weights);
            }
        ...
        {
            var clone = new TDistribution[this.Components.Length];
            for (int i = 0; i < clone.Length; i++)
                clone[i] = (TDistribution)this.Components[i].Clone();
        ...
        {
            for (int i = 0; i < this.Components.Length; i++)
                result[i] = ((ISampleableDistribution<TObservation>)this.Components[i]).Generate(source);
            return result;
        }
        ...
        {
            for (int i = 0; i < this.Components.Length; i++)
                result.SetColumn(i, ((ISampleableDistribution<TObservation>)this.Components[i]).Generate(samples, source));
            return result;
        }

        TObservation[][] IRandomNumberGenerator<TObservation[]>.Generate(int samples)
        {
            return this.Generate(samples, Jagged.Zeros<TObservation>(samples, this.Components.Length));
        }

        TObservation[] IRandomNumberGenerator<TObservation[]>.Generate()
        {
            return this.Generate(new TObservation[this.Components.Length]);
        }


        TObservation[][] ISampleableDistribution<TObservation[]>.Generate(int samples, Random source)
        {
            return this.Generate(samples, Jagged.Zeros<TObservation>(samples, this.Components.Length), source);
        }

        TObservation[] ISampleableDistribution<TObservation[]>.Generate(Random source)
        {
            return this.Generate(new TObservation[this.Components.Length], source);
        }
