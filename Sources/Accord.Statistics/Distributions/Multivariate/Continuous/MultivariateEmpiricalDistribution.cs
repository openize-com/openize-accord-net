...
namespace Openize.Accord.Statistics.Distributions.Multivariate.Continuous
{
    ...
    /// <seealso cref="EmpiricalDistribution"/>
    /// 
        ...
        ISampleableDistribution<double[]>
    {
        ...
        public MultivariateEmpiricalDistribution(double[][] samples, int[] weights, double[,] smoothing)
            : base(samples[0].Length)
        {
            this.initialize(this.kernel, samples, null, weights, smoothing);
        }
        ...
        public MultivariateEmpiricalDistribution(double[][] samples, double[] weights, double[,] smoothing)
            : base(samples[0].Length)
        {
            this.initialize(this.kernel, samples, weights, null, smoothing);
        }
            ...
            get
            {
                if (this.mean == null)
                {
                    if (this.type == WeightType.None)
                        this.mean = Measures.Mean(this.samples, dimension: 0);
        ...
        public override double[,] Covariance
        {
            get
            {
                ...
                {
                    if (this.type == WeightType.None)
                        this.covariance = Measures.Covariance(this.samples, this.Mean).ToMatrix(); // TODO: Switch to double[][]

                    else if (this.type == WeightType.Repetition)
                        this.covariance = Measures.WeightedCovariance(this.samples, this.repeats);

                    else if (this.type == WeightType.Fraction)
                        this.covariance = Measures.WeightedCovariance(this.samples, this.weights);
                }
        ...
        protected internal override double InnerProbabilityDensityFunction(params double[] x)
        {
            ...
            double sum = 0;
            double[] delta = new double[this.Dimension];
            ...
            if (this.type == WeightType.None)
            {
                for (int i = 0; i < this.samples.Length; i++)
                {
                    for (int j = 0; j < x.Length; j++)
                        delta[j] = (x[j] - this.samples[i][j]);

                    sum += this.kernel.Function(this.chol.Solve(delta));
                }

                return sum / (this.samples.Length * this.determinant);
            }
            ...
            if (this.type == WeightType.Repetition)
            {
                for (int i = 0; i < this.samples.Length; i++)
                {
                    ...
                    sum += this.kernel.Function(this.chol.Solve(delta)) * this.repeats[i];
                }

                return sum / (this.numberOfSamples * this.determinant);
            }
            ...
            if (this.type == WeightType.Fraction)
            {
                for (int i = 0; i < this.samples.Length; i++)
                {
                    ...
                    sum += this.kernel.Function(this.chol.Solve(delta)) * this.weights[i];
                }

                return sum / (this.sumOfWeights * this.determinant);
            }
        ...
        {
            this.Fit(observations, weights, options as MultivariateEmpiricalOptions);
        }
        ...
        public void Fit(double[][] observations, double[] weights, MultivariateEmpiricalOptions options)
        {
            ...
            this.initialize(this.kernel, observations, weights, null, smoothing);
        }
        ...
        public void Fit(double[][] observations, int[] weights, MultivariateEmpiricalOptions options)
        {
            ...
            this.initialize(this.kernel, observations, null, weights, smoothing);
        }
            ...
            double[] weights, int[] repeats, double[,] smoothing)
        {
            ...
            if (kernel == null)
                kernel = new GaussianKernel(this.Dimension);
        ...
        {
            var e = new MultivariateEmpiricalDistribution(this.Dimension);

            e.chol = (CholeskyDecomposition)this.chol.Clone();
            e.determinant = this.determinant;
            e.kernel = this.kernel;
            e.numberOfSamples = this.numberOfSamples;
            e.repeats = (int[])this.repeats.Clone();
            e.samples = (double[][])this.samples.MemberwiseClone();
            e.smoothing = this.smoothing.MemberwiseClone();
            e.sumOfWeights = this.sumOfWeights;
            e.type = this.type;

            if (e.weights != null)
                e.weights = (double[])this.weights.Clone();
        ...
        public override double[][] Generate(int samples, double[][] result, Random source)
        {
            for (int i = 0; i < samples; i++)
            {
                ...
                Array.Copy(this.samples[j], result[i], this.Dimension);
            }
            return result;
        }
