...
namespace Openize.Accord.Statistics.Distributions.Multivariate.Continuous
{
    ...
    public class WishartDistribution : MatrixContinuousDistribution
    {
        ...
        : base(scale.Rows(), scale.Columns())
        {
            ...
            if (!this.chol.IsPositiveDefinite)
                throw new NonPositiveDefiniteMatrixException("scale");
            //if (!chol.Symmetric)
            //    throw new NonSymmetricMatrixException("scale");

            double a = Math.Pow(this.chol.Determinant, this.n / 2.0);
            double b = Math.Pow(2, (this.n * this.size) / 2.0);
            double c = Gamma.Multivariate(this.n / 2.0, this.size);

            this.constant = 1.0 / (a * b * c);
            this.lnconstant = Math.Log(this.constant);

            this.power = (this.n - this.size - 1) / 2.0;
        }
        ...
        public override double[,] Mean
        {
            ...
            {
                if (this.mean == null)
                    this.mean = this.n.Multiply(this.scaleMatrix);
                return this.mean;
            }
        }
        ...
        public override double[] Variance
        {
            get
            {
                if (this.variance == null)
                {
                    ...
                    {
                        double vii = this.scaleMatrix[i, i];
                        this.variance[i] = 2 * this.n * (vii * vii);
                    }
                }
            ...
            get
            {
                if (this.covariance == null)
                {
                    ...
                    for (int i = 0; i < this.size; i++)
                    {
                        ...
                        {
                            double vij = this.scaleMatrix[i, j];
                            double vjj = this.scaleMatrix[j, j];

                            this.covariance[i, j] = this.n * (vij * vij + vii * vjj);
                        }
                    }
                }
        ...
        protected internal override double InnerLogProbabilityDensityFunction(double[,] x)
        {
            ...
            double z = -0.5 * Vx.Trace();
            double a = this.power * Math.Log(det);

            return this.lnconstant + a + z;
        }
        ...
        {
            return Random(samples, this.n, this.scaleMatrix, result, source);
        }
        ...
        {
            return new WishartDistribution(this.n, this.scaleMatrix);
        }
