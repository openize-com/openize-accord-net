...
namespace Openize.Accord.Statistics.Distributions.Multivariate.Continuous
{
    ...
    public class DirichletDistribution : MultivariateContinuousDistribution
    {
            ...
            : base(dimension)
        {
            ...
            this.constant = 1.0 / Beta.Multinomial(this.alphas);
        }
        ...
        public override double[] Mean
        {
            ...
            {
                if (this.mean == null)
                    this.mean = this.alphas.Divide(this.alphas.Sum());
                return this.mean;
            }
        }
        ...
        public override double[] Variance
        {
            get
            {
                ...
                {
                    double sum = this.alphas.Sum();
                    double den = sum * sum * (sum + 1);

                    this.variance = new double[this.alphas.Length];
                    for (int i = 0; i < this.variance.Length; i++)
                        this.variance[i] = (this.alphas[i] * (sum - this.alphas[i])) / den;
                }
        ...
        public override double[,] Covariance
        {
            get
            {
                if (this.covariance == null)
                {
                    ...
                    int k = this.alphas.Length;
                    this.covariance = new double[k, k];

                    for (int i = 0; i < this.alphas.Length; i++)
                        for (int j = 0; j < this.alphas.Length; j++)
                            this.covariance[i, j] = (this.alphas[i] * this.alphas[j]) / den;
                }
        ...
        protected internal override double InnerProbabilityDensityFunction(params double[] x)
        {
            ...
            for (int i = 0; i < x.Length; i++)
                product *= Math.Pow(x[i], this.alphas[i] - 1);

            return this.constant * product;
        }
        ...
        protected internal override double InnerLogProbabilityDensityFunction(params double[] x)
        {
            ...
            for (int i = 0; i < x.Length; i++)
                sum += (this.alphas[i] - 1) * Math.Log(x[i]);

            return Math.Log(this.constant) + sum;
        }
