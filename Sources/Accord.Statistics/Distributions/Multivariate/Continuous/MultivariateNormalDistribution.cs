        ...
        ISampleableDistribution<double[]>
    {
            ...
            : base(dimension)
        {
            if (init)
            {
                ...
                this.initialize(mean, cov, chol, svd: null);
            }
        }
            ...
            : base(mean.Length)
        {
            ...
            if (!robust)
            {
                ...
                this.initialize(mean, covariance, cd: chol, svd: null);
            }
            else
            {
                var svd = new SingularValueDecomposition(covariance, true, true, true);

                this.initialize(mean, covariance, cd: null, svd: svd);
            }
        }
        ...
        private void initialize(double[] m, double[,] cov, CholeskyDecomposition cd, SingularValueDecomposition svd)
        {
            ...
            if (this.chol != null || svd != null)
            {
                ...
                this.lnconstant = -(Constants.Log2PI * k + lndet) * 0.5;
            }
        }
        ...
        protected internal override double InnerDistributionFunction(params double[] x)
        {
            if (this.Dimension == 1)
            {
                double stdDev = Math.Sqrt(this.Covariance[0, 0]);

                if (stdDev == 0)
                    return (x[0] == this.mean[0]) ? 1 : 0;

                double z = (x[0] - this.mean[0]) / stdDev;
            ...
            if (this.Dimension == 2)
            {
                double sigma1 = Math.Sqrt(this.Covariance[0, 0]);
                double sigma2 = Math.Sqrt(this.Covariance[1, 1]);
                double rho = this.Covariance[0, 1] / (sigma1 * sigma2);

                if (Double.IsNaN(rho))
                    return (x.IsEqual(this.mean)) ? 1 : 0;

                double z = (x[0] - this.mean[0]) / sigma1;
                double w = (x[1] - this.mean[1]) / sigma2;
        ...
        protected internal override double InnerComplementaryDistributionFunction(params double[] x)
        {
            ...
            if (this.Dimension == 2)
            {
                ...
                if (Double.IsNaN(rho))
                    return (x.IsEqual(this.mean)) ? 0 : 1;
        ...
        {
            return Math.Exp(-0.5 * this.Mahalanobis(x) + this.lnconstant);
        }
        ...
        {
            return -0.5 * this.Mahalanobis(x) + this.lnconstant;
        }
        ...
        public double Mahalanobis(double[] x)
        {
            ...
            var z = new double[this.mean.Length];
            for (int i = 0; i < x.Length; i++)
                z[i] = x[i] - this.mean[i];

            double[] a = (this.chol == null) ? this.svd.Solve(z) : this.chol.Solve(z);
        ...
        public override void Fit(double[][] observations, double[] weights, IFittingOptions options)
        {
            ...
            this.Fit(observations, weights, normalOptions);
        }
        ...
        public void Fit(double[][] observations, double[] weights, NormalOptions options)
        {
            ...
            if (options != null && options.Shared)
            {
                options.Postprocessing = (components, pi) =>
                {
                    ...
                    // Decompose only once
                    decompose(options, pooledCov, out this.chol, out this.svd);

                    foreach (var gaussian in gaussians)
                        gaussian.initialize(gaussian.mean, pooledCov, this.chol, this.svd);
                };

                this.initialize(mean, cov, null, null);
            }
            else
            {
                decompose(options, cov, out this.chol, out this.svd);

                this.initialize(mean, cov, this.chol, this.svd);
            }
        }
        ...
        public override object Clone()
        {
            ...
            clone.covariance = (double[,])this.covariance.Clone();
            clone.mean = (double[])this.mean.Clone();

            if (this.chol != null)
                clone.chol = (CholeskyDecomposition)this.chol.Clone();

            if (this.svd != null)
                clone.svd = (SingularValueDecomposition)this.svd.Clone();
        ...
        public override double[][] Generate(int samples, double[][] result, Random source)
        {
            ...
            if (this.leftTriangularFactor == null)
            {
                if (this.svd != null)
                {
                    this.leftTriangularFactor = new double[this.Dimension, this.Dimension];
                    double[,] V = this.svd.RightSingularVectors;
                    for (int i = 0; i < this.svd.Diagonal.Length; i++)
                    {
                        double d = Math.Sqrt(this.svd.Diagonal[i]);
                        for (int j = 0; j < this.Dimension; j++)
                            this.leftTriangularFactor[j, i] = d * V[j, i];
                    }
                }
                else
                {
                    this.leftTriangularFactor = this.chol.LeftTriangularFactor;
                }
            }

            for (int i = 0; i < samples; i++)
            {
                NormalDistribution.Random(this.Dimension, result: z, source: source);
                this.leftTriangularFactor.Dot(z, result: result[i]);
        ...
        /// actually represents a <see cref="NormalDistribution"/>.</returns>
        /// 
