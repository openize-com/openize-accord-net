...
namespace FileFormat.Accord.Statistics.Distributions.Multivariate.Discrete
{
    ...
    ///   <see cref="BernoulliDistribution">Bernoulli</see>
    ///   trials, with the same probability of "success" on each trial.</para>
    ///   
    ///   <para>In a multinomial distribution, the analog of the
    ///   <see cref="BernoulliDistribution">Bernoulli distribution</see> is the
    ///   <see cref="GeneralDiscreteDistribution">categorical distribution</see>,
    ...
    /// <seealso cref="GeneralDiscreteDistribution"/>
    /// 
    [Serializable]
        ...
        IFittableDistribution<double[], IFittingOptions>
    {
        ...
        {
            this.initialize(numberOfTrials, probabilities);
        }
        ...
        public override double[] Mean
        {
            get
            {
                if (this.mean == null)
                {
                    this.mean = new double[this.probabilities.Length];
                    for (int i = 0; i < this.mean.Length; i++)
                        this.mean[i] = this.N * this.probabilities[i];
                }
        ...
        public override double[] Variance
        {
            get
            {
                ...
                {
                    this.variance = new double[this.probabilities.Length];
                    for (int i = 0; i < this.variance.Length; i++)
                        this.variance[i] = -this.N * this.probabilities[i] * (1.0 - this.probabilities[i]);
                }
        ...
        public override double[,] Covariance
        {
            get
            {
                ...
                {
                    int k = this.probabilities.Length;
                    this.covariance = new double[k, k];
                    for (int i = 0; i < k; i++)
                        for (int j = 0; j < k; j++)
                            this.covariance[i, j] = -this.N * this.probabilities[j] * this.probabilities[i];
                }
        ...
        protected internal override double InnerProbabilityMassFunction(params int[] x)
        {
            ...
            if (x.Sum() != this.N)
                throw new ArgumentException("Elements must sum up to the number of trials (" + this.N + ")", "x");
            ...
            return Math.Exp(this.lnfac - theta + prod);
        }
        ...
        public override void Fit(double[][] observations, double[] weights, IFittingOptions options)
        {
            ...
            double[] pi = new double[this.probabilities.Length];
        ...
        {
            return new MultinomialDistribution(this.N, this.probabilities);
        }
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            ...
            return String.Format(formatProvider, "Multinomial(x; n = {0}, p = {1})",
                this.N.ToString(format, provider),
                this.probabilities.ToString(format, provider));
        }
