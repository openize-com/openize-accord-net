...
namespace Openize.Accord.Statistics.Distributions.Multivariate.Discrete
{
    ...
    ///   <see cref="GeneralDiscreteDistribution"/>
        ...
        IFittableDistribution<double[]>
    {
        ...
        public int[] Maximum
        {
            get { return this.start.Add(this.symbols); }
        }
        ...
        public override IntRange[] Support
        {
            get
            {
                var range = new IntRange[this.Dimension];
                for (int i = 0; i < range.Length; i++)
                    range[i] = new IntRange(this.Minimum[i], this.Maximum[i]);
                return range;
            }
        }
        ...
        public JointDistribution(int[] symbols)
            : base(symbols.Length)
        {
            this.init(new int[symbols.Length], symbols);
        }
        ...
        public JointDistribution(int[] starts, Array probabilities)
            : base(starts.Length)
        {
            this.init(starts, probabilities);
        }
        ...
        {
            this.init(new int[probabilities.Rank], probabilities);
        }
        ...
        private void init(int[] starts, int[] symbols)
        {
            ...
            for (int i = this.positions.Length - 2; i >= 0; i--)
                this.positions[i] = this.positions[i + 1] * symbols[i + 1];
            ...
            for (int i = 0; i < this.probabilities.Length; i++)
                this.probabilities[i] = 1.0 / total;
        }
        ...
        private void init(int[] starts, Array array)
        {
            ...
            this.start = starts;
            this.positions = new int[this.symbols.Length];
            this.probabilities = new double[array.Length];

            this.positions[this.positions.Length - 1] = 1;
            for (int i = this.positions.Length - 2; i >= 0; i--)
                this.positions[i] = this.positions[i + 1] * this.symbols[i + 1];

            foreach (var idx in array.GetIndices(true))
            {
                int index = 0;
                for (int j = 0; j < idx.Length; j++)
                    index += idx[j] * this.positions[j];
                this.probabilities[index] = (double)System.Convert.ChangeType(array.GetValue(true, idx), typeof(double));
            }
        }
        ...
        private int ravel(int[] indices)
        {
            ...
            for (int j = 0; j < indices.Length; j++)
                index += (indices[j] - this.start[j]) * this.positions[j];
            return index;
        }
        ...
        private int ravel(double[] indices)
        {
            int index = 0;
            for (int j = 0; j < indices.Length; j++)
                index += ((int)indices[j] - this.start[j]) * this.positions[j];
            return index;
        }
        ...
        protected internal override double InnerLogProbabilityMassFunction(int[] x)
        {
            ...
            if (index < 0 || index >= this.probabilities.Length)
                return Math.Log(0);
            return Math.Log(this.probabilities[index]);
        }
        ...
        public override double[] Mean
        {
            get
            {
                if (this.mean == null)
                {
                    this.mean = this.start.ToDouble();

                    foreach (int[] idx in Combinatorics.Sequences(this.symbols))
                    {
                        double p = this[idx.Add(this.start)];
        ...
        {
            int[] value = x.Subtract(this.start);
        ...
        public override void Fit(double[][] observations, double[] weights, IFittingOptions options)
        {
            ...
            if (weights != null)
            {
                ...
                for (int i = 0; i < observations.Length; i++)
                    this.probabilities[this.ravel(observations[i])] += weights[i];
            }
            else
            {
                for (int i = 0; i < observations.Length; i++)
                    this.probabilities[this.ravel(observations[i])]++;
            }
