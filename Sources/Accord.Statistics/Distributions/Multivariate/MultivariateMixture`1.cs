...
namespace Openize.Accord.Statistics.Distributions.Multivariate
{
        ...
        where T : class, IDistribution
    {
        ...
        /// </summary>
        /// 
        public double Coefficient { get { return this.mixture.Coefficients[this.index]; } }

        /// <summary>
        ///   Gets the component distribution.
        /// </summary>
        /// 
        public T Component { get { return this.mixture.Components[this.index]; } }
        ...
        where T : IMultivariateDistribution<double[]>
    {
            ...
            : base(components[0].Dimension)
        {
            ...
            for (int i = 0; i < this.coefficients.Length; i++)
                this.coefficients[i] = 1.0 / this.coefficients.Length;

            this.cache = new IDistribution<double[]>[this.coefficients.Length];
            for (int i = 0; i < this.cache.Length; i++)
                this.cache[i] = components[i];
        ...
        {
            return this.coefficients[componentIndex] * this.components[componentIndex].ProbabilityFunction(x);
        }
        ...
        {
            return Math.Log(this.coefficients[componentIndex]) + this.components[componentIndex].LogProbabilityFunction(x);
        }
        ...
        protected internal override double InnerProbabilityDensityFunction(params double[] x)
        {
            ...
            for (int i = 0; i < this.components.Length; i++)
                r += this.coefficients[i] * this.components[i].ProbabilityFunction(x);
            return r;
        }
        ...
        protected internal override double InnerLogProbabilityDensityFunction(params double[] x)
        {
            ...
            for (int i = 0; i < this.components.Length; i++)
                r = Special.LogSum(r, Math.Log(this.coefficients[i]) + this.components[i].LogProbabilityFunction(x));
            return r;
        }
        ...
        protected internal override double InnerDistributionFunction(params double[] x)
        {
            double r = 0.0;
            for (int i = 0; i < this.components.Length; i++)
                r += this.coefficients[i] * this.components[i].DistributionFunction(x);
            return r;
        }
        ...
        {
            return this.coefficients[componentIndex] * this.components[componentIndex].DistributionFunction(x);
        }
        ...
        public override void Fit(double[][] observations, double[] weights, IFittingOptions options)
        {
            ...
            this.Fit(observations, weights, mixOptions);
        }
        ...
        {
            var pdf = new IFittableDistribution<double[]>[this.coefficients.Length];
            for (int i = 0; i < this.components.Length; i++)
                pdf[i] = (IFittableDistribution<double[]>)this.components[i];
            ...
            if (log)
            {
                ...
                var em = new LogExpectationMaximization<double[]>(this.coefficients, pdf);
...
#pragma warning restore 612, 618
            ...
            {
                var em = new ExpectationMaximization<double[]>(this.coefficients, pdf);
...
#pragma warning restore 612, 618
            ...
            for (int i = 0; i < this.components.Length; i++)
                this.cache[i] = this.components[i] = (T)pdf[i];
        ...
        {
            return ExpectationMaximization<double[]>.LogLikelihood(this.coefficients, this.cache,
        ...
        {
            return ExpectationMaximization<double[]>.LogLikelihood(this.coefficients, this.cache, observations);
        }
        ...
        public override object Clone()
        {
            ...
            double[] pi = (double[])this.coefficients.Clone();

            // Clone the mixture components
            T[] pdf = new T[this.components.Length];
            for (int i = 0; i < this.components.Length; i++)
                pdf[i] = (T)this.components[i].Clone();
            ...
            get
            {
                if (this.mean == null)
                {
                    ...
                    for (int j = 0; j < this.mean.Length; j++)
                        for (int i = 0; i < this.coefficients.Length; i++)
                            this.mean[j] += this.coefficients[i] * this.components[i].Mean[j];
                }
        ...
        public override double[,] Covariance
        {
            get
            {
                ...
                {
                    // E[Var[X|Y]]
                    double[,] EVar = new double[this.Dimension, this.Dimension];
                    for (int i = 0; i < this.Dimension; i++)
                    {
                        for (int j = 0; j < this.Dimension; j++)
                        {
                            for (int k = 0; k < this.Components.Length; k++)
                                EVar[i, j] += this.components[k].Covariance[i, j];
                            EVar[i, j] /= this.Components.Length;
                        }
                    }

                    // Var[E[X|Y]]
                    double[][] means = new double[this.components.Length][];
                    for (int k = 0; k < this.components.Length; k++)
                        means[k] = this.components[k].Mean;
                    double[,] VarE = Measures.Scatter(means, (double)this.components.Length).ToMatrix();

                    // Var[X] = E[Var [X|Y]] + Var[E[X|Y]]
                    this.covariance = EVar.Add(VarE, result: EVar);
                }
        ...
        public override double[] Variance
        {
            get
            {
                if (this.variance == null)
                    this.variance = Matrix.Diagonal(this.Covariance);
        ...
        {
            if (this.sampleable == null)
            {
                this.sampleable = new ISampleableDistribution<double[]>[this.components.Length];
                for (int i = 0; i < this.sampleable.Length; i++)
                    this.sampleable[i] = this.components[i] as ISampleableDistribution<double[]>;
            }

            for (int i = 0; i < samples; i++)
            {
                // Choose one coefficient at random
                int j = GeneralDiscreteDistribution.Random(this.coefficients, source);

                // Sample from the chosen coefficient
                result[i] = this.sampleable[j].Generate();
            }
            return result;
        }
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            ...
            for (int i = 0; i < this.coefficients.Length; i++)
            {
                sb.AppendFormat("{0}*", this.coefficients[i].ToString(format, formatProvider));

                var fmt = this.components[i] as IFormattable;

                if (fmt != null)
                    sb.AppendFormat(fmt.ToString(format, formatProvider));
                else sb.AppendFormat(this.components[i].ToString());
