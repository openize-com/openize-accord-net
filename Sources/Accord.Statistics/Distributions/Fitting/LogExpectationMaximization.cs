...
namespace Openize.Accord.Statistics.Distributions.Fitting
{
    ...
    public class LogExpectationMaximization<TObservation> : ParallelLearningBase
    {
            ...
            IFittableDistribution<TObservation>[] distributions)
        {
            ...
            this.LogGamma = new double[coefficients.Length][];
        }
        ...
        {
            return this.compute(observations);
        }
        ...
        private double compute(TObservation[] observations)
        {
            ...
            this.Convergence.NewValue = LogLikelihood(logPi, pdf, observations, this.ParallelOptions);
            ...
            do
            {
                ...
                Parallel.For(0, this.LogGamma.Length, this.ParallelOptions, k =>
                {
                    double[] logGammak = this.LogGamma[k];
                ...
                Parallel.For(0, lnnorms.Length, i =>
                {
                    ...
                    for (int k = 0; k < this.LogGamma.Length; k++)
                        lnsum = Special.LogSum(lnsum, this.LogGamma[k][i]);
                    lnnorms[i] = lnsum;
                });
                ...
                try
                {
                    Parallel.For(0, this.LogGamma.Length, this.ParallelOptions, k =>
                    {
                        double[] lngammak = this.LogGamma[k];
                        ...
                        pdf[k].Fit(observations, lngammak, this.InnerOptions);
                    });
                }
            ...
            for (int i = 0; i < logPi.Length; i++)
                this.Coefficients[i] = Math.Exp(logPi[i]);
