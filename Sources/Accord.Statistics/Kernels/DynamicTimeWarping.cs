...
namespace Openize.Accord.Statistics.Kernels
{
        ...
        ICloneable, IDistance, IDistance<double[][]>
    {
        ...
        {
            double cos = this.k(x, y);

            if (this.degree == 1)
                return cos;

            return System.Math.Pow(cos, this.degree);
        }
        ...
        public double Distance(double[] x, double[] y)
        {
            ...
            return 2 - 2 * System.Math.Pow(cos, this.degree);
        }
        ...
        internal double k(double[] x, double[] y)
        {
            ...
            Locals m = this.locals.Value;

            double[] sx = this.snorm(x);
            double[] sy = this.snorm(y);

            // Compute the cosine of the global distance
            double distance = this.D(m, sx, sy);
        ...
        private unsafe double[] snorm(double[] input)
        {
            ...
            int n = input.Length / this.length;
            ...
            fixed (double* result = projection)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    double norm = this.alpha * this.alpha;

                    for (int j = 0; j < this.length; j++)
                        norm += src[j] * src[j];
                    norm = Math.Sqrt(norm);

                    for (int j = 0; j < this.length; j++, src++, dst++)
                        *dst = *src / norm;
        ...
        private double[][] snorm(double[][] input)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                ...
                projection[i][input[i].Length] = this.alpha / norm;
            }
        ...
        private class Locals
        {
            ...
            public void Create(int n, int m)
            {
                ...
                for (int i = 1; i <= n; i++)
                    this.DTW[i, 0] = double.PositiveInfinity;

                for (int i = 1; i <= m; i++)
                    this.DTW[0, i] = double.PositiveInfinity;
            }
