...
namespace Openize.Accord.Statistics.Kernels
{
    ...
    public sealed class StringSubsequence : KernelBase<string>, IKernel<string>, ICloneable
    {
        ...
        {
            double xy = this.K(this.k, x, x.Length, y, y.Length);

            if (this.normalizationPower > 0)
            {
                double xx = this.K(this.k, x, x.Length, x, x.Length);
                double yy = this.K(this.k, y, y.Length, y, y.Length);
                return xy / Math.Pow(xx + yy, this.normalizationPower);
            }
        ...
        private double K(int n, string s, int slen, string t, int tlen)
        {
            ...
            double sum = this.K(n, s, slen - 1, t, tlen);
            for (int j = 0; j < tlen; j++)
                if (t[j] == x)
                    sum += this.Kp(n - 1, s, slen - 1, t, tlen - j - 1) * (this.lambda * this.lambda);
        ...
        private double Kp(int i, string s, int slen, string t, int tlen)
        {
            ...
            double sum = this.lambda * this.Kp(i, s, slen - 1, t, tlen);
            for (int j = 0; j < tlen; j++)
                if (t[j] == x)
                    sum += this.Kp(i - 1, s, slen - 1, t, tlen - j - 1) * Math.Pow(this.lambda, tlen - (j + 1) + 2);
