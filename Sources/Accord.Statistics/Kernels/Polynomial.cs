...
namespace FileFormat.Accord.Statistics.Kernels
{
        ...
        IKernel<Sparse<double>>, IDistance<Sparse<double>>
    {
        ...
        {
            double sum = x.Dot(y) + this.constant;
            return Math.Pow(sum, this.degree);
        }
        ...
        {
            return Math.Pow(z + this.constant, this.degree);
        }
        ...
        public double Distance(double[] x, double[] y)
        {
            ...
            double sumx = this.constant;
            double sumy = this.constant;
            double sum = this.constant;
        ...
        public double Distance(Sparse<double> x, Sparse<double> y)
        {
            ...
            double sumx = this.constant + x.Dot(x);
            double sumy = this.constant + y.Dot(y);
            double sum = this.constant + x.Dot(y);
        ...
        public double[] Transform(double[] input)
        {
            ...
            {
                case 1:
                    return Linear.Transform(input, this.constant);

                case 2:
                    return Quadratic.Transform(input, this.constant);

                default:
                    return Transform(input, this.degree, this.constant);
            }
        }
