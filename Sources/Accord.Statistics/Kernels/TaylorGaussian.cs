...
namespace FileFormat.Accord.Statistics.Kernels
{
    ...
    public struct TaylorGaussian : ITransform, ILinear, IReverseDistance, IDistance
    {
        ...
        {
            get { return this.gaussian; }
            set { this.gaussian = value; }
        }
        ...
        {
            return this.linear.Function(x, this.Transform(y));
        }
        ...
        public double[] Transform(double[] input)
        {
            ...
            double[] features = new double[this.coefficients.Length];

            features[0] = 1;

            for (int index = 1, k = 0; index < this.coefficients.Length; k++)
            {
                double alpha = this.coefficients[k];
            ...
            double constant = Math.Exp(-this.gaussian.Gamma * norm);
        ...
        private void createCoefficients(int degree)
        {
            ...
            for (int i = 0; i < this.coefficients.Length; i++)
                this.coefficients[i] = Math.Sqrt(Math.Pow(2 * this.gaussian.Gamma, i + 1) / Special.Factorial(i + 1));
        }
        ...
        public object Clone()
        {
            return new TaylorGaussian(this.gaussian, this.Degree);
        }
        ...
        {
            this.linear.Product(a, this.Transform(b), result);
        }
        ...
        {
            return this.linear.Compress(weights, supportVectors.Apply(this.Transform), out c);
        }
        ...
        {
            return this.gaussian.ReverseDistance(x, y);
        }
        ...
        {
            return this.linear.Distance(this.Transform(x), this.Transform(y));
        }
        ...
        {
            this.linear.Add(this.Transform(a), this.Transform(b), result);
        }
