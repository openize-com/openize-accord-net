...
namespace Openize.Accord.Statistics.Kernels
{
    ...
    public struct Wavelet : IKernel, ICloneable, IKernel<int[]>, IDistance, IDistance<int[]>
    {
        ...
        {
            get { return this.dilation; }
            set { this.dilation = value; }
        }
        ...
        public double Function(double[] x, double[] y)
        {
            ...
            if (this.invariant)
            {
                ...
                {
                    prod *= (this.h((x[i] - this.translation) / this.dilation)) *
                            (this.h((y[i] - this.translation) / this.dilation));
                }
            }
            else
            {
                for (int i = 0; i < x.Length; i++)
                    prod *= this.h((x[i] - y[i]) / this.dilation);
            }
        ...
        {
            return this.Function(x, x) + this.Function(y, y) - 2 * this.Function(x, y);
        }
