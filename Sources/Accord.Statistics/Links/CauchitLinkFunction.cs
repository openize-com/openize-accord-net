...
namespace FileFormat.Accord.Statistics.Links
{
    ...
    ///   <see cref="GeneralizedLinearRegression"/> and Support
    ///   Vector Machines.</para>    
    /// </remarks>
    /// 
    /// <seealso cref="ILinkFunction"/>
    /// <seealso cref="GeneralizedLinearRegression"/>
    /// 
    [Serializable]
    public class CauchitLinkFunction : ILinkFunction
    {
        ...
        {
            return Math.Tan((x - this.A) / this.B);
        }
        ...
        {
            return Math.Atan(x) * this.B + this.A;
        }
        ...
        {
            return Math.Log(Math.Atan(x) * this.B + this.A);
        }
        ...
        {
            double x = Math.Tan((y - this.A) / this.B);
            return this.B / (x * x + 1);
        }
