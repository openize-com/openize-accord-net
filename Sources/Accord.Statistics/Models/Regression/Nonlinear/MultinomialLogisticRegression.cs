...
#pragma warning restore 0649
        ...
        public MultinomialLogisticRegression(int inputs, int categories)
        {
            ...
            {
                this.coefficients[i] = new double[inputs + 1];
                this.standardErrors[i] = new double[inputs + 1];
            }
        }
        ...
        {
            for (int i = 0; i < this.coefficients.Length; i++)
                this.coefficients[i][0] = intercepts[i];
        }
        ...
        {
            get { return this.coefficients.GetTotalLength(); }
        }
        ...
        {
            return this.Probabilities(input);
        }
        ...
        public override double LogLikelihood(double[] input, int classIndex)
        {
            ...
            double[] c = this.coefficients[classIndex - 1];
        ...
        {
            for (int j = 1; j < this.coefficients.Length; j++)
            {
                // Get category coefficients
                double[] c = this.coefficients[j - 1];
        ...
        public ChiSquareTest ChiSquare(double[][] input, double[][] output)
        {
            ...
            double[] intercept = new double[this.NumberOfOutputs - 1];
            for (int i = 0; i < intercept.Length; i++)
                intercept[i] = Math.Log(sums[i + 1] / sums[0]);

            var regression = new MultinomialLogisticRegression(this.NumberOfInputs, this.NumberOfOutputs, intercept);

            double ratio = this.GetLogLikelihoodRatio(input, output, regression);

            return new ChiSquareTest(ratio, (this.NumberOfInputs) * (this.NumberOfOutputs - 1));
        }
        ...
        {
            return this.ChiSquare(input, Jagged.OneHot(classes));
        }
        ...
        {
            double coeff = this.coefficients[category][coefficient];
            double error = this.standardErrors[category][coefficient];
        ...
        {
            var ranges = new DoubleRange[this.NumberOfInputs + 1];
            for (int i = 0; i < ranges.Length; i++)
                ranges[i] = this.GetConfidenceInterval(category, i);
            return ranges;
        }
        ...
        {
            return Math.Exp(this.coefficients[category][coefficient]);
        }
        ...
        {
            var odds = new double[this.NumberOfInputs + 1];
            for (int i = 0; i < odds.Length; i++)
                odds[i] = this.GetOddsRatio(category, i);
            return odds;
        }
        ...
        {
            return new WaldTest(this.coefficients[category][coefficient], 0.0, this.standardErrors[category][coefficient]);
        }
        ...
        {
            var tests = new WaldTest[this.NumberOfInputs + 1];
            for (int i = 0; i < tests.Length; i++)
                tests[i] = this.GetWaldTest(category, i);
            return tests;
        }
        ...
        {
            return -2.0 * this.GetLogLikelihood(input, output);
        }
        ...
        {
            return -2.0 * this.GetLogLikelihood(inputs, classes);
        }
        ...
        {
            return this.GetLogLikelihood(inputs, Jagged.OneHot(classes));
        }
        ...
        public double GetLogLikelihood(double[][] input, double[][] output)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                double[] y = this.Probabilities(input[i]);
                double[] o = output[i];
                y = y.Multiply(o.Sum());
        ...
        {
            if (this.NumberOfInputs == 0 && this.NumberOfOutputs == 0)
            {
#pragma warning disable 0618, 0612
                this.NumberOfOutputs = this.inputs;
                this.NumberOfOutputs = this.categories;
#pragma warning restore 0618, 0612
        ...
        public object Clone()
        {
            var mlr = new MultinomialLogisticRegression(this.NumberOfInputs, this.NumberOfOutputs);
            for (int i = 0; i < this.coefficients.Length; i++)
            {
                for (int j = 0; j < this.coefficients[i].Length; j++)
                {
                    mlr.coefficients[i][j] = this.coefficients[i][j];
                    mlr.standardErrors[i][j] = this.standardErrors[i][j];
                }
            }
