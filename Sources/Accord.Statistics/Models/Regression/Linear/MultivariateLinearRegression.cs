...
#pragma warning restore 612, 618
    {
        ...
        public double[][] Weights
        {
            ...
            {
                this.weights = value;
                this.NumberOfInputs = value.Rows();
                this.NumberOfOutputs = value.Columns();
                this.coefficients = value.ToMatrix();
            }
        }
        ...
        {
            get { return this.intercepts; }
            set { this.intercepts = value; }
        }
        ...
        {
            get { return this.NumberOfInputs * this.NumberOfOutputs + this.NumberOfOutputs; }
        }
        ...
        public int Inputs
        {
            get { return this.coefficients.GetLength(0); }
        }
        ...
        public int Outputs
        {
            get { return this.coefficients.GetLength(1); }
        }
        ...
        public virtual double Regress(double[][] inputs, double[][] outputs)
        {
            ...
            for (int c = 0; c < this.coefficients.GetLength(1); c++)
            {
            ...
            this.Weights = this.coefficients.ToJagged();

            // Calculate Sum-Of-Squares error
            double error = 0.0, e;
            for (int i = 0; i < outputs.Length; i++)
            {
                double[] y = this.Transform(inputs[i]);
        ...
        {
            return this.CoefficientOfDetermination(inputs, outputs, false, weights);
        }
        ...
        {
            int N = input.Length;
            int M = this.coefficients.GetLength(1);
        ...
        {
            double[][] inv = this.Weights.PseudoInverse();
            return new MultivariateLinearRegression()
            {
                Weights = inv,
                Intercepts = this.intercepts != null ? this.intercepts.Dot(inv).Multiply(-1) : null
            };
        }
...
#pragma warning restore 612, 618
        ...
        {
            input.Dot(this.Weights, result: result);
            if (this.intercepts != null)
                result.Add(this.intercepts, dimension: (VectorType)0, result: result);
            return result;
        }
        ...
        public double[] GetStandardError(double[][] inputs, double[][] outputs)
        {
            ...
            double[][] results = this.Transform(inputs);

            double[] SSe = new double[this.NumberOfOutputs];
            ...
            double DFe = this.GetDegreesOfFreedom(inputs.Length);
        ...
        {
            double[][] se = Jagged.Zeros(this.NumberOfOutputs, informationMatrix.Length);
        ...
        public DoubleRange[] GetConfidenceInterval(double[] input, double[] mse, int numberOfSamples, double[][] informationMatrix, double percent = 0.95)
        {
            ...
            return this.createInterval(input, numberOfSamples, percent, se);
        }
