...
namespace Openize.Accord.Statistics.Models.Survival
{
        ...
        BinaryLikelihoodClassifierBase<Tuple<double[], double>>
    {
        ...
        public ProportionalHazards(int inputs, IUnivariateDistribution baseline)
        {
            ...
            this.StandardErrors = new double[inputs];
#pragma warning disable 612, 618
            this.Offsets = new double[inputs];
#pragma warning restore 612, 618
            this.BaselineHazard = baseline;
        }
        ...
        {
            this.BaselineHazard = new EmpiricalHazardDistribution();
        }
        ...
        {
            return this.Probability(input, time);
        }
        ...
        {
            if (this.BaselineHazard == null)
                throw new InvalidOperationException();

            return this.BaselineHazard.ComplementaryDistributionFunction(time);
        }
        ...
        public double GetLogHazardRatio(double[] x, double[] y)
        {
            ...
            for (int i = 0; i < this.Coefficients.Length; i++)
                sum += this.Coefficients[i] * (x[i] - y[i]);
            return sum;
        }
        ...
        {
            return -2.0 * this.GetPartialLogLikelihood(inputs, time, output);
        }
        ...
        {
            return this.GetPartialLogLikelihood(inputs, time, output.To<SurvivalOutcome[]>());
        }
...
#pragma warning disable 612, 618
            ...
            for (int i = 0; i < inputs.Length; i++)
            {
                ...
                for (int j = 0; j < this.Coefficients.Length; j++)
                    sum1 += this.Coefficients[j] * (inputs[i][j] - this.Offsets[j]);
                ...
                for (int j = 0; j < inputs.Length; j++)
                {
                    if (time[j] >= time[i])
                    {
                        ...
                        for (int k = 0; k < this.Coefficients.Length; k++)
                            s += this.Coefficients[k] * (inputs[j][k] - this.Offsets[k]);
                        sum += Math.Exp(s);
                    }
                }
                sum2 += Math.Log(sum);
...
#pragma warning restore 612, 618
        ...
        {
            return this.GetPartialLogLikelihood(time, output.To<SurvivalOutcome[]>());
        }
        ...
        {
            double coeff = this.Coefficients[index];
            double error = this.StandardErrors[index];
        ...
        {
            return new WaldTest(this.Coefficients[index], 0.0, this.StandardErrors[index]);
        }
        ...
        {
            return this.ChiSquare(input, time, output.To<SurvivalOutcome[]>());
        }
        ...
        {
            ProportionalHazards regression = new ProportionalHazards(this.NumberOfInputs);

            double ratio = this.GetLogLikelihoodRatio(input, time, output, regression);

            return new ChiSquareTest(ratio, this.Coefficients.Length);
        }
        ...
        public object Clone()
        {
            var regression = new ProportionalHazards(this.Coefficients.Length);
        ...
        {
            return Math.Exp(this.Coefficients[index]);
        }
...
#pragma warning disable 612, 618
            ...
            {
                double sum = this.Intercept;
                for (int i = 0; i < this.Coefficients.Length; i++)
                    sum += this.Coefficients[i] * (input[j].Item1[i] - this.Offsets[i]);
                double h0 = this.BaselineHazard.LogCumulativeHazardFunction(input[j].Item2);
                result[j] = h0 + sum;
            }
            return result;
#pragma warning restore 612, 618
        ...
        {
            return this.Decide(Tuple.Create(input, time)) ? SurvivalOutcome.Failed : SurvivalOutcome.Censored;
        }
        ...
        public SurvivalOutcome[] Decide(double[][] input, double[] time)
        {
            ...
            for (int i = 0; i < input.Length; i++)
                result[i] = this.Decide(input[i], time[i]);
            return result;
        }
        ...
        {
            return this.Probability(Tuple.Create(input, time));
        }
        ...
        public double[] Score(double[][] input, double[] time)
        {
            ...
            for (int i = 0; i < input.Length; i++)
                result[i] = this.Probability(input[i], time[i]);
            return result;
        }
        ...
        {
            return this.LogLikelihood(Tuple.Create(input, time));
        }
        ...
        public double[] LogLikelihood(double[][] input, double[] time)
        {
            var result = new double[input.Length];
            for (int i = 0; i < input.Length; i++)
                result[i] = this.LogLikelihood(input[i], time[i]);
            return result;
        }
        ...
        {
            return this.Probability(input) > 0.5 ? SurvivalOutcome.Failed : SurvivalOutcome.Censored;
        }
        ...
        {
            return this.LogLikelihood(input) - Math.Log(0.5);
        }
        ...
        {
            return Math.Exp(this.LogLikelihood(input));
        }
...
#pragma warning disable 612, 618
            double sum = this.Intercept;
            for (int i = 0; i < this.Coefficients.Length; i++)
                sum += this.Coefficients[i] * (input[i] - this.Offsets[i]);
            return sum;
#pragma warning restore 612, 618
        }
        ...
        public double Probability(double time)
        {
            ...
            return this.BaselineHazard.CumulativeHazardFunction(time);
        }
        ...
        public double LogLikelihood(double time)
        {
            ...
            return this.BaselineHazard.LogCumulativeHazardFunction(time);
        }
