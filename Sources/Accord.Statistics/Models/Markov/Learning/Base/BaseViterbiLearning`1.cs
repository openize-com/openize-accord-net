    ...
    public abstract class BaseViterbiLearning<T>
    {
        ...
        public double Run(params T[] observations)
        {
            ...
            do // Until convergence or max iterations is reached
            {
                ...
                if (this.batches == 1)
                {
                    this.RunEpoch(observations, paths);
                }
                else
                {
                    // Divide in batches
                    int[] groups = Classes.Random(observations.Length, this.batches);
                ...
                newLogLikelihood = this.ComputeLogLikelihood(observations);

                // Check convergence
                this.convergence.NewValue = newLogLikelihood;

            } while (!this.convergence.HasConverged && !this.Token.IsCancellationRequested);
