...
namespace FileFormat.Accord.Statistics.Models.Markov.Learning.Base
{
        ...
        where TOptions : IFittingOptions
    {
        ...
        public TModel Learn(TObservation[][] x, int[][] y, double[] weights = null)
        {
            ...
            if (this.Model == null)
                this.Model = this.Create(x, numberOfClasses: y.Max() + 1);

            MarkovHelperMethods.CheckObservationDimensions(x, this.Model);

            // Grab model information
            var model = this.Model;
            var fittingOptions = this.FittingOptions;
            ...
            Debug.Assert(!model.LogInitial.HasNaN());
            Debug.Assert(!model.LogTransitions.HasNaN());
