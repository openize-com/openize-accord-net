...
namespace Openize.Accord.Statistics.Models.Markov.Learning.Base
{
    ...
    public abstract class BaseBaumWelchLearning : IConvergenceLearning
    {
        ...
        protected double Run(Array[] observations)
        {
            ...
            this.LogWeights = new double[observations.Length];

            return this.run(observations);
        }
        ...
        protected double Run(Array[] observations, double[] weights)
        {
            ...
            this.LogWeights = new double[observations.Length];
            for (int i = 0; i < observations.Length; i++)
                this.LogWeights[i] = Math.Log(weights[i]);
        ...
        private double run(Array[] observations)
        {
            ...
            this.convergence.NewValue = Double.NegativeInfinity;
            ...
            do // Until convergence or max iterations is reached
            {
                ...
                for (int i = 0; i < observations.Length; i++)
                {
                    ...
                    this.ComputeForwardBackward(i, lnFwd, lnBwd);
                ...
                this.convergence.NewValue = newLogLikelihood;

                // Check for convergence
                if (!this.convergence.HasConverged)
                {
