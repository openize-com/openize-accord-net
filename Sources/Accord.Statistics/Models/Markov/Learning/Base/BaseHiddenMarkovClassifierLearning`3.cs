...
#pragma warning disable 612, 618
        ...
        where TDistribution : IDistribution<TObservation>
    {
        ...
        {
            get { return this.ParallelOptions.CancellationToken; }
            set { this.ParallelOptions.CancellationToken = value; }
        }
        ...
        protected double Run<T>(T[] inputs, int[] outputs)
        {
            ...
            for (int i = 0; i < outputs.Length; i++)
                if (outputs[i] < 0 || outputs[i] >= this.Classifier.Classes)
                    throw new ArgumentOutOfRangeException("outputs");


            int classes = this.Classifier.Classes;
            ...
            Parallel.For(0, classes, i =>
            {
                ...
                this.OnGenerativeClassModelLearningStarted(args);
                ...
                this.OnGenerativeClassModelLearningFinished(args);
            });

            if (this.Empirical)
            {
                for (int i = 0; i < classes; i++)
                    this.Classifier.Priors[i] = (double)classCounts[i] / inputs.Length;
            }

            if (this.Rejection)
            {
                this.Classifier.Threshold = this.Threshold();
            }
        ...
        {
            var models = this.Classifier.Models;
        ...
        {
            if (this.ClassModelLearningFinished != null)
                this.ClassModelLearningFinished(this, args);
        }
        ...
        {
            if (this.ClassModelLearningStarted != null)
                this.ClassModelLearningStarted(this, args);
        }
        ...
        public TClassifier Learn(TObservation[][] x, int[] y, double[] weights = null)
        {
            ...
            if (this.Classifier == null)
                this.Classifier = this.Create(x, y, numberOfClasses: y.Max() + 1); // create a new classifier given the number of classes

            for (int i = 0; i < y.Length; i++)
                if (y[i] < 0 || y[i] >= this.Classifier.Classes)
                    throw new ArgumentOutOfRangeException("outputs");
            ...
            {
                for (int i = 0; i < classes; i++)
                    this.LearnInner(x, y, i, classes, logLikelihood, classCounts);
            }
            else
            {
                // For each model,
                Parallel.For(0, classes, this.ParallelOptions, i =>
                    this.LearnInner(x, y, i, classes, logLikelihood, classCounts));
            }
            ...
            {
                for (int i = 0; i < classes; i++)
                    this.Classifier.Priors[i] = (double)classCounts[i] / x.Length;
            }
            ...
            this.LogLikelihood = logLikelihood.Sum();
        ...
        private void LearnInner(TObservation[][] x, int[] y, int i, int classes, double[] logLikelihood, int[] classCounts)
        {
            ...
            {
                // Create and configure the learning algorithm
                var innerModelTeacher = this.Learner(i);
                var innerModel = innerModelTeacher.Learn(observations);
                this.Classifier.Models[i] = innerModel;
