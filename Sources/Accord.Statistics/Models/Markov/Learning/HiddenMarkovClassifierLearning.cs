...
namespace Openize.Accord.Statistics.Models.Markov.Learning
{
        ...
        BaseHiddenMarkovClassifierLearning<HiddenMarkovClassifier, HiddenMarkovModel, GeneralDiscreteDistribution, int>
    {
        ...
        {
            get { return this.smoothingSigma; }
            set
            {
                this.smoothingSigma = value;
        ...
        public double Run(int[][] inputs, int[] outputs)
        {
            ...
            for (int i = 0; i < inputs.Length; i++)
            {
                for (int j = 0; j < inputs[i].Length; j++)
                {
                    ...
                    if (symbol < 0 || symbol >= this.Classifier.Symbols)
                    {
                        ...
                        throw new ArgumentOutOfRangeException("inputs", String.Format(message, this.Classifier.Symbols));
                    }
                }
            }
        ...
        public double ComputeError(int[][] inputs, int[] outputs)
        {
            ...
            Parallel.For(0, inputs.Length, i =>
            {
                ...
                int actualOutput = this.Classifier.Compute(inputs[i]);
        ...
        public override HiddenMarkovModel Threshold()
        {
            ...
            var topology = this.CreateThresholdTopology();
            ...
            // Apply smoothing
            if (this.smoothingSigma > 0)
            {
                for (int i = 0; i < states; i++)
                {
                    double[] e = emissions.GetRow(i);
                    double[] g = e.Convolve(this.gaussianKernel, true);
        ...
        {
            Gaussian g = new Gaussian(this.smoothingSigma);
            this.gaussianKernel = g.Kernel(this.smoothingKernelSize);

            // Normalize
            double norm = this.gaussianKernel.Euclidean();
            this.gaussianKernel = this.gaussianKernel.Divide(norm);
        }
