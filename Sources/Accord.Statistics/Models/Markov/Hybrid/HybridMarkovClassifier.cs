...
namespace FileFormat.Accord.Statistics.Models.Markov.Hybrid
{
    ...
    public class HybridMarkovClassifier 
    {
        ...
        /// been rejected by the <see cref="BaseHiddenMarkovClassifier{TModel}.Threshold">
        /// threshold model</see>.</returns>
        /// 
        public int Compute(double[][] sequence)
        {
            double[] responses;
            return this.Compute(sequence, out responses);
        }
        ...
        {
            double[] responses;
            int result =  this.Compute(sequence, out responses);
        ...
        {
            responsibilities = new double[this.Models.Length];
            for (int i = 0; i < responsibilities.Length; i++)
                responsibilities[i] = this.Models[i].Evaluate(sequence);
