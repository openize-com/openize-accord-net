...
namespace FileFormat.Accord.Statistics.Models.Markov.Hybrid
{
    ...
    public class HybridMarkovModel
    {
        ...
        public int[] Decode(double[][] observations, out double logLikelihood)
        {
            ...
            // Base
            output[0] = this.Function(-1, observations[0]);
            ...
            for (int t = 1; t < T; t++)
            {
                ...
                for (int i = 0; i < this.States; i++)
                    output[i] = this.Function(i, observation);
        ...
        {
            int T = observations.Length;
            double[,] lnFwd = new double[this.States, T];
            double[][] output = new double[this.States][];
