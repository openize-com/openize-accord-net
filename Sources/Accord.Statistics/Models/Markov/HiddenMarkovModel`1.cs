        ...
        where TDistribution : IDistribution
    {
            ...
            : base(topology)
        {
            ...
            for (int i = 0; i < this.B.Length; i++)
                this.B[i] = (TDistribution)emissions.Clone();


            if (emissions is IMultivariateDistribution)
            {
                this.multivariate = true;
                this.dimension = ((IMultivariateDistribution)this.B[0]).Dimension;
            }
        }
        ...
        public int[] Decode(Array observations, out double logLikelihood)
        {
            ...
            // Argument check
            double[][] x = MarkovHelperMethods.checkAndConvert(observations, this.dimension);

            return this.viterbi(x, out logLikelihood);
        }
        ...
        private int[] viterbi(double[][] x, out double logLikelihood)
        {
            ...
            double[] logPi = this.Probabilities;
            double[,] logA = this.Transitions;
            ...
            for (int i = 0; i < states; i++)
                lnFwd[i, 0] = logPi[i] + this.B[i].LogProbabilityFunction(x[0]);
            ...
            for (int t = 1; t < T; t++)
            {
                ...
                for (int j = 0; j < states; j++)
                {
                    ...
                    lnFwd[j, t] = maxWeight + this.B[j].LogProbabilityFunction(observation);
                    s[j, t] = maxState;
                }
            }
        ...
        public double[][] Posterior(Array observations)
        {
            ...
            {
                double[] states = probabilities[i] = new double[this.States];
        ...
        {
            double[][] probabilities = this.Posterior(observations);

            path = new int[observations.Length];
            for (int i = 0; i < path.Length; i++)
                Matrix.Max(probabilities[i], out path[i]);
        ...
        public double Evaluate(Array observations, int[] path)
        {
            ...
            try
            {
                double logLikelihood = this.Probabilities[path[0]]
                    + this.Emissions[path[0]].LogProbabilityFunction(x[0]);

                for (int i = 1; i < observations.Length; i++)
                {
                    double a = this.Transitions[path[i - 1], path[i]];
                    double b = this.Emissions[path[i]].LogProbabilityFunction(x[i]);
                    logLikelihood += a + b;
                }
        ...
        public double[] Predict(double[][] observations, out double logLikelihood)
        {
            ...
            double[][] prediction = this.predict(observations, 1, out logLikelihood, out weights);
        ...
        public double Predict(double[] observations, out double logLikelihood)
        {
            ...
            double[][] obs = MarkovHelperMethods.convertNoCheck(observations, this.dimension);
            ...
            // Compute the next observation (currently only one ahead is supported).
            double[][] prediction = this.predict(obs, 1, out logLikelihood, out weights);
            ...
            where TMultivariate : DistributionBase, TDistribution, IMultivariateDistribution<double[]>
        {
            ...
            // Compute the next observation (currently only one ahead is supported)
            double[][] prediction = this.predict(observations, out logLikelihood, out probabilities);
            ...
            where TMultivariate : DistributionBase, TDistribution, IMultivariateDistribution<double[]>
        {
            ...
            // Compute the next observation (currently only one ahead is supported)
            double[][] prediction = this.predict(observations, out probability, out probabilities);
        ...
        public double[][] Predict(double[][] observations, int next, out double logLikelihood)
        {
            ...
            double[][] prediction = this.predict(observations, next, out logLikelihood, out weights);
        ...
        public double[] Predict(double[] observations, int next, out double logLikelihood)
        {
            ...
            // Compute the next observations
            double[][] prediction = this.predict(obs, next, out logLikelihood, out weights);

            // Return the first (single) dimension of the next observations.
            return Matrix.Concatenate(prediction);
        }
        ...
        {
            double[] transitions = this.Probabilities;

            logLikelihood = 0; // log(1)
            path = new int[samples];

            var multivariate = this.Emissions as ISampleableDistribution<double[]>[];
            if (multivariate != null)
            {
                ...
                for (int t = 0; t < observations.Length; t++)
                {
                    ...
                    // Continue sampling
                    transitions = this.Transitions.GetRow(state);
                }

                return observations;
            }

            var univariate = this.Emissions as ISampleableDistribution<double>[];
            if (univariate != null)
            {
                double[] observations = new double[samples];
        ...
        public object Clone()
        {
            double[,] A = (double[,])this.Transitions.Clone();
            double[] pi = (double[])this.Probabilities.Clone();

            TDistribution[] B = new TDistribution[this.Emissions.Length];
            for (int i = 0; i < this.Emissions.Length; i++)
                B[i] = (TDistribution)this.Emissions[i].Clone();
