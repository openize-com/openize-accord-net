...
namespace Openize.Accord.Statistics.Models.Markov.Topology
{
    ...
    /// <seealso cref="ITopology"/>
    /// <seealso cref="Ergodic"/>
    /// <seealso cref="Custom"/>
    ///
    /// 
    /// <example>
    ...
    public class Forward : ITopology
    {
        ...
        {
            int m = System.Math.Min(this.States, this.Deepness);
            double[,] A = new double[this.States, this.States];
            ...
            else
            {
                ...
                for (int i = 0; i < this.states; i++)
                {
                    double d = 1.0 / Math.Min(m, this.states - i);
                    for (int j = i; j < this.states && (j - i) < m; j++)
                        A[i, j] = d;
                }
            }
            ...
            if (logarithm)
            {
                ...
                initialState = Elementwise.Log(this.pi);
            }
            else
            {
                transitionMatrix = A;
                initialState = (double[])this.pi.Clone();
            }
