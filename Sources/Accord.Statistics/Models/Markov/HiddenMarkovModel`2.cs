...
namespace Openize.Accord.Statistics.Models.Markov
{
    ...
    ///   as <see cref="NormalDistribution">Gaussian</see>
    ///   <see cref="Mixture{T}">Mixture Model</see>) for
    ...
    ///  
    ///  <para>
    ///   If a <see cref="GeneralDiscreteDistribution">general
        ...
        where TDistribution : IDistribution<TObservation>
    {
        ...
        {
            double[,] a;
            this.states = topology.Create(true, out a, out this.logPi);
            this.logA = a.ToJagged();
            this.NumberOfOutputs = this.states;
        }
            ...
            : this(topology)
        {
            ...
            this.B = new TDistribution[this.states];
            for (int i = 0; i < this.B.Length; i++)
                this.B[i] = (TDistribution)emissions(i);


            if (this.B[0] is IMultivariateDistribution)
                this.NumberOfInputs = ((IMultivariateDistribution)this.B[0]).Dimension;
        }
        ...
        private void emissions(TObservation[] x, double[,] output)
        {
            for (int t = 0; t < x.Length; t++)
                for (int j = 0; j < this.states; j++)
                    output[j, t] = this.B[j].LogProbabilityFunction(x[t]);
        }
        ...
        private void viterbiDecode(TObservation[] x, out double logLikelihood, double[,] lnFwd, int[,] s, ref int[] path)
        {
            int T = x.Length;
            int maxState;
            logLikelihood = this.viterbiLikelihood(x, lnFwd, s, out maxState);
        ...
        {
            this.viterbiPosterior(x, lnFwd, s, null);
        ...
        private double viterbiLikelihood(TObservation[] observations, int[] path)
        {
            ...
            try
            {
                double logLikelihood = this.LogInitial[path[0]]
                    + this.Emissions[path[0]].LogProbabilityFunction(observations[0]);

                for (int i = 1; i < observations.Length; i++)
                {
                    double a = this.LogTransitions[path[i - 1]][path[i]];
                    double b = this.Emissions[path[i]].LogProbabilityFunction(observations[i]);
                    logLikelihood += a + b;
                }
            ...
            {
                this.checkHiddenStates(ex, path);
                throw;
            }
        }
        ...
        private void viterbiPosterior(TObservation[] x, double[,] lnFwd, int[,] s, double[][] result)
        {
            ...
            int T = x.Length;
            double[] logPi = this.LogInitial;
            double[][] logA = this.LogTransitions;

            // Base
            for (int i = 0; i < this.states; i++)
                lnFwd[0, i] = logPi[i] + this.B[i].LogProbabilityFunction(x[0]);
            ...
            for (int t = 1; t < x.Length; t++)
            {
                ...
                for (int j = 0; j < this.states; j++)
                {
                    ...
                    lnFwd[t, j] = maxWeight + this.B[j].LogProbabilityFunction(observation);
                    s[t, j] = maxState;
                }
            }
        ...
        {
            double logLikelihood;
            return this.Decode(observations, out logLikelihood);
        }
        ...
        {
            var prev = this.Algorithm;
            this.Algorithm = HiddenMarkovModelAlgorithm.Viterbi;
            int[] path = null;
            logLikelihood = this.LogLikelihood(observations, ref path);
            this.Algorithm = prev;
            return path;
        }
        ...
        public double[][] Posterior(TObservation[] observations)
        {
            var prev = this.Algorithm;
            this.Algorithm = HiddenMarkovModelAlgorithm.Forward;
            double[][] r = this.LogLikelihoods(observations);
            this.Algorithm = prev;
            return r;
        }
        ...
        public double[][] Posterior(TObservation[] observations, out int[] path)
        {
            ...
            path = null;
            double[][] r = this.LogLikelihoods(observations, ref path);
            this.Algorithm = prev;
            return r;
        }
        ...
        public double Evaluate(TObservation[] observations)
        {
            var prev = this.Algorithm;
            this.Algorithm = HiddenMarkovModelAlgorithm.Forward;
            double r = this.LogLikelihood(observations);
            this.Algorithm = prev;
            return r;
        }
        ...
        {
            return this.viterbiLikelihood(observations, path);
        }
        ...
        public TObservation Predict(TObservation[] observations)
        {
            ...
            double logLikelihood;
            return this.Predict(observations, out logLikelihood);
        }
        ...
        public TObservation Predict(TObservation[] observations, out double logLikelihood)
        {
            ...
            return this.predict(observations, 1, out logLikelihood, out weights)[0];
        }
        ...
        {
            // Compute the next observation (currently only one ahead is supported)
            return this.predict(observations, out logLikelihood, out probabilities)[0];
        }
            ...
            where TMultivariate : DistributionBase, TDistribution, IMultivariateDistribution<double[]>
        {
            ...
            // Compute the next observation (currently only one ahead is supported)
            return this.predict(observations, out probability, out probabilities)[0];
        }
        ...
        public virtual TObservation[] Predict(TObservation[] observations, int next)
        {
            ...
            return this.predict(observations, next, out logLikelihood, out weights);
        }
        ...
        public TObservation[] Generate(int samples)
        {
            ...
            return this.Generate(samples, out path, out logLikelihood);
        }
        ...
        {
            double[] transitions = this.LogInitial;
            ...
            for (int t = 0; t < observations.Length; t++)
            {
                ...
                var sampling = (ISampleableDistribution<TObservation>)this.Emissions[state];
                ...
                logLikelihood += transitions[state] +
                    this.Emissions[state].LogProbabilityFunction(symbol);

                // Continue sampling
                transitions = this.LogTransitions.GetRow(state);
            }
            ...
            where TMultivariate : DistributionBase, TDistribution, IMultivariateDistribution<double[]>
        {
            ...
            // Compute the next observation (currently only one ahead is supported).
            TObservation[] prediction = this.predict(observations, 1, out logLikelihood, out weights);
            ...
            TMultivariate[] b = this.B.Apply(x => (TMultivariate)x);
            ...
            where TUnivariate : DistributionBase, TDistribution, IUnivariateDistribution<double>
        {
            ...
            // Create the mixture distribution defining the model likelihood in
            // assuming the next observation belongs will belong to each state.
            TUnivariate[] b = this.B.Apply(x => (TUnivariate)x);
        ...
        private TObservation[] predict(TObservation[] observations, int next, out double logLikelihood, out double[][] lnFuture)
        {
            ...
            TObservation[] expectation = new TObservation[this.states];

            // Compute expectations for each state
            for (int i = 0; i < this.states; i++)
                expectation[i] = getMode(this.B[i]);
            ...
            for (int i = 0; i < lnFwd.Length; i++)
                lnFwd[i] = new double[this.states];
            ...
            for (int t = 0; t < T; t++)
            {
                ...
                for (int i = 0; i < weights.Length; i++)
                {
                    ...
                    weights[i] = sum + this.B[i].LogProbabilityFunction(expectation[i]);
                }
        ...
        private void checkHiddenStates(IndexOutOfRangeException ex, int[] path)
        {
            for (int i = 0; i < path.Length; i++)
            {
                if (path[i] < 0 || path[i] >= this.states)
                {
                    throw new ArgumentException("path", "The hidden states vector must "
                    + "only contain values higher than or equal to 0 and less than " + this.states
        ...
        {
            double[][] A = this.LogTransitions.Copy();
            double[] pi = this.LogInitial.Copy();
            TDistribution[] B = this.Emissions.MemberwiseClone();
        ...
        {
            return this.viterbiLikelihood(sequences, path);
        }
        ...
        public double[] LogLikelihood(TObservation[][] sequences, int[][] path)
        {
            return this.LogLikelihood(sequences, path, new double[sequences.Length]);
        }
        ...
        public double[] LogLikelihood(TObservation[][] sequences, int[][] path, double[] result)
        {
            for (int i = 0; i < sequences.Length; i++)
                result[i] = this.viterbiLikelihood(sequences[i], path[i]);
            return result;
        }
        ...
        public override double[] LogLikelihood(TObservation[][] sequences, double[] result)
        {
            ...
            var lnFwd = new double[T, this.states];

            if (this.Algorithm == HiddenMarkovModelAlgorithm.Forward)
            {
                for (int i = 0; i < sequences.Length; i++)
                    result[i] = this.forwardLikelihood(sequences[i], lnFwd);
            }
            else
            {
                var s = new int[T, this.states];
                int maxState;
                for (int i = 0; i < sequences.Length; i++)
                    result[i] = this.viterbiLikelihood(sequences[i], lnFwd, s, out maxState);
            }
        ...
        public override double[] LogLikelihood(TObservation[][] sequences, ref int[][] decision, double[] result)
        {
            ...
            for (int i = 0; i < sequences.Length; i++)
                this.viterbiDecode(sequences[i], out result[i], lnFwd, s, ref decision[i]);

            if (this.Algorithm == HiddenMarkovModelAlgorithm.Forward)
            {
                for (int i = 0; i < sequences.Length; i++)
                    result[i] = this.forwardLikelihood(sequences[i], lnFwd); // overwrite
            }
        ...
        public override double[][] LogLikelihoods(TObservation[] sequence, double[][] result)
        {
            ...
            if (this.Algorithm == HiddenMarkovModelAlgorithm.Forward)
            {
                var lnBwd = new double[T, this.states];
                this.forwardPosterior(sequence, lnFwd, lnBwd, result);
            }
            else
            {
                var s = new int[T, this.states];
                this.viterbiPosterior(sequence, lnFwd, s, result);
            }
        ...
        public override double[][] LogLikelihoods(TObservation[] sequence, ref int[] decision, double[][] result)
        {
            ...
            double logLikelihood;
            var s = new int[T, this.states];
            this.viterbiDecode(sequence, out logLikelihood, lnFwd, s, ref decision);
        ...
        public override int[][] Decide(TObservation[][] input, int[][] result)
        {
            ...
            for (int i = 0; i < input.Length; i++)
                this.viterbiDecode(input[i], out logLikelihood, lnFwd, s, ref result[i]);
        ...
        public override int[] Decide(TObservation[] input, int[] result)
        {
            ...
            var s = new int[T, this.states];
            double logLikelihood;
            this.viterbiDecode(input, out logLikelihood, lnFwd, s, ref result);
            return result;
        }
