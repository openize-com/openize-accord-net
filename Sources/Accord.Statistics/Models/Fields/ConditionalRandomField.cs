...
namespace FileFormat.Accord.Statistics.Models.Fields
{
    ...
    public class ConditionalRandomField<T> : TaggerBase<T>
    {
        ...
        {
            return Math.Exp(this.LogPartition(observations));
        }
        ...
        public double LogPartition(T[] observations)
        {
            ...
            ForwardBackwardAlgorithm.LogForward(this.Function.Factors[0], observations, 0, out logLikelihood);
        ...
        {
            double p = this.Function.Factors[0].Compute(-1, labels[0], observations, 0);
            for (int t = 1; t < observations.Length; t++)
                p += this.Function.Factors[0].Compute(labels[t - 1], labels[t], observations, t);

            double z = this.LogPartition(observations);
            ...
            Debug.Assert(!Double.IsInfinity(p));

            Debug.Assert(!Double.IsNaN(z));
            Debug.Assert(!Double.IsInfinity(z));
        ...
        {
            return this.viterbi(this.Function.Factors[0], observations, out logLikelihood);
        }
        ...
        private int[] viterbi(FactorPotential<T> factor, T[] observations, out double logLikelihood)
        {
            ...
            for (int i = 0; i < states; i++)
                lnFwd[i, 0] = this.Function.Factors[0].Compute(-1, i, observations, 0);
            ...
            for (int t = 1; t < observations.Length; t++)
            {
                ...
                {
                    maxState = 0;
                    maxWeight = lnFwd[0, t - 1] + this.Function.Factors[0].Compute(0, j, observations, t);

                    for (int i = 1; i < states; i++)
                    {
                        weight = lnFwd[i, t - 1] + this.Function.Factors[0].Compute(i, j, observations, t);
        ...
        public double LogLikelihood(T[][] observations, int[][] labels)
        {
            ...
            for (int i = 0; i < observations.Length; i++)
                logLikelihood += this.LogLikelihood(observations[i], labels[i]);

            Debug.Assert(!Double.IsNaN(logLikelihood));
            Debug.Assert(!Double.IsInfinity(logLikelihood));
...
#pragma warning disable 612, 618
        ...
        {
            this.Save(new FileStream(path, FileMode.Create));
        }
...
#pragma warning restore 612, 618
        ...
        public object Clone()
        {
            return new ConditionalRandomField<T>(this.States, (IPotentialFunction<T>)this.Function.Clone());
        }
        ...
        public override int[][] Decide(T[][] input, int[][] result)
        {
            ...
            for (int i = 0; i < input.Length; i++)
                this.Compute(input[i], out logLikelihood).CopyTo(result[i]);
            return result;
        }
    }
