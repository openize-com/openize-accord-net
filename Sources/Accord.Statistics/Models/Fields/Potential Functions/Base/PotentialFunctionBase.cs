...
namespace FileFormat.Accord.Statistics.Models.Fields.Potential_Functions.Base
{
    using System;
    ...
    public abstract class PotentialFunctionBase<T> 
    {
        ...
        {
            double[] featureVector = new double[this.Weights.Length];

            for (int i = 0; i < featureVector.Length; i++)
                featureVector[i] = this.Features[i].Compute(states, observations, output);
        ...
        internal void Deoptimize()
        {
            for (int i = 0; i < this.Factors.Length; i++)
            {
                ...
                this.Factors[i] = new FactorPotential<T>(factor.Owner, factor.States, factor.Index,
