...
namespace FileFormat.Accord.Statistics.Models.Fields.Potential_Functions.Clique_Factor_Potentials
{
    using System;
    ...
    public class FactorPotential<T> : IEnumerable<IFeature<T>>
    {
        ...
        {
            this.EdgeParameters = new ArraySegment<double>(owner.Weights, edgeIndex, edgeCount);
            this.StateParameters = new ArraySegment<double>(owner.Weights, stateIndex, stateCount);
            this.OutputParameters = new ArraySegment<double>(owner.Weights, classIndex, classCount);

            this.FactorParameters = new ArraySegment<double>(owner.Weights,
        ...
        {
            double p = this.Compute(-1, states[0], observations, 0, output);
            for (int t = 1; t < observations.Length; t++)
                p += this.Compute(states[t - 1], states[t], observations, t, output);
        ...
        {
            double[] featureVector = new double[this.FactorParameters.Count];

            for (int i = 0, k = this.FactorParameters.Offset; i < featureVector.Length; i++, k++)
                featureVector[i] = this.Owner.Features[k].Compute(states, observations, output);
            ...
            T[] observations, int index, int outputClass = 0)
        {
            ...
            int start = this.FactorParameters.Offset;
            int end = this.FactorParameters.Offset + this.FactorParameters.Count;

            double sum = 0;

            for (int k = start; k < end; k++)
            {
                double weight = this.Owner.Weights[k];

                if (Double.IsNaN(weight))
                    this.Owner.Weights[k] = weight = 0;

                if (weight != 0)
                {
                    IFeature<T> feature = this.Owner.Features[k];
        ...
        public IEnumerator<IFeature<T>> GetEnumerator()
        {
            ...
            for (int k = start; k < end; k++)
                yield return this.Owner.Features[k];
