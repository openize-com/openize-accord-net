...
#pragma warning disable 612, 618
        ...
        : PotentialFunctionBase<double[]>, IPotentialFunction<double[]>
    {
        ...
        public MarkovMultivariateFunction(HiddenMarkovClassifier<MultivariateNormalDistribution> classifier, bool includePriors = true)
        {
            ...
            this.Factors = new FactorPotential<double[]>[this.Outputs];
            ...
            Debug.Assert(factorIndex == factorParams.Count);
            Debug.Assert(factorIndex == factorFeatures.Count);
            ...
            {
                this.Factors[c] = new MarkovMultivariateNormalFactor(this, classifier.Models[c].States, c, this.Dimensions,
        ...
        public MarkovMultivariateFunction(HiddenMarkovModel<MultivariateNormalDistribution> model)
        {
            ...
            this.Factors = new[] // First features and parameters are always belonging to edges
            {
                new MarkovMultivariateNormalFactor(this, model.States, 0, this.Dimensions,
        ...
        public MarkovMultivariateFunction(HiddenMarkovClassifier<Independent> classifier, bool includePriors = true)
        {
            ...
            for (int c = 0; c < classifier.Models.Length; c++)
            {
                this.Factors[c] = new MarkovIndependentFactor(this, classifier.Models[c].States, c, lookupTables[c],
                    classIndex: classOffset[c], classCount: classCount[c],  // 1. classes
                    edgeIndex: edgeOffset[c], edgeCount: edgeCount[c],      // 2. edges
                    stateIndex: stateOffset[c], stateCount: stateCount[c]); // 3. states
            }
        ...
        public object Clone()
        {
            ...
            clone.Factors = new FactorPotential<double[]>[this.Factors.Length];
            for (int i = 0; i < this.Factors.Length; i++)
                clone.Factors[i] = this.Factors[i].Clone(newOwner: clone);

            clone.Features = new IFeature<double[]>[this.Features.Length];
            for (int i = 0; i < this.Features.Length; i++)
                clone.Features[i] = this.Features[i].Clone(newOwner: clone);
