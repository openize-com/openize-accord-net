...
namespace FileFormat.Accord.Statistics.Models.Fields.Features
{
    ...
    public sealed class OccupancyFeature<T> : FeatureBase<T>, IFeature<T>
    {
        ...
        {
            if (y != this.FactorIndex)
                return 0;

            double marginal = 0;
            for (int t = 0; t < x.Length; t++)
            {
                marginal += fwd[t, this.current] * bwd[t, this.current]; ;
            }
        ...
        public override double LogMarginal(double[,] lnFwd, double[,] lnBwd, T[] x, int y)
        {
            ...
            for (int t = 0; t < x.Length; t++)
                marginal = Special.LogSum(marginal, lnFwd[t, this.current] + lnBwd[t, this.current]);
        ...
        {
            var clone = (OccupancyFeature<T>)this.MemberwiseClone();
