    ...
    public sealed class MultivariateEmissionFeature : FeatureBase<double[]>, IFeature<double[]>
    {
            ...
            int observationIndex, int outputClass = 0)
        {
            if (currentState == this.state)
            {
                ...
                {
                    if (observations[observationIndex][this.dimension] == this.symbol)
                        return 1.0;
                }
            }
        ...
        public override double Marginal(double[,] fwd, double[,] bwd, double[][] x, int y)
        {
            ...
            {
                if (x[t][this.dimension] == this.symbol)
                    marginal += fwd[t, this.state] * bwd[t, this.state];
            }
        ...
        public override double LogMarginal(double[,] lnFwd, double[,] lnBwd, double[][] x, int y)
        {
            ...
            {
                if (x[t][this.dimension] == this.symbol)
                    marginal = Special.LogSum(marginal, lnFwd[t, this.state] + lnBwd[t, this.state]);
            }
        ...
        {
            var clone = (MultivariateEmissionFeature)this.MemberwiseClone();
