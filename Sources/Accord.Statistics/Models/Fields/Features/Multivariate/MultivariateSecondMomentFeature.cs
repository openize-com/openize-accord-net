...
namespace FileFormat.Accord.Statistics.Models.Fields.Features.Multivariate
{
    using System;
    using Base;
    ...
    public sealed class MultivariateSecondMomentFeature : FeatureBase<double[]>, IFeature<double[]>
    {
            ...
            int observationIndex, int outputClass = 0)
        {
            if (currentState == this.state)
            {
                ...
                {
                    double x = observations[observationIndex][this.dimension];
                    return x * x;
                }
            }
        ...
        public override double Marginal(double[,] fwd, double[,] bwd, double[][] x, int y)
        {
            ...
            for (int t = 0; t < x.Length; t++)
                marginal += fwd[t, this.state] * bwd[t, this.state] * (x[t][this.dimension] * x[t][this.dimension]);
        ...
        public override double LogMarginal(double[,] lnFwd, double[,] lnBwd, double[][] x, int y)
        {
            ...
            for (int t = 0; t < x.Length; t++)
                marginal = Special.LogSum(marginal, lnFwd[t, this.state] + lnBwd[t, this.state] + Math.Log(x[t][this.dimension] * x[t][this.dimension]));
        ...
        {
            var clone = (MultivariateSecondMomentFeature)this.MemberwiseClone();
