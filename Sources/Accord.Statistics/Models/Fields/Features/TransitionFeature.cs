...
namespace FileFormat.Accord.Statistics.Models.Fields.Features
{
    using System;
    using Base;
    ...
    public sealed class TransitionFeature<T> : FeatureBase<T>, IFeature<T>
    {
        ...
        public override double Marginal(double[,] fwd, double[,] bwd, T[] x, int y)
        {
            ...
            FactorPotential<T> owner = this.Owner.Factors[this.FactorIndex];

            double marginal = 0;
            for (int t = 0; t < x.Length - 1; t++)
            {
                marginal += fwd[t, this.previous] * bwd[t + 1, this.current]
                                                  * Math.Exp(owner.Compute(this.previous, this.current, x, t + 1, y));
            }
        ...
        public override double LogMarginal(double[,] lnFwd, double[,] lnBwd, T[] x, int y)
        {
            ...
            for (int t = 0; t < x.Length - 1; t++)
            {
                marginal = Special.LogSum(marginal, lnFwd[t, this.previous] + lnBwd[t + 1, this.current]
                                                                            + owner.Compute(this.previous, this.current, x, t + 1, y));
            }
        ...
        {
            var clone = (TransitionFeature<T>)this.MemberwiseClone();
