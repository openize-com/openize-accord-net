...
namespace FileFormat.Accord.Statistics.Models.Fields.Learning.Visible
{
        ...
        IConditionalRandomFieldLearning<T>, IConvergenceLearning
    {
        ...
        public bool HasConverged
        {
            get
            {
                return this.lbfgs.Status == BoundedBroydenFletcherGoldfarbShannoStatus.FunctionConvergence
                    || this.lbfgs.Status == BoundedBroydenFletcherGoldfarbShannoStatus.GradientConvergence;
            }
        }
        ...
        private void init()
        {
            this.lbfgs = new BoundedBroydenFletcherGoldfarbShanno(this.Model.Function.Weights.Length);
            this.lbfgs.FunctionTolerance = this.Tolerance;
            this.lbfgs.MaxIterations = this.MaxIterations;

            for (int i = 0; i < this.lbfgs.UpperBounds.Length; i++)
            {
                this.lbfgs.UpperBounds[i] = 1e10;
                this.lbfgs.LowerBounds[i] = -1e100;
            }
        }
        ...
        {
            return this.run(observations, labels);
        }

        private double run(T[][] observations, int[][] labels)
        {
            int n = observations.Length;
            int numberOfParameters = this.Model.Function.Weights.Length;
            ...
            this.lbfgs.Function = parameters =>
            {
                model.Function.Weights = parameters;
                return -model.LogLikelihood(observations, labels);
            };

            this.lbfgs.Gradient = parameters =>
            {
                model.Function.Weights = parameters;
                this.gradient(observations, labels, g);
                return g;
            };

            this.lbfgs.Token = this.Token;

            if (this.lbfgs.Minimize(model.Function.Weights))
                model.Function.Weights = this.lbfgs.Solution;
        ...
        private double[] gradient(T[][] observations, int[][] labels, double[] g)
        {
            ...
            int n = observations.Length;
            int d = this.Model.Function.Weights.Length;
            ...
            Parallel.For(0, observations.Length, this.ParallelOptions,
                ...
                (i, state, local) =>
                {
                    ...
                    Parallel.For(0, g.Length, this.ParallelOptions, k =>
                    {
        ...
        public ConditionalRandomField<T> Learn(T[][] x, int[][] y, double[] weights = null)
        {
            ...
            if (this.Model == null)
            {
                this.Model = new ConditionalRandomField<T>(states: y.Max() + 1, function: this.Function);
                this.init();
            }
