...
namespace Openize.Accord.Statistics.Models.Fields
{
    ...
    public class HiddenConditionalRandomField<T> : MulticlassClassifierBase<T[]>, ICloneable
    {
        ...
        public int Outputs { get { return this.Function.Outputs; } }
        ...
        public IPotentialFunction<T> Function
        {
            ...
            {
                this.function = value;
                this.NumberOfOutputs = this.function.Outputs;
                this.NumberOfClasses = this.function.Outputs;
            }
        }
        ...
        public int Compute(T[] observations)
        {
            ...
            return this.compute(observations, out logLikelihoods);
        }
        ...
        private int compute(T[] observations, out double[] logLikelihoods)
        {
            ...
            logLikelihoods = this.LogLikelihood(observations);
        ...
        {
            double[] logLikelihoods;
            int i = this.compute(observations, out logLikelihoods);
        ...
        public int[] Decode(T[] observations, int output)
        {
            ...
            return this.Decode(observations, output, out logLikelihood);
        }
        ...
        public int[] Decode(T[] observations, int output, out double logLikelihood)
        {
            ...
            var factor = this.Function.Factors[output];
        ...
        {
            double[] logLikelihoods;
            return this.LogLikelihood(observations, output, out logLikelihoods);
        }
        ...
        public double LogLikelihood(T[] observations, int output, out double[] logLikelihoods)
        {
            ...
            Debug.Assert(!Double.IsNaN(logZx));
            Debug.Assert(!Double.IsNaN(logZxy));
            Debug.Assert(!Double.IsNaN(logLikelihoods[output]));
        ...
        public double LogLikelihood(T[][] observations, int[] output)
        {
            ...
            for (int i = 0; i < observations.Length; i++)
                sum += this.LogLikelihood(observations[i], output[i], out logLikelihoods);
        ...
        public double LogLikelihood(T[][] observations, int[] output, out double[][] logLikelihoods)
        {
            ...
            for (int i = 0; i < observations.Length; i++)
                sum += this.LogLikelihood(observations[i], output[i], out logLikelihoods[i]);
        ...
        public double Partition(T[] x, int y)
        {
            ...
            ForwardBackwardAlgorithm.Forward(this.Function.Factors[y], x, y, out logLikelihood);
        ...
        public double Partition(T[] x)
        {
            ...
            for (int j = 0; j < this.NumberOfOutputs; j++)
            {
                ...
                ForwardBackwardAlgorithm.Forward(this.Function.Factors[j], x, j, out logLikelihood);
        ...
        public double[][] LogLikelihood(T[][] observations)
        {
            ...
            for (int i = 0; i < result.Length; i++)
                result[i] = this.LogLikelihood(observations[i]);
            return result;
        }
        ...
        public double[] LogLikelihood(T[] observations)
        {
            double[] logLikelihoods = new double[this.NumberOfOutputs];
...
#endif
            {
                ...
                ForwardBackwardAlgorithm.LogForward(this.Function.Factors[y],
...
#region ICloneable Members
        ...
        public object Clone()
        {
            return new HiddenConditionalRandomField<T>((IPotentialFunction<T>)this.Function.Clone());
        }
        ...
        {
            double[] v;
            return this.compute(input, out v);
        }
    }
}
