...
namespace Openize.Accord.Statistics.Accord.MachineLearning
{
        ...
        ISupervisedLearning<MinimumMeanDistanceClassifier, double[], int>
    {
        ...
        public double[][] Means
        {
            get { return this.means; }
            set { this.means = value; }
        }
        ...
        {
            distances = new double[this.means.Length];
            for (int i = 0; i < distances.Length; i++)
                distances[i] = this.distance.Distance(input, this.means[i]);
        ...
        {
            double[] distances;
            return this.Compute(input, out distances);
        }
        ...
        public override double[] Scores(double[] input, double[] result)
        {
            for (int i = 0; i < this.means.Length; i++)
                result[i] = -this.distance.Distance(input, this.means[i]);
            return result;
        }
        ...
        {
            return -this.distance.Distance(input, this.means[classIndex]);
        }
        ...
        public MinimumMeanDistanceClassifier Learn(double[][] x, int[] y, double[] weights = null)
        {
            ...
            this.NumberOfOutputs = y.Max() + 1;
            this.NumberOfClasses = y.Max() + 1;
            this.means = Jagged.Zeros(this.NumberOfOutputs, this.NumberOfInputs);

            int[] counts = new int[this.NumberOfOutputs];
            ...
            this.means.Divide(counts, dimension: (VectorType)1, result: this.means);
