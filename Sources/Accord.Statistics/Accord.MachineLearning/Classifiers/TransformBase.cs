...
namespace Openize.Accord.Statistics.Accord.MachineLearning.Classifiers
{
    ...
    public abstract class MultipleTransformBase<TInput, TOutput> : TransformBase<TInput, TOutput[]>
    {
        ...
        {
            return this.Transform(input, new TOutput[this.NumberOfOutputs]);
        }
        ...
        public virtual TOutput[] Transform(TInput input, TOutput[] result)
        {
            return this.Transform(new[] { input }, new[] { result })[0];
        }
        ...
        public override TOutput[][] Transform(TInput[] input)
        {
            return this.Transform(input, Jagged.Create<TOutput>(input.Length, this.NumberOfOutputs));
        }
        ...
        {
            for (int i = 0; i < input.Length; i++)
                this.Transform(input[i], result[i]);
            return result;
        }
    }
