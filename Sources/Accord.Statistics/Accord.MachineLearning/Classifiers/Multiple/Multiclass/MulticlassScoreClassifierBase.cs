...
namespace FileFormat.Accord.Statistics.Accord.MachineLearning.Classifiers.Multiple.Multiclass
{
        ...
        IMulticlassScoreClassifier<TInput>
    {
        ...
        {
            double max = this.Score(input, classIndex: 0);
            int imax = 0;
            for (int i = 1; i < this.NumberOfClasses; i++)
            {
                double d = this.Score(input, classIndex: i);
                if (d > max)
                {
                    max = d;
                    imax = i;
        ...
        {
            return this.Score(input, classIndex, new double[input.Length]);
        }
        ...
        {
            double[] temp = new double[this.NumberOfClasses];
            for (int i = 0; i < input.Length; i++)
                result[i] = this.Scores(input[i], temp)[classIndex[i]];
            return result;
        }
        ...
        public double[] Score(TInput[] input, int classIndex, double[] result)
        {
            double[] temp = new double[this.NumberOfClasses];
            for (int i = 0; i < input.Length; i++)
                result[i] = this.Scores(input[i], temp)[classIndex];
            return result;
        }
        ...
        {
            int value;
            var result = this.Scores(input, out value);
            return result[value];
        }
        ...
        {
            double[] result = this.Scores(input, out decision);
        ...
        {
            int value;
            double[] result = this.Scores(input, out value);
            decision = value;
            return result[value];
        }
        ...
        {
            int value;
            this.Scores(input, out value, result);
            decision = value;
            return result;
        }
        ...
        public double[] Score(TInput[] input, ref int[] decision, double[] result)
        {
            decision = this.createOrReuse(input, decision);
            for (int i = 0; i < input.Length; i++)
                result[i] = this.Score(input[i], out decision[i]);
            return result;
        }
        ...
        public double[][] Scores(TInput[] input, ref int[] decision, double[][] result)
        {
            decision = this.createOrReuse(input, decision);
            for (int i = 0; i < input.Length; i++)
                this.Scores(input[i], out decision[i], result[i]);
            return result;
        }
        ...
        double[][] IMultilabelScoreClassifierBase<TInput, bool[]>.Scores(TInput[] input, ref bool[][] decision, double[][] result)
        {
            decision = this.createOrReuse(input, decision);
            for (int i = 0; i < input.Length; i++)
                this.ToMultilabel().Scores(input[i], ref decision[i], result[i]);
            return result;
        }
        ...
        double[][] IMultilabelScoreClassifierBase<TInput, int[]>.Scores(TInput[] input, ref int[][] decision, double[][] result)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                this.Scores(input[i], out value, result[i]);
