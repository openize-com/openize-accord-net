...
namespace Openize.Accord.Statistics.Accord.MachineLearning.Classifiers.Multiple.Multiclass
{
        ...
        IMulticlassClassifier<TInput>
    {
        ...
        bool[][] IClassifier<TInput, bool[]>.Decide(TInput[] input)
        {
            return this.ToMultilabel().Decide(input, this.create<bool>(input));
        }

        
        double[][] IClassifier<TInput, double[]>.Decide(TInput[] input)
        {
            return this.ToMultilabel().Decide(input, this.create<double>(input));
        }

        
        int[][] IClassifier<TInput, int[]>.Decide(TInput[] input)
        {
            return this.ToMultilabel().Decide(input, this.create<int>(input));
        }
        ...
        virtual public bool[][] Transform(TInput[] input, bool[][] result)
        {
            return this.ToMultilabel().Decide(input, result);
        }
