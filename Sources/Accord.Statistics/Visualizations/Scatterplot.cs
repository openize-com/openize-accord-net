...
namespace Openize.Accord.Statistics.Visualizations
{
    ...
    public class Scatterplot
    {
        ...
        private void initialize(double[] x, double[] y, int[] z)
        {
            ...
            if (z == null)
            {
                this.LabelValues = new int[] { };
            }
            else
            {
                this.LabelValues = this.LabelAxis.Distinct();
            }

            this.LabelNames = new string[this.LabelValues.Length];
            for (int i = 0; i < this.LabelNames.Length; i++)
                this.LabelNames[i] = i.ToString();

            var classes = new ScatterplotClassValueCollection[this.LabelValues.Length];
            for (int i = 0; i < classes.Length; i++)
                classes[i] = new ScatterplotClassValueCollection(this, i);
            this.Classes = new ReadOnlyCollection<ScatterplotClassValueCollection>(classes);
        }
        ...
        public void Compute(double[] values)
        {
            ...
            this.initialize(idx, values, null);
        }
