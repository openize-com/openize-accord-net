...
#pragma warning disable 612, 618
        ...
        where TDistribution : IDistribution
    {
        ...
        public void Push(params double[] value)
        {
            if (!this.Started)
            {
                for (int i = 0; i < this.Current.Length; i++)
                    this.Current[i] = this.Model.LogInitial[i] + this.Model.Emissions[i].LogProbabilityFunction(value);
                this.Started = true;
            }
            else
            {
                ...
                for (int i = 0; i < this.Current.Length; i++)
                {
                    ...
                    for (int j = 0; j < this.previous.Length; j++)
                        sum = Special.LogSum(sum, this.previous[j] + this.Model.Transitions[j, i]);
                    this.Current[i] = sum + this.Model.Emissions[i].LogProbabilityFunction(value);
                }
            }
        ...
        public double Peek(double[] value)
        {
            ...
            {
                for (int i = 0; i < this.Current.Length; i++)
                    this.next[i] = this.Model.LogInitial[i] + this.Model.Emissions[i].LogProbabilityFunction(value);
            }
            else
            {
                for (int i = 0; i < this.Current.Length; i++)
                {
                    ...
                    this.next[i] = sum + this.Model.Emissions[i].LogProbabilityFunction(value);
                }
            }
