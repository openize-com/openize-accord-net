...
namespace Openize.Accord.Statistics.Testing
{
    ...
    public class TTest : HypothesisTest<TDistribution>
    {
        ...
        /// </summary>
        /// 
        public IPowerAnalysis Analysis { get { return this.powerAnalysis; } }
        ...
        public DoubleRange GetConfidenceInterval(double percent = 0.95)
        {
            ...
            return new DoubleRange(
                this.EstimatedValue - u * this.StandardError,
                this.EstimatedValue + u * this.StandardError);
        }
            ...
            OneSampleHypothesis hypothesis = OneSampleHypothesis.ValueIsDifferentFromHypothesis)
        {
            ...
            this.Compute(statistic, degreesOfFreedom, hypothesis);
        }
        ...
        {
            this.Compute(estimatedValue, standardError, degreesOfFreedom, hypothesizedValue, alternate);
        }
            ...
            OneSampleHypothesis alternate = OneSampleHypothesis.ValueIsDifferentFromHypothesis)
        {
            ...
            this.Compute(n, mean, hypothesizedMean, stdError, alternate);
            ...
            OneSampleHypothesis alternate = OneSampleHypothesis.ValueIsDifferentFromHypothesis)
        {
            ...
            this.Compute(samples, mean, hypothesizedMean, stdError, alternate);

            this.power(stdDev, samples);
        }
        ...
        protected void Compute(int n, double mean, double hypothesizedMean, double stdError, OneSampleHypothesis hypothesis)
        {
            ...
            double df = n - 1;
            double t = (this.EstimatedValue - hypothesizedMean) / this.StandardError;

            this.Compute(t, df, hypothesis);
        }
            ...
            double hypothesizedValue, OneSampleHypothesis alternate)
        {
            ...
            double t = (this.EstimatedValue - hypothesizedValue) / this.StandardError;

            this.Compute(t, df, alternate);
        }

        private void power(double stdDev, int samples)
        {
            this.powerAnalysis = new TTestPowerAnalysis(this.Hypothesis)
            {
                Samples = samples,
                Effect = (this.EstimatedValue - this.HypothesizedValue) / stdDev,
                Size = this.Size,
            };
        ...
        {
            return PValueToStatistic(p, this.StatisticDistribution, this.Tail);
        }
        ...
        public override double StatisticToPValue(double x)
        {
            ...
            return StatisticToPValue(x, this.StatisticDistribution, this.Tail);
        }
