...
namespace FileFormat.Accord.Statistics.Testing.Contingency
{
    ...
    public class KappaTest : ZTest
    {
        ...
        {
            this.Variance = standardError * standardError;

            this.Compute(sampleKappa, hypothesizedKappa, standardError, alternate);
        }
            ...
            OneSampleHypothesis alternate = OneSampleHypothesis.ValueIsDifferentFromHypothesis)
        {
            if (hypothesizedKappa == 0)
            {
                ...
                this.Compute(matrix.Kappa, hypothesizedKappa, matrix.StandardErrorUnderNull, alternate);
                this.Variance = matrix.VarianceUnderNull;
            }
            else
            {
                // Use the default variance
                this.Compute(matrix.Kappa, hypothesizedKappa, matrix.StandardError, alternate);
            ...
            OneSampleHypothesis alternate = OneSampleHypothesis.ValueIsDifferentFromHypothesis)
        {
            ...
            {
                // Use the null hypothesis variance
                this.Compute(matrix.WeightedKappa, hypothesizedWeightedKappa, matrix.WeightedStandardErrorUnderNull, alternate);
                this.Variance = matrix.WeightedVarianceUnderNull;
            }
            else
            {
                // Use the default variance
                this.Compute(matrix.WeightedKappa, hypothesizedWeightedKappa, matrix.WeightedStandardError, alternate);
                this.Variance = matrix.WeightedVariance;
            }
        }
            ...
            bool nullHypothesis = false)
        {
            ...
            Debug.Assert(!(Math.Abs(variance - stdDev * stdDev) > 1e-10 * variance));
