...
namespace FileFormat.Accord.Statistics.Testing.Power
{
    using System;
    using Base;
    ...
    public class ZTestPowerAnalysis : BaseOneSamplePowerAnalysis
    {
        ...
        public override void ComputePower()
        {
            switch (this.Tail)
            {
                case DistributionTail.TwoTail:
                    {
                        double Za = NormalDistribution.Standard.InverseDistributionFunction(1.0 - this.Size / 2.0);
                        double Zb = Za - Math.Sqrt(this.Samples) * this.Effect;

                        this.Power = NormalDistribution.Standard.ComplementaryDistributionFunction(Zb);
                        break;
                    }

                case DistributionTail.OneLower:
                    {
                        double Za = NormalDistribution.Standard.InverseDistributionFunction(this.Size);
                        double Zb = Za - Math.Sqrt(this.Samples) * this.Effect;
                ...
                case DistributionTail.OneUpper:
                    {
                        double Za = NormalDistribution.Standard.InverseDistributionFunction(1.0 - this.Size);
                        double Zb = Za - Math.Sqrt(this.Samples) * this.Effect;
        ...
        {
            double Za = ZTest.PValueToStatistic(this.Size, this.Tail);
            double Zb = NormalDistribution.Standard
                .InverseDistributionFunction(this.Power);

            double n = (Za + Zb) / this.Effect;
        ...
        public override void ComputeEffect()
        {
            ...
            double n = Math.Sqrt(this.Samples);
            double d = (Za + Zb) / n;
