...
namespace FileFormat.Accord.Statistics.Testing.Multiple_Samples
{
    ...
    public class TwoWayAnova : IAnova
    {
        ...
        {
            this.FirstFactorSamples = firstFactorLabels.Max() + 1;
            this.SecondFactorSamples = secondFactorLabels.Max() + 1;

            List<double>[][] groups = new List<double>[this.FirstFactorSamples][];
            for (int i = 0; i < groups.Length; i++)
            {
                groups[i] = new List<double>[this.SecondFactorSamples];
            ...
            double[][][] sets = new double[this.FirstFactorSamples][][];
            for (int i = 0; i < sets.Length; i++)
            {
                sets[i] = new double[this.SecondFactorSamples][];
                for (int j = 0; j < sets[i].Length; j++)
                    sets[i][j] = groups[i][j].ToArray();
            }

            this.Replications = sets[0][0].Length;

            // Assert equal number of replicates
            for (int i = 0; i < sets.Length; i++)
                for (int j = 0; j < sets[i].Length; j++)
                    if (sets[i][j].Length != this.Replications)
                        throw new ArgumentException("Samples do not have the same number of replicates.", "samples");

            this.initialize(sets, type);
        }
        ...
        public TwoWayAnova(double[][][] samples, TwoWayAnovaModel type = TwoWayAnovaModel.Mixed)
        {
            this.FirstFactorSamples = samples.Length;
            this.SecondFactorSamples = samples[0].Length;
            this.Replications = samples[0][0].Length;
            this.Observations = this.FirstFactorSamples * this.SecondFactorSamples * this.Replications;

            // Assert equal number of replicates
            for (int i = 0; i < samples.Length; i++)
                for (int j = 0; j < samples[i].Length; j++)
                    if (samples[i][j].Length != this.Replications)
                        throw new ArgumentException("Samples do not have the same number of replicates.", "samples");

            this.initialize(samples, type);
        }
        ...
        private void initialize(double[][][] samples, TwoWayAnovaModel type)
        {
            ...
            int cellDegreesOfFreedom = this.FirstFactorSamples * this.SecondFactorSamples - 1;
            int aDegreesOfFreedom = this.FirstFactorSamples - 1;
            int bDegreesOfFreedom = this.SecondFactorSamples - 1;
            int abDegreesOfFreedom = cellDegreesOfFreedom - aDegreesOfFreedom - bDegreesOfFreedom;
            int errorDegreesOfFreedom = this.FirstFactorSamples * this.SecondFactorSamples * (this.Replications - 1);
            int totalDegreesOfFreedom = this.Observations - 1;


            // Step 1. Calculate cell means
            this.cellMeans = new double[this.FirstFactorSamples, this.SecondFactorSamples];

            double sum = 0;
            for (int i = 0; i < samples.Length; i++)
                for (int j = 0; j < samples[i].Length; j++)
                    sum += this.cellMeans[i, j] = Measures.Mean(samples[i][j]);


            // Step 2. Calculate the total mean (grand mean)
            this.totalMean = sum / (this.FirstFactorSamples * this.SecondFactorSamples);


            // Step 3. Calculate factor means
            this.aMean = new double[this.FirstFactorSamples];
            for (int i = 0; i < samples.Length; i++)
            {
                sum = 0;
                for (int j = 0; j < samples[i].Length; j++)
                ...
                this.aMean[i] = sum / (this.SecondFactorSamples * this.Replications);
            }

            this.bMean = new double[this.SecondFactorSamples];
            for (int j = 0; j < samples[0].Length; j++)
            {
                ...
                this.bMean[j] = sum / (this.FirstFactorSamples * this.Replications);
            }
            ...
            for (int i = 0; i < samples.Length; i++)
            {
                for (int j = 0; j < samples[i].Length; j++)
                {
                    for (int k = 0; k < samples[i][j].Length; k++)
                    {
                        double u = samples[i][j][k] - this.totalMean;
                        ssum += u * u;
                    }
                }
            }
            ...
            ssum = 0;
            for (int i = 0; i < this.FirstFactorSamples; i++)
            {
                for (int j = 0; j < this.SecondFactorSamples; j++)
                {
                    double u = this.cellMeans[i, j] - this.totalMean;
                    ssum += u * u;
                }
            }
            double cellSumOfSquares = ssum * this.Replications;
            ...
            for (int i = 0; i < samples.Length; i++)
            {
                for (int j = 0; j < samples[i].Length; j++)
                {
                    for (int k = 0; k < samples[i][j].Length; k++)
                    {
                        double u = samples[i][j][k] - this.cellMeans[i, j];
                        ssum += u * u;
                    }
                }
            }
            ...
            ssum = 0;
            for (int i = 0; i < this.aMean.Length; i++)
            {
                double u = this.aMean[i] - this.totalMean;
                ssum += u * u;
            }
            double aSumOfSquares = ssum * this.SecondFactorSamples * this.Replications;

            ssum = 0;
            for (int i = 0; i < this.bMean.Length; i++)
            {
                double u = this.bMean[i] - this.totalMean;
                ssum += u * u;
            }
            double bSumOfSquares = ssum * this.FirstFactorSamples * this.Replications;
