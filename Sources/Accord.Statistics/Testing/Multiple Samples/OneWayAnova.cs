...
namespace Openize.Accord.Statistics.Testing.Multiple_Samples
{
    ...
    public class OneWayAnova : IAnova
    {
        ...
        {
            this.totalSize = samples.Length;
            this.groupCount = labels.Max();

            this.sizes = new int[this.groupCount];

            double[][] groups = new double[this.groupCount][];
        ...
        {
            this.sizes = new int[samples.Length];

            this.groupCount = samples.Length;
            for (int i = 0; i < samples.Length; i++)
                this.totalSize += this.sizes[i] = samples[i].Length;
        ...
        {
            this.DFb = this.groupCount - 1;
            this.DFw = this.totalSize - this.groupCount;
            this.DFt = this.totalSize - 1;

            // Step 1. Calculate the mean within each group
            this.means = Measures.Mean(samples, 1);

            // Step 2. Calculate the overall mean
            this.totalMean = Measures.GrandMean(this.means, this.sizes);
            ...
            for (int i = 0; i < samples.Length; i++)
            {
                ...
                double u = (this.means[i] - this.totalMean);
                this.SSb += this.sizes[i] * u * u;
            }
            ...
            for (int i = 0; i < samples.Length; i++)
            {
                ...
                {
                    double u = samples[i][j] - this.means[i];
                    this.SSw += u * u;
                }
            }

            this.SSt = this.SSb + this.SSw; // total sum of squares


            // Step 5. Calculate the F statistic
            this.MSb = this.SSb / this.DFb; // between-group mean square
            this.MSw = this.SSw / this.DFw; // within-group mean square
            this.FTest = new FTest(this.MSb / this.MSw, this.DFb, this.DFw);


            // Step 6. Create the ANOVA table
            List<AnovaVariationSource> table = new List<AnovaVariationSource>();
            table.Add(new AnovaVariationSource(this, "Between-Groups", this.SSb, this.DFb, this.FTest));
            table.Add(new AnovaVariationSource(this, "Within-Groups", this.SSw, this.DFw, null));
            table.Add(new AnovaVariationSource(this, "Total", this.SSt, this.DFt, null));
