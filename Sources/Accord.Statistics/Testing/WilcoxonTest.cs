...
namespace Openize.Accord.Statistics.Testing
{
    ...
    public class WilcoxonTest : HypothesisTest<WilcoxonDistribution>
    {
        ...
        /// </summary>
        /// 
        public int NumberOfSamples { get { return this.StatisticDistribution.NumberOfSamples; } }
        ...
        {
            this.Compute(signs, diffs, tail, null, adjustForTies: true);
        }
            ...
            bool? exact, bool adjustForTies)
        {
            ...
            this.Signs = signs;
            this.Delta = diffs;
            this.Ranks = this.Delta.Rank(hasTies: out this.hasTies, adjustForTies: adjustForTies);

            double W = WilcoxonDistribution.WPositive(this.Signs, this.Ranks);

            this.Compute(W, this.Ranks, tail, exact);
        }
        ...
        protected void Compute(double statistic, double[] ranks, DistributionTail tail, bool? exact)
        {
            ...
            if (ranks.Length != 0)
            {
                ...
                {
                    Correction = (this.Tail == DistributionTail.TwoTail) ? ContinuityCorrection.Midpoint : ContinuityCorrection.KeepInside
                };
        ...
        {
            if (this.StatisticDistribution == null)
        ...
        public override double PValueToStatistic(double p)
        {
            ...
            {
                case DistributionTail.OneLower:
                    b = this.StatisticDistribution.InverseDistributionFunction(p);
                    break;
                case DistributionTail.OneUpper:
                    b = this.StatisticDistribution.InverseDistributionFunction(1.0 - p);
                    break;
                case DistributionTail.TwoTail:
                    b = this.StatisticDistribution.InverseDistributionFunction(1.0 - p / 2.0);
                    break;
                default:
                    throw new InvalidOperationException();
            }
