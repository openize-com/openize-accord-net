...
namespace FileFormat.Accord.Statistics.Testing.TwoSample
{
    ...
    public class TwoSampleTTest : HypothesisTest<TDistribution>
    {
        ...
        /// </summary>
        /// 
        public ITwoSamplePowerAnalysis Analysis { get { return this.powerAnalysis; } }
        ...
        /// </summary>
        /// 
        public double DegreesOfFreedom { get { return this.StatisticDistribution.DegreesOfFreedom; } }
        ...
        {
            double u = this.PValueToStatistic(1.0 - percent);

            return new DoubleRange(
                this.ObservedDifference - u * this.StandardError,
                this.ObservedDifference + u * this.StandardError);
        }
            ...
            TwoSampleHypothesis alternate = TwoSampleHypothesis.ValuesAreDifferent)
        {
            ...
            this.Compute(x1, s1, n1, x2, s2, n2,
                hypothesizedDifference, assumeEqualVariances, alternate);

            this.power(s1, s2, sample1.Length, sample2.Length);
        }
            ...
            TwoSampleHypothesis alternate = TwoSampleHypothesis.ValuesAreDifferent)
        {
            ...
            this.Compute(mean1, var1, samples1, mean2, var2, samples2,
                hypothesizedDifference, assumeEqualVariances, alternate);
            ...
            TwoSampleHypothesis alternate)
        {
            ...
            if (this.AssumeEqualVariance)
            {
                // Assume the two samples share the same underlying population variance.
                this.Variance = ((n1 - 1) * s1 + (n2 - 1) * s2) / (n1 + n2 - 2);
                this.StandardError = Math.Sqrt(this.Variance) * Math.Sqrt(1.0 / n1 + 1.0 / n2);
                df = n1 + n2 - 2;
            }
            else
            {
                // Assume samples have unequal variances.
                this.StandardError = Math.Sqrt(s1 / n1 + s2 / n2);
                this.Variance = (s1 + s2) / 2.0;
            ...
            this.Statistic = (this.ObservedDifference - this.HypothesizedDifference) / this.StandardError;
        ...
        private void power(double var1, double var2, int n1, int n2)
        {
            ...
            this.powerAnalysis = new TwoSampleTTestPowerAnalysis(this.Hypothesis)
            {
                Samples1 = n1,
                Samples2 = n2,
                Effect = (this.ObservedDifference - this.HypothesizedDifference) / stdDev,
                Size = this.Size,
            };

            this.powerAnalysis.ComputePower();
        }

        /// <summary>Update event.</summary>
        protected override void OnSizeChanged()
        {
            this.Confidence = this.GetConfidenceInterval(1.0 - this.Size);
            if (this.Analysis != null)
            {
                this.powerAnalysis.Size = this.Size;
                this.powerAnalysis.ComputePower();
            }
        }
        ...
        {
            return TTest.PValueToStatistic(p, this.StatisticDistribution, this.Tail);
        }
        ...
        {
            return TTest.StatisticToPValue(x, this.StatisticDistribution, this.Tail);
        }
