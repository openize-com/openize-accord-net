...
namespace FileFormat.Accord.Statistics.Testing.TwoSample
{
    ...
    public class MannWhitneyWilcoxonTest : HypothesisTest<MannWhitneyDistribution>
    {
        ...
        /// </summary>
        /// 
        public bool HasTies { get { return this.hasTies; } }
            ...
            bool? exact = null, bool adjustForTies = true)
        {
            ...
            double[] rank = samples.Rank(hasTies: out this.hasTies, adjustForTies: adjustForTies);

            // Split the rankings back and sum
            this.Rank1 = rank.Get(0, this.NumberOfSamples1);
            this.Rank2 = rank.Get(this.NumberOfSamples1, 0);

            // Compute rank sum statistic
            this.RankSum1 = this.Rank1.Sum();
            this.RankSum2 = this.Rank2.Sum();

            if (this.hasTies && exact == true)
            {
            ...
            this.Statistic1 = this.RankSum1 - ((double)this.NumberOfSamples1 * (this.NumberOfSamples1 + 1)) / 2.0; // U1
            this.Statistic2 = this.RankSum2 - ((double)this.NumberOfSamples2 * (this.NumberOfSamples2 + 1)) / 2.0; // U2
            ...
            Debug.Assert(this.Statistic1 == MannWhitneyDistribution.MannWhitneyU(this.Rank1));
            Debug.Assert(this.Statistic2 == MannWhitneyDistribution.MannWhitneyU(this.Rank2));
            ...
            this.Statistic = (this.NumberOfSamples1 < this.NumberOfSamples2) ? this.Statistic1 : this.Statistic2;
            ...
            {
                this.Statistic = this.Statistic1;
                this.StatisticDistribution = new MannWhitneyDistribution(this.Rank1, this.Rank2, exact)
                {
                    Correction = (this.Tail == DistributionTail.TwoTail) ?
                        ContinuityCorrection.Midpoint : ContinuityCorrection.KeepInside
                };
            }
            else
            {
                this.Statistic = this.Statistic2;
                this.StatisticDistribution = new MannWhitneyDistribution(this.Rank2, this.Rank1, exact)
                {
                    Correction = (this.Tail == DistributionTail.TwoTail) ?
                        ContinuityCorrection.Midpoint : ContinuityCorrection.KeepInside
                };
        ...
        public override double StatisticToPValue(double x)
        {
            ...
            switch (this.Tail)
            {
                case DistributionTail.TwoTail:
                    double a = this.StatisticDistribution.DistributionFunction(x);
                    double b = this.StatisticDistribution.ComplementaryDistributionFunction(x);
