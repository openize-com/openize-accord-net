        ...
        IAutoConfigurableFilter
    {
            ...
            foreach (DataColumn column in data.Columns)
            {
                if (column.DataType == typeof(Double) || column.DataType == typeof(Decimal))
                {
                    ...
                    if (!this.Columns.Contains(name))
                        this.Columns.Add(new Options(name));

                    this.Columns[name].Mean = mean;
                    this.Columns[name].StandardDeviation = sdev;
                }
            }
        }
        ...
        public void Detect(double[][] data)
        {
            ...
            for (int i = 0; i < cols; i++)
            {
                ...
                this.Columns[name].Tag = i;
                this.Columns[name].Mean = means[i];
                this.Columns[name].StandardDeviation = stdDev[i];
            }
        }
