...
namespace FileFormat.Accord.Statistics.Filters
{
    ...
    public partial class Codification<T>
    {
            ...
            IUnsupervisedLearning<Options, T, double[]>
        {
            ...
            {
                get { return this.hasMissingValue; }
                set { this.hasMissingValue = value; }
            }
            ...
            public object MissingValueReplacement
            {
                get
                {
                    if (this.missingValueReplacement == null)
                        return this.Owner.DefaultMissingValueReplacement;
                    return this.missingValueReplacement;
                }
                set { this.missingValueReplacement = value; }
            }
            ...
            public int Symbols { get { return this.NumberOfSymbols; } }
            ...
            public int NumberOfSymbols
            {
                get
                {
                    ...
                    throw new InvalidOperationException(String.Format("Unexpected VariableType {0}." +
                        " Execution should never have reached here.", this.VariableType));
                }
            }
            ...
            public T[] Values
            {
                get
                {
                    var values = new T[this.Mapping.Count];
                    for (int i = 0; i < values.Length; i++)
                        values[i] = this.Mapping.Reverse[i];
                    return values;
                }
            }
            ...
            {
                get { return this.NumberOfSymbols; }
            ...
            {
                int oldValue = this.Mapping[key];
                T oldKey = this.Mapping.Reverse[value];

                this.Mapping[key] = value;
                this.Mapping[oldKey] = oldValue;
            }
            ...
            {
                if (this.hasMissingValue && input.IsEqual(this.missingValue))
                    return -1;
#if !NETSTANDARD1_4
                this.Check();
#endif
            ...
            {
                this.Check();
                return this.Transform((T)input[this.ColumnName]);
            }

            private void Check()
            {
                if (this.VariableType == CodificationVariable.Continuous)
                    throw new InvalidOperationException("The variable is continuous and cannot be codified as an integer.");
                if (this.VariableType == CodificationVariable.Discrete)
            ...
            {
                if (this.hasMissingValue && output == -1)
                    return this.missingValue;
                return this.Mapping.Reverse[output];
            }
            ...
            public T[] Revert(int[] output)
            {
                return this.Revert(output, new T[output.Length]);
            }
            ...
            private bool CanBeCodified
            {
                get
                {
                    return this.VariableType == CodificationVariable.Categorical ||
                           this.VariableType == CodificationVariable.CategoricalWithBaseline ||
                           this.VariableType == CodificationVariable.Ordinal;
                }
            }
            ...
            public Options Learn(DataTable x, double[] weights = null)
            {
                ...
                {
                    // Do a select distinct to get distinct values
                    DataTable d = x.DefaultView.ToTable(true, this.ColumnName);
                    object[] unique = new object[d.Rows.Count];
                    for (int i = 0; i < d.Rows.Count; i++)
                        unique[i] = d.Rows[i][this.ColumnName];
#if USE_SORTED_ORDER
                    Array.Sort(unique);
#endif
                    return unique;
            ...
            public Options Learn(DataRow[] x, double[] weights = null)
            {
                ...
                {
                    // Do a select distinct to get distinct values
                    object[] unique = x.Apply(xi => xi[this.ColumnName]).Distinct();
#if USE_SORTED_ORDER
                    Array.Sort(unique);
#endif
                    return unique;
            ...
            private void TryAddValue(object value)
            {
...
#endif
                {
                    if (!this.hasMissingValue)
                    {
                        ...
                        this.missingValue = (T)System.Convert.ChangeType(null, typeof(T));
                    }
                }
                else
                {
                    ...
                    if (!this.Mapping.ContainsKey(key))
                        this.Mapping.Add(key, this.Mapping.Count); // And register the String->Integer mapping
                }
            }
            ...
            {
                return Vector.OneHot(this.Transform(input), columns: this.NumberOfClasses);
            }

            double[][] ICovariantTransform<T, double[]>.Transform(T[] input)
            {
                return this.Transform(input, Jagged.Zeros(input.Length, this.NumberOfClasses));
            }
            ...
            {
                return Jagged.OneHot(this.Transform(input), result: result);
            }
