...
namespace Openize.Accord.Statistics.Analysis.Performance
{
    ...
    public class ReceiverOperatingCharacteristic
    {
        ...
        public ReceiverOperatingCharacteristic(int[] expected, double[] actual)
        {
            ...
            this.init(expected.ToDouble(), actual);
        }
        ...
        private void init(double[] expected, double[] actual)
        {
            ...
            this.dtrue = this.dfalse = expected[0];
            for (int i = 1; i < expected.Length; i++)
            {
                if (this.dtrue < expected[i]) this.dtrue = expected[i];
                if (this.dfalse > expected[i]) this.dfalse = expected[i];
            }

            // Count the real number of positive and negative cases
            for (int i = 0; i < expected.Length; i++)
            {
                if (expected[i] == this.dtrue)
                    this.positiveCount++;
            }
        ...
        {
            get { return this.error * this.error; }
        }
        ...
        /// </summary>
        /// 
        public double[] PositiveResults { get { return this.positiveResults; } }
        ...
        /// </summary>
        /// 
        public double[] NegativeResults { get { return this.negativeResults; } }

        /// <summary>
        ///   Gets DeLong's pseudoaccuracies for the <see cref="PositiveResults">positive subjects</see>.
        /// </summary>
        /// 
        public double[] PositiveAccuracies { get { return this.positiveAccuracy; } }

        /// <summary>
        ///   Gets DeLong's pseudoaccuracies for the <see cref="NegativeResults">negative subjects</see>
        /// </summary>
        /// 
        public double[] NegativeAccuracies { get { return this.negativeAccuracy; } }
        ...
        public void Compute(int points)
        {
            ...
            double increment = this.max == this.min ? 1 : (this.max - this.min) / points;
            this.Compute(increment);
        }
        ...
        public void Compute(double increment, bool forceOrigin)
        {
            ...
            for (cutoff = this.min; cutoff <= this.max; cutoff += increment)
                points.Add(this.ComputePoint(cutoff));
            ...
            if (forceOrigin)
            {
                ...
                if (last.FalsePositiveRate != 0.0 || last.Sensitivity != 0.0)
                    points.Add(this.ComputePoint(Double.PositiveInfinity));
            }
            ...
            this.area = this.calculateAreaUnderCurve();
            this.error = this.calculateStandardError();
        }
        ...
        public void Compute(params double[] cutpoints)
        {
            ...
            for (int i = 0; i < cutpoints.Length; i++)
                points.Add(this.ComputePoint(cutpoints[i]));
        ...
        public ReceiverOperatingCharacteristicPoint ComputePoint(double threshold)
        {
            ...
            {
                bool actual = (this.measurement[i] == this.dtrue);
            ...
            int falsePositives = this.negativeCount - trueNegatives;
            int falseNegatives = this.positiveCount - truePositives;
        ...
        public Scatterplot GetScatterplot(bool includeRandom = false)
        {
            ...
            double[] x = this.Points.GetOneMinusSpecificity();
            double[] y = this.Points.GetSensitivity();
        ...
        private double calculateAreaUnderCurve()
        {
            ...
            for (int i = 0; i < this.collection.Count - 1; i++)
            {
                ...
                tpz = this.collection[i].Sensitivity + this.collection[i + 1].Sensitivity;
                tpz = tpz * (this.collection[i].FalsePositiveRate - this.collection[i + 1].FalsePositiveRate) / 2.0;
                sum += tpz;
            }
        ...
        {
            return Math.Sqrt(ReceiverOperatingCurveTest.DeLongVariance(this.positiveAccuracy, this.negativeAccuracy));
        }

        private void calculatePlacements()
        {
            // Get ratings for true positives
            int[] positiveIndices = this.measurement.Find(x => x == this.dtrue);
            double[] X = this.prediction.Get(positiveIndices);

            int[] negativeIndices = this.measurement.Find(x => x == this.dfalse);
            double[] Y = this.prediction.Get(negativeIndices);

            this.positiveResults = X;
            this.negativeResults = Y;

            // Transform into V variables
            this.positiveAccuracy = X.Apply(x => v(x, Y));
            this.negativeAccuracy = Y.Apply(y => 1.0 - v(y, X));
        }
    ...
    public class ReceiverOperatingCharacteristicPoint : ConfusionMatrix
    {
        ...
        {
            return String.Format(System.Globalization.CultureInfo.CurrentCulture,
                "({0}; {1})", this.Sensitivity, this.FalsePositiveRate);
        }
