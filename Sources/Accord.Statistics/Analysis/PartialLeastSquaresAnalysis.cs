...
#pragma warning restore 612, 618
            ...
            this.stdDevX = Measures.StandardDeviation(inputs, this.meanX);
            this.stdDevY = Measures.StandardDeviation(outputs, this.meanY);

            base.NumberOfInputs = this.sourceX.Columns();
            base.NumberOfOutputs = this.NumberOfInputs;
...
#pragma warning restore 612, 618
        ...
        {
            get { return this.inputVariables; }
        }
        ...
        {
            get { return this.outputVariables; }
        }
        ...
        {
            get { return this.factorCollection; }
        }
        ...
        {
            get { return this.overwriteSourceMatrix; }
            set { this.overwriteSourceMatrix = value; }
        }
        ...
        public int NumberOfLatentFactors
        {
            ...
            set
            {
                if (value < 0 || (this.MaximumNumberOfFactors > 0 && value > this.MaximumNumberOfFactors))
                    throw new ArgumentOutOfRangeException("value", "Number of latent factors should be less than or equal the number of columns in the input data {0}.".Format(this.MaximumNumberOfFactors));
        ...
        public MultivariateLinearRegression Learn(double[][] x, double[][] y, double[] weights = null)
        {
            ...
            this.MaximumNumberOfFactors = System.Math.Min(x.Rows() - 1, x.Columns());
            ...
            if (this.NumberOfLatentFactors == 0 || this.NumberOfLatentFactors > this.MaximumNumberOfFactors)
                this.NumberOfLatentFactors = this.MaximumNumberOfFactors;

            this.meanX = x.Mean(dimension: 0);
            this.meanY = y.Mean(dimension: 0);
            this.stdDevX = x.StandardDeviation(this.meanX);
            this.stdDevY = y.StandardDeviation(this.meanY);

            // Initialize and prepare the data
            var inputs = this.Adjust(x, this.meanX, this.stdDevX, this.Overwrite);
            var outputs = this.Adjust(y, this.meanY, null, this.Overwrite);


            // Run selected algorithm
            if (this.algorithm == PartialLeastSquaresAlgorithm.SIMPLS)
            {
                this.simpls(inputs, outputs, this.MaximumNumberOfFactors);
            }
            else
            {
                this.nipals(inputs, outputs, this.MaximumNumberOfFactors, 0);
            }


            // Calculate cumulative proportions
            this.cumulativeProportionX = new double[this.MaximumNumberOfFactors];
            this.cumulativeProportionY = new double[this.MaximumNumberOfFactors];
            this.cumulativeProportionX[0] = this.componentProportionX[0];
            this.cumulativeProportionY[0] = this.componentProportionY[0];
            for (int i = 1; i < this.MaximumNumberOfFactors; i++)
            {
        ...
        {
            // maxFactors = min(rows-1,cols)
            this.MaximumNumberOfFactors = System.Math.Min(this.sourceX.GetLength(0) - 1, this.sourceX.GetLength(1));

            this.Compute(this.MaximumNumberOfFactors);
        }
...
#pragma warning disable 612, 618
            ...
            if (factors > this.MaximumNumberOfFactors)
                throw new ArgumentOutOfRangeException("factors");

            // Initialize and prepare the data
            double[,] inputs = this.Adjust(this.sourceX, this.meanX, this.stdDevX, this.Overwrite);
            double[,] outputs = this.Adjust(this.sourceY, this.meanY, null, this.Overwrite);

            this.NumberOfLatentFactors = factors;
            base.NumberOfInputs = this.sourceX.Columns();
            base.NumberOfOutputs = this.sourceY.Columns();

            // Run selected algorithm
            if (this.algorithm == PartialLeastSquaresAlgorithm.SIMPLS)
            {
                this.simpls(inputs.ToJagged(), outputs.ToJagged(), factors);
            }
            else
            {
                this.nipals(inputs.ToJagged(), outputs.ToJagged(), factors, 0);
            }
            ...
            // Compute Variable Importance in Projection (VIP)
            this.vip = this.ComputeVariableImportanceInProjection(factors);
...
#pragma warning restore 612, 618
        ...
        {
            return this.Transform(data, this.loadingsX.GetLength(1));
        }
        ...
        public double[,] Transform(double[,] data, int dimensions)
        {
            ...
            if (cols > this.loadingsX.GetLength(0))
            {
            ...
            double[,] source = this.Adjust(data, this.meanX, this.stdDevX, false);

            // multiply the data matrix by the selected factors
            for (int i = 0; i < rows; i++)
                for (int j = 0; j < dimensions; j++)
                    for (int k = 0; k < cols; k++)
                        result[i, j] += source[i, k] * this.loadingsX[k][j];
        ...
        {
            double[][] source = this.Adjust(input, this.meanX, this.stdDevX, false);
        ...
        {
            return this.TransformOutput(outputs, this.loadingsY.Columns());
        }
        ...
        public double[,] TransformOutput(double[,] outputs)
        {
            return this.TransformOutput(outputs.ToJagged(), this.loadingsY.Columns()).ToMatrix();
        }
        ...
        {
            return this.TransformOutput(outputs.ToJagged(), dimensions).ToMatrix();
        }
        ...
        public double[][] TransformOutput(double[][] outputs, int dimensions)
        {
            ...
            double[][] source = this.Adjust(outputs, this.meanY, this.stdDevY, false);

            // multiply the data matrix by the selected factors
            for (int i = 0; i < rows; i++)
                for (int j = 0; j < dimensions; j++)
                    for (int k = 0; k < cols; k++)
                        result[i][j] += source[i][k] * this.loadingsY[k][j];
        ...
        {
            return this.CreateRegression(this.NumberOfLatentFactors);
        }
        ...
        {
            if (factors > this.factorCollection.Count)
                throw new ArgumentOutOfRangeException("factors",
            ...
            double[][] B = Jagged.Zeros(this.coeffbase.Length, this.loadingsY.Length);
            for (int i = 0; i < this.coeffbase.Length; i++)
                for (int j = 0; j < this.loadingsY.Length; j++)
                    for (int k = 0; k < factors; k++)
                        B[i][j] += this.coeffbase[i][k] * this.loadingsY[j][k];
            ...
            double[] A = new double[this.loadingsY.Length];
            for (int i = 0; i < this.meanY.Length; i++)
            {
                double sum = 0.0;
        ...
        private void nipals(double[][] inputsX, double[][] outputsY, int factors, double tolerance)
        {
            ...
            {
                this.componentProportionY[i] = varY[i] / sumY;
                this.componentProportionX[i] = varX[i] / sumX;
            }
        }
        ...
        protected double[][] ComputeVariableImportanceInProjection(int factors)
        {
            ...
            for (int j = 0; j < xcols; j++)
            {
                ...
                for (int k = 0; k < factors; k++)
                {
                    ...
                    double b = this.loadingsY.GetColumn(k)[0];
                    double[] t = this.scoresX.GetColumn(k);
                    double[] w = this.loadingsX.GetColumn(k);
    ...
    public sealed class PartialLeastSquaresFactor : IAnalysisComponent
    {
        ...
        {
            get { return this.analysis.componentProportionX[this.index]; }
        }
        ...
        {
            get { return this.analysis.cumulativeProportionX[this.index]; }
        }
        ...
        {
            get { return this.analysis.componentProportionY[this.index]; }
        }
        ...
        {
            get { return this.analysis.cumulativeProportionY[this.index]; }
        }
        ...
        {
            get { return this.analysis.loadingsX.GetColumn(this.index); }
        }
        ...
        {
            get { return this.analysis.loadingsY.GetColumn(this.index); }
        }
        ...
        {
            get { return this.analysis.Importance.GetColumn(this.index); }
        }
        ...
        {
            get { return this.DependentProportion; }
        }
        ...
        {
            get { return this.DependentCumulativeProportion; }
        }
    }
    ...
    public class PartialLeastSquaresVariables
    {
        ...
        {
#pragma warning disable 612, 618
            get { return this.inputs ? this.analysis.sourceX : this.analysis.sourceY; }
#pragma warning restore 612, 618
        }
        ...
        {
            get { return this.inputs ? this.analysis.scoresX.ToMatrix() : this.analysis.scoresY.ToMatrix(); }
        }
        ...
        public double[] Means
        {
            get { return this.inputs ? this.analysis.meanX : this.analysis.meanY; }
        }
        ...
        {
            get { return this.inputs ? this.analysis.stdDevX : this.analysis.stdDevY; }
        }
        ...
        {
            get { return this.inputs ? this.analysis.loadingsX : this.analysis.loadingsY; }
        }
        ...
        {
            get { return this.inputs ? this.analysis.componentProportionX : this.analysis.componentProportionY; }
        }
        ...
        {
            get { return this.inputs ? this.analysis.cumulativeProportionX : this.analysis.cumulativeProportionY; }
        }
        ...
        public double[][] Transform(double[][] data)
        {
            return this.inputs ? this.analysis.Transform(data) : this.analysis.TransformOutput(data);
        }
        ...
        {
            int previous = this.analysis.NumberOfOutputs;
            this.analysis.NumberOfOutputs = factors;
            double[][] result = this.inputs ? this.analysis.Transform(data) : this.analysis.TransformOutput(data);
            this.analysis.NumberOfOutputs = previous;
            return result;
        }
    }
