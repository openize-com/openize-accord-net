...
#pragma warning restore 612, 618
    {
            ...
            IndependentComponentAlgorithm algorithm)
        {
            ...
            this.columnMeans = Measures.Mean(this.sourceMatrix, dimension: 0);
            this.columnStdDev = Measures.StandardDeviation(this.sourceMatrix, this.columnMeans);
        ...
        {
            get { return this.whiteningMatrix; }
        }
        ...
        {
            get { return this.componentCollection; }
        }
        ...
        public void Compute(int components)
        {
            ...
            if (this.algorithm == IndependentComponentAlgorithm.Deflation)
            {
                this.revertMatrix = this.deflation(whiten, components, initial);
            }
            else // if (algorithm == IndependentComponentAlgorithm.Parallel)
            {
                this.revertMatrix = this.parallel(whiten, components, initial);
            }

            // Combine the rotation and demixing matrices
            this.revertMatrix = this.whiteningMatrix.DotWithTransposed(this.revertMatrix);
            this.revertMatrix.Divide(this.revertMatrix.Sum(), result: this.revertMatrix);

            // Compute the original source mixing matrix
            this.mixingMatrix = Matrix.PseudoInverse(this.revertMatrix);
            this.mixingMatrix.Divide(this.mixingMatrix.Sum(), result: this.mixingMatrix);

            // Demix the data into independent components
            this.resultMatrix = Matrix.Dot(matrix, this.revertMatrix).ToMatrix();

            this.demix = createRegression(this.revertMatrix, this.columnMeans, this.columnStdDev, this.analysisMethod);
            this.mix = this.demix.Inverse();
        ...
        {
            return this.demix.Transform(data.ToJagged()).ToMatrix();
        }
        ...
        {
            return this.demix.Transform(data.ToDouble().Transpose()).ToSingle().Transpose(); // TODO: Implement float support in linear regression
        }
        ...
        {
            return this.mix.Transform(data.ToJagged()).ToMatrix();
        }
        ...
        {
            return this.mix.Transform(data.ToDouble().Transpose()).ToSingle().Transpose(); // TODO: Implement float support in linear regression;
        }
        ...
        private double[][] deflation(double[][] X, int components, double[][] init)
        {
            ...
            for (int i = 0; i < components; i++)
            {
                ...
                do // until convergence
                {
                    ...
                    if (!(delta > this.tolerance && iterations < this.maxIterations) || this.Token.IsCancellationRequested)
                    {
                        stop = true;
                    }
                    else
                    {
                        ...
                        this.contrast.Evaluate(wx, gwx, dgwx);
        ...
        private double[][] parallel(double[][] X, int components, double[][] winit)
        {
            ...
            do // until convergence
            {
                ...
                else
                {
                    ...
                    Parallel.For(0, components, this.parallelOptions, i =>
                    {
        ...
        private static double getMaximumAbsoluteChange(double[] w, double[] w0)
        {
            ...
            return Math.Abs((int)(Math.Abs(diag.Sum()) - 1));
        }
        ...
        public MultivariateLinearRegression Learn(double[][] x, double[] weights = null)
        {
            ...
            this.columnStdDev = Measures.StandardDeviation(x, this.columnMeans);

            this.NumberOfInputs = x.Columns();
            if (this.NumberOfOutputs == 0)
                this.NumberOfOutputs = this.NumberOfInputs;
            ...
            if (this.algorithm == IndependentComponentAlgorithm.Deflation)
            {
                this.revertMatrix = this.deflation(whiten, this.NumberOfOutputs, initial);
            }
            else // if (algorithm == IndependentComponentAlgorithm.Parallel)
            {
                this.revertMatrix = this.parallel(whiten, this.NumberOfOutputs, initial);
            }
            ...
            // Creates the object-oriented structure to hold the principal components
            var array = new IndependentComponent[this.NumberOfOutputs];
            for (int i = 0; i < array.Length; i++)
                array[i] = new IndependentComponent(this, i);
            this.componentCollection = new IndependentComponentCollection(array);
        ...
        protected float[][] Adjust(float[][] matrix, bool inPlace)
        {
            ...
            matrix.Subtract(this.columnMeans, dimension: (VectorType)0, result: result);
            if (this.analysisMethod == AnalysisMethod.Standardize)
                result.Divide(this.columnStdDev, dimension: (VectorType)0, result: result);
        ...
        public override double[] Transform(double[] input, double[] result)
        {
            ...
            double[] matrix = this.Adjust(new[] { input }, false)[0];
            return Matrix.Dot(matrix, this.revertMatrix, result: result);
        }
    }
    ...
    public class IndependentComponent
    {
        ...
        {
            get { return this.analysis.MixingMatrix.GetColumn(this.index); }
        }
        ...
        {
            get { return this.analysis.DemixingMatrix.GetColumn(this.index); }
        }
        ...
        {
            get { return this.analysis.WhiteningMatrix.GetColumn(this.index); }
        }
