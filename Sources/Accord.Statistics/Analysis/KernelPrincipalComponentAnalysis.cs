...
#pragma warning restore 612, 618
    {
        ...
        public bool Center
        {
            get { return this.centerFeatureSpace; }
            set { this.centerFeatureSpace = value; }
        }
        ...
        {
            get { return this.allowReversion; }
            set { this.allowReversion = value; }
        }
        ...
        public void Compute()
        {
            this.Compute(this.NumberOfOutputs);
        }
        ...
        public MultivariateKernelRegression Learn(double[][] x, double[] weights = null)
        {
            ...
            if (this.Method == PrincipalComponentMethod.KernelMatrix)
            {
                K = x;
            ...
            else
            {
                ...
                this.sourceCentered = this.Overwrite ? x : Jagged.CreateAs(x);
                x.Subtract(this.Means, dimension: (VectorType)0, result: this.sourceCentered);

                if (this.Method == PrincipalComponentMethod.Standardize)
                {
                    this.StandardDeviations = x.StandardDeviation(this.Means);
                    this.sourceCentered.Divide(this.StandardDeviations, dimension: (VectorType)0, result: this.sourceCentered);
                }
            ...
            Debug.Assert(this.NumberOfOutputs > 0);
        ...
        private MultivariateKernelRegression CreateRegression()
        {
            return new MultivariateKernelRegression()
            {
                ...
                Kernel = this.kernel,
                Weights = this.ComponentVectors,
                BasisVectors = this.sourceCentered,
                Means = this.Means,
                StandardDeviations = this.StandardDeviations,
                FeatureGrandMean = this.featureGrandMean,
                FeatureMeans = this.featureMean
            };
        }
        ...
        {
            this.NumberOfOutputs = components;
            if (this.array != null)
                this.Learn(this.array);
            else
#pragma warning disable 612, 618
                this.Learn(this.source);
#pragma warning restore 612, 618
        }
        ...
        public override double[][] Transform(double[][] data, double[][] result)
        {
            ...
            {
                if (data.Columns() != this.NumberOfInputs)
                    throw new DimensionMismatchException("data",
                        "The input data should have the same number of columns as the original data.");

                data = data.Subtract(this.Means, dimension: (VectorType)0);
                if (this.Method == PrincipalComponentMethod.Standardize)
                    data.Divide(this.StandardDeviations, dimension: (VectorType)0, result: data);
            ...
            if (this.NumberOfOutputs == this.ComponentVectors.Length)
            {
                return newK.DotWithTransposed(this.ComponentVectors, result: result);
            }

            if (this.NumberOfOutputs < this.ComponentVectors.Length)
            {
                var selectedComponents = this.ComponentVectors.Get(0, this.NumberOfOutputs);
        ...
        {
            return this.Revert(data.ToJagged(), neighbors).ToMatrix();
        }
        ...
        public double[][] Revert(double[][] data, int neighbors = 10)
        {
            ...
            var distance = this.kernel as IReverseDistance;
            ...
            double[][] reversion = Jagged.Zeros(rows, this.sourceCentered.Columns());

            // number of neighbors cannot exceed the number of training vectors.
            int nn = System.Math.Min(neighbors, this.sourceCentered.Rows());
            ...
            if (this.Method == PrincipalComponentMethod.Standardize)
            {
                ...
                reversion.Multiply(this.StandardDeviations, dimension: (VectorType)0, result: reversion)
                    .Add(this.Means, dimension: (VectorType)0, result: reversion);
            }
