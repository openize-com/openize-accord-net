...
#pragma warning restore 612, 618
    {
        ...
        public MultivariateLinearRegression Learn(double[][] x, double[] weights = null)
        {
            ...
            if (this.Method == PrincipalComponentMethod.Center || this.Method == PrincipalComponentMethod.Standardize)
            {
                if (weights == null)
                {
                    this.Means = x.Mean(dimension: 0);

                    double[][] matrix = this.Overwrite ? x : Jagged.CreateAs(x);
                    x.Subtract(this.Means, dimension: (VectorType)0, result: matrix);

                    if (this.Method == PrincipalComponentMethod.Standardize)
                    {
                        this.StandardDeviations = x.StandardDeviation(this.Means);
                        matrix.Divide(this.StandardDeviations, dimension: (VectorType)0, result: matrix);
                    }
                    ...
                    this.Eigenvalues = this.SingularValues.Pow(2);
                    this.Eigenvalues.Divide(x.Rows() - 1, result: this.Eigenvalues);
                    this.ComponentVectors = svd.RightSingularVectors.Transpose();
                }
                else
                {
                    ...
                    if (this.Method == PrincipalComponentMethod.Standardize)
                    {
                        this.StandardDeviations = x.WeightedStandardDeviation(weights, this.Means);
                        matrix.Divide(this.StandardDeviations, dimension: (VectorType)0, result: matrix);
                    }

                    double[,] cov = x.WeightedCovariance(weights, this.Means);
                    ...
                    this.SingularValues = this.Eigenvalues.Sqrt();
                    this.ComponentVectors = Jagged.Transpose(evd.Eigenvectors);
                }
            }
            else if (this.Method == PrincipalComponentMethod.CovarianceMatrix
                  || this.Method == PrincipalComponentMethod.CorrelationMatrix)
            {
                if (weights != null)
                    throw new Exception();
                ...
                this.Eigenvalues = evd.RealEigenvalues;
                this.SingularValues = this.Eigenvalues.Sqrt();
                this.ComponentVectors = evd.Eigenvectors.Transpose();
            }
            else
            {
                // The method type should have been validated before we even entered this section
                throw new InvalidOperationException("Invalid method, this should never happen: {0}".Format(this.Method));
            }

            if (this.Whiten)
                this.ComponentVectors.Divide(this.SingularValues, dimension: (VectorType)1, result: this.ComponentVectors);
        ...
        {
            double[][] weights = this.ComponentVectors;
            if (this.Method == PrincipalComponentMethod.Standardize || this.Method == PrincipalComponentMethod.CorrelationMatrix)
                weights = weights.Divide(this.StandardDeviations, dimension: (VectorType)0);

            double[] bias = weights.Dot(this.Means).Multiply(-1);
        ...
        public virtual void Compute()
        {
            if (!this.onlyCovarianceMatrixAvailable)
            {
                ...
                if (this.array != null)
                {
                    ...
                    double[][] matrix = this.Adjust(this.array, this.Overwrite);
                ...
                {
                    rows = this.source.GetLength(0);

                    // Center and standardize the source matrix
#pragma warning disable 612, 618
                    double[,] matrix = this.Adjust(this.source, this.Overwrite);
#pragma warning restore 612, 618
                    ...
                    // The right singular vectors contains the principal components of the data matrix
                    this.ComponentVectors = svd.RightSingularVectors.ToJagged(transpose: true);
            ...
            {
                // We only have the covariance matrix. Compute the Eigenvalue decomposition
                var evd = new EigenvalueDecomposition(this.covarianceMatrix,
                ...
                this.SingularValues = this.Eigenvalues.Sqrt();
                this.ComponentVectors = eigenvectors.Transpose();
            }

            if (this.Whiten)
            {
                this.ComponentVectors = this.ComponentVectors.Transpose().Divide(this.Eigenvalues, dimension: 0).Transpose();
            }
            ...
            {
                if (this.array != null)
                    this.result = this.Transform(this.array).ToMatrix();
                else if (this.source != null)
                    this.result = this.Transform(this.source.ToJagged()).ToMatrix();
            }
        }
...
#pragma warning restore 612, 618
            ...
            for (int i = 0; i < rows; i++)
                for (int j = 0; j < this.NumberOfOutputs; j++)
                    for (int k = 0; k < this.ComponentVectors[j].Length; k++)
                        result[i][j] += data[i][k] * this.ComponentVectors[j][k];
        ...
        {
            return this.Revert(data.ToJagged()).ToMatrix();
        }
        ...
        public virtual double[][] Revert(double[][] data)
        {
            ...
            int rows = data.Rows();
            int cols = data.Columns();
            int components = this.NumberOfOutputs;
            ...
            for (int i = 0; i < components; i++)
                for (int j = 0; j < rows; j++)
                    for (int k = 0; k < cols; k++)
                        reversion[j][i] += data[j][k] * this.ComponentVectors[k][i];


            // if the data has been standardized or centered,
            //  we need to revert those operations as well
            if (this.Method == PrincipalComponentMethod.Standardize || this.Method == PrincipalComponentMethod.CorrelationMatrix)
                reversion.Multiply(this.StandardDeviations, dimension: (VectorType)0, result: reversion);

            reversion.Add(this.Means, dimension: (VectorType)0, result: reversion);
            return reversion;
        }
    ...
    public class PrincipalComponent : IAnalysisComponent
    {
        ...
        {
            get { return this.principalComponentAnalysis.ComponentProportions[this.index]; }
        }
        ...
        {
            get { return this.principalComponentAnalysis.CumulativeProportions[this.index]; }
        }
        ...
        {
            get { return this.principalComponentAnalysis.SingularValues[this.index]; }
        }
        ...
        {
            get { return this.principalComponentAnalysis.Eigenvalues[this.index]; }
        }
        ...
        {
            get { return this.principalComponentAnalysis.ComponentVectors[this.index]; }
        }
    }
