...
#pragma warning restore 612, 618
    {
        ...
        {
            this.CheckSampleDataArgument(samples);
        ...
        {
            return this.Translate(samples, new double[samples.GetLength(1)]);
        }
        ...
        {
            double[,] res = new double[samples.GetLength(0), samples.GetLength(1)];
            double sqr = this.GetDatasetScale(samples);
        ...
        void UpdateProcrustesDistances(params ProcrustedDataset[] p)
        {
            ...
            this.ProcrustesDistances = new double[p.Length, p.Length];
            for (int i = 0; i < this.ProcrustesDistances.GetLength(0); i++)
            {
                this.ProcrustesDistances[i, i] = 0;
            }
            ...
            for (int i = 0; i < this.ProcrustesDistances.GetLength(0); i++)
            {
                for (int j = 0; j <= i; j++)
                {
                    if (i != j)
                    {
                        ...
                        this.ProcrustesDistances[i, j] = this.ProcrustesDistance(p[i].Dataset, p[j].Dataset);

                        // The other corner of the matrix is the same as the other corner
                        this.ProcrustesDistances[j, i] = this.ProcrustesDistances[i, j];
                    }
                }
            }
        }
        ...
        private void ApplyTranslateScale(ProcrustedDataset p, double[,] samples)
        {
            ...
            p.Dataset = this.Translate(samples);

            // Save the original scale of the dataset
            p.Scale = this.GetDatasetScale(p.Dataset);
            // Scale the dataset to 1
            p.Dataset = this.Scale(p.Dataset);
        }
        ...
        public double[,] Compute(int reference_sample_index, params double[][,] samples)
        {
            ...
            this.ProcrustedDatasets = new ProcrustedDataset[samples.Length];

            // Start with the reference dataset
            this.ProcrustedDatasets[reference_sample_index] = new ProcrustedDataset();
            // The rotation martrix of the reference is the identity matrix (i.e. no rotation since it's the reference itself)
            this.ProcrustedDatasets[reference_sample_index].RotationMatrix = Matrix.Identity(samples[reference_sample_index].GetLength(1));
            // Translate then scale the dataset to set it with scale = 1 and center position at zero
            this.ApplyTranslateScale(this.ProcrustedDatasets[reference_sample_index], samples[reference_sample_index]);
            ...
            for (int i = 0; i < samples.Length; i++)
            {
                ...
                {
                    this.ProcrustedDatasets[i] = new ProcrustedDataset();
                    // Translate then scale the dataset
                    this.ApplyTranslateScale(this.ProcrustedDatasets[i], samples[i]);

                    // Finally, rotate the sample to fit the reference data rotation
                    this.ProcrustedDatasets[i].Dataset = this.Rotate(this.ProcrustedDatasets[i], this.ProcrustedDatasets[reference_sample_index]);
                }
            }

            // Update the Procrustes distance matrix
            this.UpdateProcrustesDistances(this.ProcrustedDatasets);

            return this.ProcrustesDistances;
        }
    }
    ...
    public class ProcrustedDataset
    {
        ...
        public double[,] Transform(ProcrustedDataset p_reference)
        {
            ...
            double[,] tData = (double[,])this.Dataset.Clone();
