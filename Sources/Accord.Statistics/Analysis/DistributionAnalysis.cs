...
namespace FileFormat.Accord.Statistics.Analysis
{
    ...
    using Distributions.Reflection;
    ...
    public class DistributionAnalysis
    {
        ...
        {
            this.Distributions = new List<IFittableDistribution<double>>()
            {
            ...
            this.Options = new Dictionary<IFittableDistribution<double>, IFittingOptions>();
        }
        ...
        private void compute(double[] data, double[] weights)
        {
            ...
            this.KolmogorovSmirnov = new KolmogorovSmirnovTest[this.Distributions.Count];
            this.ChiSquare = new ChiSquareTest[this.Distributions.Count];
            this.AndersonDarling = new AndersonDarlingTest[this.Distributions.Count];
            this.DistributionNames = new string[this.Distributions.Count];

            double[] ks = new double[this.Distributions.Count];
            double[] cs = new double[this.Distributions.Count];
            double[] ad = new double[this.Distributions.Count];
            ...
            for (int i = 0; i < this.Distributions.Count; i++)
            {
                ...
                run(() =>
                {
                    ...
                    ks[i] = -this.KolmogorovSmirnov[i].Statistic;
                }, ms);

                run(() =>
                {
                    this.ChiSquare[i] = new ChiSquareTest(data, d);
                    cs[i] = -this.ChiSquare[i].Statistic;
                }, ms);

                run(() =>
                {
                    this.AndersonDarling[i] = new AndersonDarlingTest(data, d);
                    ad[i] = this.AndersonDarling[i].Statistic;
                }, ms);
            ...
            this.KolmogorovSmirnovRank = this.getRank(ks);
            this.ChiSquareRank = this.getRank(cs);
            this.AndersonDarlingRank = this.getRank(ad);
        ...
        {
            int[] idx = Vector.Range(0, this.Distributions.Count);
        ...
        {
            return this.Distributions.Find(d => d.GetType().Name == name);
        }
    }
}
