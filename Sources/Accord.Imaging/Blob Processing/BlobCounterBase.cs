...
namespace FileFormat.Accord.Imaging.Blob_Processing
{
    ...
    public abstract class BlobCounterBase : IDisposable
    {
        ...
        public int ObjectsCount
        {
            ...
            protected set { this.objectsCount = value; }
        }
        ...
        public int[] ObjectLabels
        {
            ...
            protected set { this.objectLabels = value; }
        }
        ...
        public void ProcessImage(UnmanagedImage image)
        {
            ...
            if (this.filterBlobs)
            {
                ...
                int[] labelsMap = new int[this.objectsCount + 1];
                ...
                if (this.filter == null)
                {
                    for (int i = this.objectsCount - 1; i >= 0; i--)
                    {
                        int blobWidth = this.blobs[i].Rectangle.Width;
                        int blobHeight = this.blobs[i].Rectangle.Height;
        ...
        public Blob[] GetObjectsInformation()
        {
            ...
            Blob[] blobsToReturn = new Blob[this.objectsCount];
        ...
        public Blob[] GetObjects(UnmanagedImage image, bool extractInOriginalSize)
        {
            ...
            Blob[] objects = new Blob[this.objectsCount];

            // create each image
            for (int k = 0; k < this.objectsCount; k++)
            {
                int objectWidth = this.blobs[k].Rectangle.Width;
                int objectHeight = this.blobs[k].Rectangle.Height;

                int blobImageWidth = (extractInOriginalSize) ? width : objectWidth;
                int blobImageHeight = (extractInOriginalSize) ? height : objectHeight;

                int xmin = this.blobs[k].Rectangle.X;
                int xmax = xmin + objectWidth - 1;
                int ymin = this.blobs[k].Rectangle.Y;
                int ymax = ymin + objectHeight - 1;

                int label = this.blobs[k].ID;
        ...
        public void GetBlobsTopAndBottomEdges(Blob blob, out List<IntPoint> topEdge, out List<IntPoint> bottomEdge)
        {
            ...
            for (int x = xmin; x <= xmax; x++)
            {
                ...
                int p = ymin * this.ImageWidth + x;
                for (int y = ymin; y <= ymax; y++, p += this.ImageWidth)
                {
                    if (this.objectLabels[p] == label)
                    {
                ...
                for (int y = ymax; y >= ymin; y--, p -= this.ImageWidth)
                {
                    if (this.objectLabels[p] == label)
                    {
        ...
        private unsafe void CollectObjectsInfo(UnmanagedImage image)
        {
            ...
            int[] x1 = new int[this.objectsCount + 1];
            int[] y1 = new int[this.objectsCount + 1];
            int[] x2 = new int[this.objectsCount + 1];
            int[] y2 = new int[this.objectsCount + 1];

            int[] area = new int[this.objectsCount + 1];
            long[] xc = new long[this.objectsCount + 1];
            long[] yc = new long[this.objectsCount + 1];

            long[] meanR = new long[this.objectsCount + 1];
            long[] meanG = new long[this.objectsCount + 1];
            long[] meanB = new long[this.objectsCount + 1];

            long[] stdDevR = new long[this.objectsCount + 1];
            long[] stdDevG = new long[this.objectsCount + 1];
            long[] stdDevB = new long[this.objectsCount + 1];
            ...
            {
                int offset = image.Stride - this.ImageWidth;
                byte g; // pixel's grey value

                // walk through labels array
                for (int y = 0; y < this.ImageHeight; y++)
                {
                    for (int x = 0; x < this.ImageWidth; x++, i++, src++)
                    {
                        // get current label
            ...
            else
            {
                // color images
                int pixelSize = Bitmap.GetPixelFormatSize(image.PixelFormat) / 8;
                int offset = image.Stride - this.ImageWidth * pixelSize;
                byte r, g, b; // RGB value

                // walk through labels array
                for (int y = 0; y < this.ImageHeight; y++)
                {
                    for (int x = 0; x < this.ImageWidth; x++, i++, src += pixelSize)
                    {
                        // get current label
                        label = this.objectLabels[i];
