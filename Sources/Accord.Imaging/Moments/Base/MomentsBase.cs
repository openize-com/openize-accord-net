...
namespace FileFormat.Accord.Imaging.Moments.Base
{
    using System;
    ...
    public abstract class MomentsBase
    {
        ...
        protected MomentsBase(Bitmap image, int order)
            : this(order)
        {
            this.Compute(image, Rectangle.Empty);
        }
        ...
        {
            this.Compute(image, new Rectangle(0, 0, image.Width, image.Height));
        }
        ...
        public void Compute(Bitmap image, Rectangle area)
        {
            ...
            try
            {
                this.Compute(new UnmanagedImage(imageData), area);
            }
            finally
            {
        ...
        {
            this.Compute(new UnmanagedImage(image), area);
        }
