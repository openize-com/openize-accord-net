...
namespace FileFormat.Accord.Imaging.Filters
{
    ...
    public class Blend : BaseTransformationFilter
    {
        ...
        {
            get { return this.homography; }
            set { this.homography = value; }
        }
        ...
        public Blend(MatrixH homography, Bitmap overlayImage)
        {
            ...
            this.formatTranslations[PixelFormat.Format8bppIndexed] = PixelFormat.Format32bppArgb;
            this.formatTranslations[PixelFormat.Format24bppRgb] = PixelFormat.Format32bppArgb;
        ...
        protected override Size CalculateNewImageSize(UnmanagedImage sourceData)
        {
            ...
            corners = this.homography.Inverse().TransformPoints(corners);
            ...
            float newWidth = Math.Max(maxX, this.overlayImage.Width) - Math.Min(0, minX);

            float maxY = Matrix.Max(py);
            float minY = Matrix.Min(py);
            float newHeight = Math.Max(maxY, this.overlayImage.Height) - Math.Min(0, minY);
        ...
        protected override void ProcessFilter(UnmanagedImage sourceData, UnmanagedImage destinationData)
        {
            ...
            BitmapData overlayData = this.overlayImage.LockBits(
              new Rectangle(0, 0, this.overlayImage.Width, this.overlayImage.Height),
              ImageLockMode.ReadOnly, this.overlayImage.PixelFormat);
            ...
            Point center1 = new Point((int)(this.overlayImage.Width / 2f),
                                      (int)(this.overlayImage.Height / 2f));
            ...
            float dmax1 = Math.Min(
                distance(center1.X, center1.Y, center2.X - this.imageSize.Width / 2f, center1.Y),
                distance(center1.X, center1.Y, center1.X, center1.Y + this.overlayImage.Height / 2f));

            float dmax2 = Math.Min(
                distance(center2.X, center2.Y, center2.X + this.imageSize.Width / 2f, center2.Y),
                distance(center2.X, center2.Y, center2.X, center2.Y + this.imageSize.Height / 2f));

            float dmax = -System.Math.Abs(dmax2 - dmax1);


            // fill values
            byte fillR = this.fillColor.R;
            byte fillG = this.fillColor.G;
            byte fillB = this.fillColor.B;
            byte fillA = 0;//fillColor.A;

            // Retrieve homography matrix as float array
            float[,] H = (float[,])this.homography;
            ...
            unsafe
            {
                ...
                for (int y = 0; y < newHeight; y++)
                {
                    ...
                    {
                        ox = (int)(x + this.offset.X);
                        oy = (int)(y + this.offset.Y);
            ...
            this.overlayImage.UnlockBits(overlayData);
        }
