...
namespace Openize.Accord.Imaging.Filters
{
    ...
    using Interest_Points.FREAK;
    using Interest_Points.SURF;
    ...
    public class FeaturesMarker : BaseFilter
    {
        ...
        protected override void ProcessFilter(UnmanagedImage sourceData, UnmanagedImage destinationData)
        {
            if (sourceData.PixelFormat == PixelFormat.Format8bppIndexed)
                sourceData = this.toRGB.Apply(sourceData);
            ...
            {
                // mark all points
                foreach (SpeededUpRobustFeaturePoint p in this.points)
                {
                    int S = (int)(this.scale * p.Scale);
                    int R = (int)(S / 2f);
