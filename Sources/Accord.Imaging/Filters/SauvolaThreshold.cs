            ...
            if (sourceData.PixelFormat == PixelFormat.Format8bppIndexed)
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    for (int x = 0; x < width; x++, src++, dst++)
                    {
                        ...
                        double cut = mean * (1.0 + this.k * ((Math.Sqrt(variance) / this.r) - 1.0));
            ...
            else
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    for (int x = 0; x < width; x++, src += pixelSize, dst += pixelSize)
                    {
                        ...
                        double cutR = meanR * (1.0 + this.k * ((Math.Sqrt(varR) / this.r) - 1.0));
                        double cutG = meanG * (1.0 + this.k * ((Math.Sqrt(varG) / this.r) - 1.0));
                        double cutB = meanB * (1.0 + this.k * ((Math.Sqrt(varB) / this.r) - 1.0));
