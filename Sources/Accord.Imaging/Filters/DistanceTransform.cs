    ...
    public class DistanceTransform : BaseInPlaceFilter
    {
        ...
        protected unsafe override void ProcessFilter(UnmanagedImage image)
        {
            ...
            for (int y = 0; y < height; y++)
                this.edmLine(bPixels, this.fPixels, pointBufs, width, y * width, y);
            ...
            if (this.NeedSquareRoot(this.distance))
            {
                for (int i = 0, p = 0; i < height; i++)
                {
                    for (int j = 0; j < width; j++, p++)
                    {
                        float f = this.fPixels[p] = (this.fPixels[p] < 0f) ? 0 : (float)Math.Sqrt(this.fPixels[p]);
            ...
            else
            {
                for (int i = 0, p = 0; i < height; i++)
                {
                    for (int j = 0; j < width; j++, p++)
                    {
                        float f = this.fPixels[p] = (this.fPixels[p] < 0f) ? 0 : this.fPixels[p];
            ...
            // Normalize and store
            fixed (float* srcPtr = this.fPixels)
            {
                ...
                for (int i = 0; i < height; i++)
                {
                    for (int j = 0; j < width; j++, src++, dst++)
                        (*dst) = (byte)Vector.Scale(*src, 0, this.max, 0, 255);
                    dst += offset;
                }
            }
        }
        ...
        private void edmLine(byte[] bPixels, float[] fPixels, int[][] pointBufs, int width, int offset, int y)
        {
            ...
            for (int x = 0; x < width; x++, offset++)
            {
                ...
                {
                    // foreground pixel:
                    float dist2 = this.minDist2(points, pPrev, pDiag, x, y, distSqr, this.distance);
        ...
        private float minDist2(int[] points, int pPrev, int pDiag, int x, int y, int distSqr, DistanceTransformMethod distance)
        {
            ...
            if (p0 != -1)
            {
                ...
                int dist1Sqr = this.calcDistance(x, y, x0, y0, distance);
                if (dist1Sqr < distSqr)
                    distSqr = dist1Sqr;
            }
            ...
            if (pDiag != p0 && pDiag != -1)
            {
                ...
                int dist1Sqr = this.calcDistance(x, y, x1, y1, distance);
                if (dist1Sqr < distSqr)
                {
                    nearestPoint = pDiag;
                    distSqr = dist1Sqr;
