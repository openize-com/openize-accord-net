    ...
    public class ExtractBiggestBlob : IFilter, IFilterInformation
    {
        ...
        public Dictionary<PixelFormat, PixelFormat> FormatTranslations
        {
            get
            {
                ...
                {
                    formatTranslations[PixelFormat.Format8bppIndexed] = this.originalImage.PixelFormat;
                    formatTranslations[PixelFormat.Format24bppRgb] = this.originalImage.PixelFormat;
                    formatTranslations[PixelFormat.Format32bppArgb] = this.originalImage.PixelFormat;
                    formatTranslations[PixelFormat.Format32bppRgb] = this.originalImage.PixelFormat;
                    formatTranslations[PixelFormat.Format32bppPArgb] = this.originalImage.PixelFormat;
                }
        ...
        {
            using (UnmanagedImage biggestBlob = this.Apply(new UnmanagedImage(imageData)))
            {
        ...
        public UnmanagedImage Apply(UnmanagedImage image)
        {
            ...
            if (!this.FormatTranslations.ContainsKey(image.PixelFormat))
            ...
            this.blobPosition = new IntPoint(biggestBlob.Rectangle.Left, biggestBlob.Rectangle.Top);
            ...
            else
            {
                ...
                if (
                    (this.originalImage.PixelFormat != PixelFormat.Format24bppRgb) &&
                    (this.originalImage.PixelFormat != PixelFormat.Format32bppArgb) &&
                    (this.originalImage.PixelFormat != PixelFormat.Format32bppRgb) &&
                    (this.originalImage.PixelFormat != PixelFormat.Format32bppPArgb) &&
                    (this.originalImage.PixelFormat != PixelFormat.Format8bppIndexed)
                    )
                {
                ...
                // check its size
                if ((this.originalImage.Width != image.Width) || (this.originalImage.Height != image.Height))
                    throw new InvalidImagePropertiesException("Original image must have the same size as passed source image.");

                blobCounter.ExtractBlobsImage(this.originalImage, biggestBlob, false);
            }
        ...
        {
            this.Apply(sourceImage).Copy(destinationImage);
        }
    }
}
