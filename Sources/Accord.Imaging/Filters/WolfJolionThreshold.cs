...
namespace FileFormat.Accord.Imaging.Filters
{
    ...
    public class WolfJolionThreshold : BaseFilter
    {
        ...
        protected override unsafe void ProcessFilter(UnmanagedImage sourceData, UnmanagedImage destinationData)
        {
            ...
            int[,] kernel = Matrix.Create(this.radius * 2 + 1, this.radius * 2 + 1, 1);
            Convolution conv = new Convolution(kernel);
            FastVariance fv = new FastVariance(this.radius);
            ...
            if (sourceData.PixelFormat == PixelFormat.Format8bppIndexed)
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    for (int x = 0; x < width; x++, src++, srcMean++, srcVar++, dst++)
                    {
                        ...
                        double threshold = (mP + this.k * ((Math.Sqrt(vP) / (double)maxV - 1.0) * (mP - (double)minG)));
