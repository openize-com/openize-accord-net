...
namespace Openize.Accord.Imaging.Filters
{
    ...
    public class BinaryWatershed : BaseInPlaceFilter
    {
        ...
        protected override void ProcessFilter(UnmanagedImage image)
        {
            ...
            this.makeDirectionOffsets(width);

            byte[] back = new byte[width * height];

            // Find the maxima
            long[] maxPoints = this.getSortedMaxPoints(distance, distance1D, back, 0, dt.MaximumDistance, -808080.0);

            // Mark the maxima locations
            float maxSortingError = 1.1f * SQRT2 / 2f;
            this.maxPoints = this.analyseAndMarkMaxima(distance1D, back, maxPoints, this.tolerance, maxSortingError, width, height);

            // Transform em 8bit 0..255
            byte[] outImage = this.make8Bit(distance, back, dt.MaximumDistance, -808080.0, width, height);

            this.cleanupMaxima(outImage, back, maxPoints, width, height);
            this.watershedSegment(outImage, width, height);
        ...
        private long[] getSortedMaxPoints(float[][] distance, float[] distance1D, byte[] types, float globalMin, float globalMax, double threshold)
        {
                        ...
                        if (!(x == 0 || x == distance[0].Length - 1 || y == 0 || y == distance.Length - 1))
                        {
                            if (!(v < threshold))
                            {
                                ...
                                for (int d = 0; d < 8; d++)
                                {
                                    ...
                                    if (isInner || this.isWithin(x, y, d, distance[0].Length, distance.Length))
                                    {
            ...
            for (int y = 0; y < distance.Length; y++)
            {
                ...
                for (int x = 0, pp = x + y * distance[0].Length; x < distance[0].Length; x++, pp++)
                {
                    if (types[pp] == (byte)1)
                    {
                        double fValue = this.trueEdmHeight(x, y, distance1D, distance[0].Length, distance.Length);
                ...
                do
                {
                    ...
                    do // while neighbor list is not fully processed (to listLen)
                    {
                        ...
                        for (int d = 0; d < 8; d++)
                        {
                            ...
                            if ((isInner || this.isWithin(x, y, d, width, height)) && (types[offset2] & (byte)2) == 0)
                            {
            ...
            for (int level = highestValue; level >= 1; level--)
            {
                ...
                while (remaining > 0 && idle < 8)
                {
                    ...
                    do
                    {
                        ...
                        int n = this.processLevel(directionSequence[dIndex % 8], pixels, table,
                    ...
                    if (nextLevel > 0)
                    {
                        ...
                        for (int i = 0, p = levelStart[level]; i < remaining; i++, p++)
                        {
                            ...
                            else
                            {
                                for (int d = 0; d < 8; d++)
                                {
                                    if (this.isWithin(x, y, d, width, height) && pixels[pOffset + this.dirOffset[d]] == 0)
                                    {
