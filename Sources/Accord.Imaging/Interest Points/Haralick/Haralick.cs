...
namespace FileFormat.Accord.Imaging.Interest_Points.Haralick
{
    ...
    public class Haralick : BaseFeatureExtractor<FeatureDescriptor>
    {
        ...
        /// </summary>
        /// 
        public HaralickDescriptorDictionary[,] Descriptors { get { return this.features; } }
        ...
        /// </summary>
        /// 
        public GrayLevelCooccurrenceMatrix Matrix { get { return this.matrix; } }
        ...
        {
            this.init(this.cellSize, this.normalize, degrees);
        }
        ...
        {
            this.init(cellSize, normalize, null);
        }
        ...
        {
            this.init(cellSize, normalize, degrees);
        }
        ...
        protected override IEnumerable<FeatureDescriptor> InnerTransform(UnmanagedImage image)
        {
            ...
            this.matrix = new GrayLevelCooccurrenceMatrix(this.distance,
                CooccurrenceDegree.Degree0, normalize: true, autoGray: this.autoGray);
            ...
            if (this.cellSize > 0)
            {
                ...
                for (int i = 0; i < cellCountX; i++)
                {
                    for (int j = 0; j < cellCountY; j++)
                    {
                        ...
                        var region = new Rectangle(i * this.cellSize, j * this.cellSize, this.cellSize, this.cellSize);

                        foreach (CooccurrenceDegree degree in this.degrees)
                        {
                            this.matrix.Degree = degree;
                            double[,] glcm = this.matrix.Compute(grayImage, region);
            ...
            else
            {
                ...
                {
                    this.matrix.Degree = degree;
                    double[,] glcm = this.matrix.Compute(grayImage);
                    dict[degree] = new HaralickDescriptor(glcm);
                }
            ...
            switch (this.mode)
            {
                case HaralickMode.Average:
                    foreach (HaralickDescriptorDictionary feature in this.features)
                        blocks.Add(feature.Average(this.featureCount));
                    break;

                case HaralickMode.AverageWithRange:
                    foreach (HaralickDescriptorDictionary feature in this.features)
                        blocks.Add(feature.AverageWithRange(this.featureCount));
                    break;

                case HaralickMode.Combine:
                    foreach (HaralickDescriptorDictionary feature in this.features)
                        blocks.Add(feature.Combine(this.featureCount));
                    break;

                case HaralickMode.NormalizedAverage:
                    foreach (HaralickDescriptorDictionary feature in this.features)
                        blocks.Add(feature.Normalize(this.featureCount));
                    break;
            }
            ...
            if (this.normalize)
            {
                ...
                foreach (FeatureDescriptor block in blocks)
                    block.Descriptor.Divide(block.Descriptor.Euclidean() + this.epsilon, result: block.Descriptor);
            }
        ...
        protected override object Clone(ISet<PixelFormat> supportedFormats)
        {
            ...
            clone.autoGray = this.autoGray;
            clone.cellSize = this.cellSize;
            clone.degrees = this.degrees;
            clone.distance = this.distance;
            clone.featureCount = this.featureCount;
            clone.SupportedFormats = this.SupportedFormats;
            if (this.features != null)
                clone.features = (HaralickDescriptorDictionary[,])this.features.Clone();
            if (this.matrix != null)
                clone.matrix = (GrayLevelCooccurrenceMatrix)this.matrix.Clone();
