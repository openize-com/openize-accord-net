...
namespace FileFormat.Accord.Imaging.Interest_Points.Base
{
    ...
    /// </summary>
    /// 
    /// <seealso cref="BaseSparseFeatureExtractor{TPoint}" />
    /// <seealso cref="ICornersDetector" />
    /// 
    [Serializable]
    public abstract class BaseCornersDetector : BaseSparseFeatureExtractor<CornerFeaturePoint>, ICornersDetector
    {
        ...
        {
            // check image format
            if (!this.SupportedFormats.Contains(image.PixelFormat))
        ...
        {
            return this.InnerProcess(input).Select(x => new CornerFeaturePoint(x));
        }
        ...
        {
            // check image format
            if (!this.SupportedFormats.Contains(input.PixelFormat))
                throw new UnsupportedImageFormatException("Unsupported pixel format of the source image.");

            return this.InnerProcess(input);
        }
    }
}