...
namespace FileFormat.Accord.Imaging.Interest_Points.SURF
{
    ...
    public class SpeededUpRobustFeaturesDescriptor : ICloneable
    {
        ...
        public double GetOrientation(int x, int y, int scale)
        {
            ...
            for (double ang1 = 0; ang1 < 2.0 * Math.PI; ang1 += 0.15)
            {
                ...
                for (int k = 0; k < this.ang.Length; k++)
                {
                    ...
                    if (ang1 < ang2 && ang1 < this.ang[k] && this.ang[k] < ang2)
                    {
                        sumX += this.resX[k];
                        sumY += this.resY[k];
                    }
                    else if (ang2 < ang1 && ((this.ang[k] > 0 && this.ang[k] < ang2) || (this.ang[k] > ang1 && this.ang[k] < Math.PI)))
                    {
                        sumX += this.resX[k];
                        sumY += this.resY[k];
                    }
                ...
                while (j < 12)
                {
                    ...
                    for (int k = i; k < i + 9; k++)
                    {
                        for (int l = j; l < j + 9; l++)
                        {
                            ...
                            double rx = this.haarX(sample_y, sample_x, 2 * scale);
                            double ry = this.haarY(sample_y, sample_x, 2 * scale);
        ...
        {
            double a = this.integral.GetRectangleSum(column, row - size / 2,
                column + size / 2 - 1, row - size / 2 + size - 1);

            double b = this.integral.GetRectangleSum(column - size / 2, row - size / 2,
        ...
        {
            double a = this.integral.GetRectangleSum(column - size / 2, row,
        ...
        public object Clone()
        {
            var clone = new SpeededUpRobustFeaturesDescriptor(this.integral);
            clone.extended = this.extended;
            clone.invariant = this.invariant;
