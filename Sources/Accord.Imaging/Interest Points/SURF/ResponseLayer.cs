    ...
    internal class ResponseLayerCollection : IEnumerable<ResponseLayer[]>
    {
        ...
        public void Compute(IntegralImage integral)
        {
            for (int i = 0; i < this.responses.Length; ++i)
                this.responses[i].Compute(integral);
        }
        ...
        private void update()
        {
            ...
            if (this.octaves >= 1)
            {
                this.responses[i++].Update(w, h, s, 9);
                this.responses[i++].Update(w, h, s, 15);
                this.responses[i++].Update(w, h, s, 21);
                this.responses[i++].Update(w, h, s, 27);
            }

            if (this.octaves >= 2)
            {
                this.responses[i++].Update(w / 2, h / 2, s * 2, 39);
                this.responses[i++].Update(w / 2, h / 2, s * 2, 51);
            }

            if (this.octaves >= 3)
            {
                this.responses[i++].Update(w / 4, h / 4, s * 4, 75);
                this.responses[i++].Update(w / 4, h / 4, s * 4, 99);
            }

            if (this.octaves >= 4)
            {
                this.responses[i++].Update(w / 8, h / 8, s * 8, 147);
                this.responses[i++].Update(w / 8, h / 8, s * 8, 195);
            }

            if (this.octaves >= 5)
            {
                this.responses[i++].Update(w / 16, h / 16, s * 16, 291);
                this.responses[i++].Update(w / 16, h / 16, s * 16, 387);
            }
        }
        ...
        public IEnumerator<ResponseLayer[]> GetEnumerator()
        {
            ...
            for (int i = 0; i < this.octaves; i++)
            {
                ...
                for (int j = 0; j <= 1; j++)
                {
                    ...
                    pyramid[0] = this.responses[map[i, j + 0]];
                    pyramid[1] = this.responses[map[i, j + 1]];
                    pyramid[2] = this.responses[map[i, j + 2]];
        ...
        public void Compute(IntegralImage image)
        {
            ...
            for (int y = 0; y < this.Height; y++)
            {
                for (int x = 0; x < this.Width; x++)
                {
                    ...
                    this.Responses[y][x] = (Dxx * Dyy) - (0.9f * 0.9f * Dxy * Dxy);
                    this.Laplacian[y][x] = (Dxx + Dyy) >= 0 ? 1 : 0;
                }
            }
        }
