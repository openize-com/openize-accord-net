...
namespace Openize.Accord.Imaging.Interest_Points.SURF
{
    ...
    public class SpeededUpRobustFeaturesDetector : BaseSparseFeatureExtractor<SpeededUpRobustFeaturePoint>
    {
        ...
        {
            get { return this.computeOrientation; }
            set { this.computeOrientation = value; }
        }
        ...
        protected override IEnumerable<SpeededUpRobustFeaturePoint> InnerTransform(UnmanagedImage image)
        {
            ...
            {
                this.integral = IntegralImage.FromBitmap(image);
            }
            else
            {
                // create temporary grayscale image
                using (UnmanagedImage grayImage = Grayscale.CommonAlgorithms.BT709.Apply(image))
                    this.integral = IntegralImage.FromBitmap(grayImage);
            }
            ...
            if (this.responses == null)
            {
                ...
                this.responses = new ResponseLayerCollection(image.Width, image.Height, this.octaves, this.initial);
            }
            else
            {
                this.responses.Update(image.Width, image.Height, this.initial);
            }

            // Compute the response map
            this.responses.Compute(this.integral);
            ...
            foreach (ResponseLayer[] layers in this.responses)
            {
                ...
                for (int y = border + 1; y < top.Height - border; y++)
                {
                    ...
                    for (int x = border + 1; x < top.Width - border; x++)
                    {
                        ...
                        for (int i = -r; (currentValue >= this.threshold) && (i <= r); i++)
                        {
            ...
            if (this.featureType != SpeededUpRobustFeatureDescriptorType.None)
            {
                this.descriptor = new SpeededUpRobustFeaturesDescriptor(this.integral);
                this.descriptor.Extended = this.featureType == SpeededUpRobustFeatureDescriptorType.Extended;
                this.descriptor.Invariant = this.computeOrientation;
                this.descriptor.Compute(featureList);
            }
            else if (this.computeOrientation)
            {
                this.descriptor = new SpeededUpRobustFeaturesDescriptor(this.integral);
                foreach (var p in featureList)
                    p.Orientation = this.descriptor.GetOrientation(p);
            }
        ...
        {
            var clone = new SpeededUpRobustFeaturesDetector(this.threshold, this.octaves, this.initial);
            clone.SupportedFormats = supportedFormats;
            clone.computeOrientation = this.computeOrientation;
