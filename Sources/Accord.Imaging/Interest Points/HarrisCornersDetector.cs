...
namespace Openize.Accord.Imaging.Interest_Points
{
    ...
    public class HarrisCornersDetector : BaseCornersDetector
    {
        ...
        {
            this.initialize(HarrisCornerMeasure.Harris, this.k, this.threshold, this.sigma, this.r, this.size);
        }
        ...
        {
            this.initialize(HarrisCornerMeasure.Harris, k, this.threshold, this.sigma, this.r, this.size);
        }
        ...
        {
            this.initialize(HarrisCornerMeasure.Harris, k, threshold, this.sigma, this.r, this.size);
        }
        ...
        {
            this.initialize(HarrisCornerMeasure.Harris, k, threshold, sigma, this.r, this.size);
        }
        ...
        {
            this.initialize(HarrisCornerMeasure.Harris, k, threshold, sigma, suppression, this.size);
        }
        ...
        {
            this.initialize(measure, this.k, threshold, sigma, suppression, this.size);
        }
        ...
        {
            this.initialize(measure, this.k, threshold, sigma, this.r, this.size);
        }
        ...
        {
            this.initialize(measure, this.k, threshold, this.sigma, this.r, this.size);
        }
        ...
        {
            this.initialize(measure, this.k, this.threshold, this.sigma, this.r, this.size);
        }
        ...
        {
            double[] aforgeKernel = Normal.Kernel(this.sigma * this.sigma, this.size);
        ...
        protected override List<IntPoint> InnerProcess(UnmanagedImage image)
        {
            ...
            if (this.sigma > 0.0)
            {
                ...
                convolve(diffx, temp, this.kernel);
                convolve(diffy, temp, this.kernel);
                convolve(diffxy, temp, this.kernel);
            }
            ...
            // for each row
            for (int y = this.r, maxY = height - this.r; y < maxY; y++)
            {
                // for each pixel
                for (int x = this.r, maxX = width - this.r; x < maxX; x++)
                {
                    float currentValue = map[y, x];

                    // for each windows' row
                    for (int i = -this.r; (currentValue != 0) && (i <= this.r); i++)
                    {
        ...
        protected override object Clone(ISet<PixelFormat> supportedFormats)
        {
            ...
            clone.initialize(this.measure, this.k, this.threshold, this.sigma, this.r, this.size);
            return clone;
        }
    }
}
