...
namespace Openize.Accord.Imaging.Converters
{
        ...
        IConverter<Color[], UnmanagedImage>
    {
        ...
        public void Convert(double[] input, out Bitmap output)
        {
            ...
            int offset = data.Stride - this.Width;
            int src = 0;
            ...
            unsafe
            {
                ...
                {
                    for (int x = 0; x < this.Width; x++, src++, dst++)
                    {
                        *dst = (byte)Vector.Scale(input[src], this.Min, this.Max, 0, 255);
                    }
                    dst += offset;
                }
            }
        ...
        public void Convert(double[][] input, out Bitmap output)
        {
            ...
            output = new Bitmap(this.Width, this.Height, format);

            BitmapData data = output.LockBits(ImageLockMode.WriteOnly);

            int pixelSize = System.Drawing.Image.GetPixelFormatSize(format) / 8;
            int offset = data.Stride - this.Width * pixelSize;
            int src = 0;
            ...
            unsafe
            {
                ...
                for (int y = 0; y < this.Height; y++)
                {
                    for (int x = 0; x < this.Width; x++, src++)
                    {
                        for (int c = channels - 1; c >= 0; c--, dst++)
                        {
                            *dst = (byte)Vector.Scale(input[src][c], this.Min, this.Max, (byte)0, (byte)255);
                        }
                    }
                    dst += offset;
                }
        ...
        public void Convert(Color[] input, out Bitmap output)
        {
            ...
            unsafe
            {
                ...
                for (int y = 0; y < this.Height; y++)
                {
                    for (int x = 0; x < this.Width; x++, src++, dst += pixelSize)
                    {
