...
namespace Openize.Accord.Imaging
{
    ...
    using Interest_Points.Haralick;
    ...
    public class GrayLevelCooccurrenceMatrix : ICloneable
    {
        ...
        {
            get { return this.autoGray; }
            set { this.autoGray = value; }
        }
        ...
        public double[,] Compute(Bitmap source)
        {
            ...
            try
            {
                return this.Compute(bitmapData);
            }
            finally
            {
        ...
        public double[,] Compute(BitmapData source)
        {
            using (var unmanagedImage = new UnmanagedImage(source))
                return this.Compute(unmanagedImage);
        }
        ...
        {
            return this.Compute(source, new Rectangle(0, 0, source.Width, source.Height));
        }
            ...
            unsafe
            {
                ...
                switch (this.degree)
                {
                    case CooccurrenceDegree.Degree0:
                        ...
                        {
                            for (int x = startX + this.distance; x < width; x++)
                            {
                                byte a = src[stride * y + (x - this.distance)];
                    ...
                    case CooccurrenceDegree.Degree45:
                        for (int y = startY + this.distance; y < height; y++)
                        {
                            for (int x = startX; x < width - this.distance; x++)
                            {
                                byte a = src[stride * y + x];
                                byte b = src[stride * (y - this.distance) + (x + this.distance)];
                                cooccurrence[a, b]++;
                                this.numPairs++;
                            }
                        }
                    ...
                    case CooccurrenceDegree.Degree90:
                        for (int y = startY + this.distance; y < height; y++)
                        {
                            ...
                            {
                                byte a = src[stride * (y - this.distance) + x];
                                byte b = src[stride * y + x];
                                cooccurrence[a, b]++;
                                this.numPairs++;
                            }
                    ...
                    case CooccurrenceDegree.Degree135:
                        for (int y = startY + this.distance; y < height; y++)
                        {
                            ...
                            for (int x = startX; x < width - this.distance; x++)
                            {
                                ...
                                byte b = src[stride * (y - this.distance) + (steps - this.distance - x)];
                                cooccurrence[a, b]++;
                                this.numPairs++;
                            }
                        }
                ...
                if (this.normalize && this.numPairs > 0)
                {
