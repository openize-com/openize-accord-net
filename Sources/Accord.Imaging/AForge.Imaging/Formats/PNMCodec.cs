...
namespace FileFormat.Accord.Imaging.AForge.Imaging.Formats
{
    ...
    public class PNMCodec : IImageDecoder
    {
        ...
        {
            PNMImageInfo imageInfo = this.ReadHeader(stream);
        ...
        public int Open(Stream stream)
        {
            ...
            this.imageInfo = this.ReadHeader(stream);
            this.stream = stream;
        ...
        public Bitmap DecodeFrame(int frameIndex, out ImageInfo imageInfo)
        {
            ...
            this.stream.Seek(this.dataPosition, SeekOrigin.Begin);
        ...
        private PNMImageInfo ReadHeader(Stream stream)
        {
            ...
            try
            {
                ...
                width = this.ReadIntegerValue(stream);
                height = this.ReadIntegerValue(stream);
                // read pixel's highiest value
                maxValue = this.ReadIntegerValue(stream);
            }
            catch
            {
        ...
        private int ReadIntegerValue(Stream stream)
        {
            ...
            buffer[0] = this.SkipSpaces(stream);
