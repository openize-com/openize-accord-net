...
namespace FileFormat.Accord.Imaging.AForge.Imaging.Color_Reduction
{
    ...
    public class ColorImageQuantizer
    {
        ...
        public Color[] CalculatePalette(Bitmap image, int paletteSize)
        {
            ...
            try
            {
                return this.CalculatePalette(new UnmanagedImage(data), paletteSize);
            }
            finally
            {
        ...
        public Color[] CalculatePalette(UnmanagedImage image, int paletteSize)
        {
            ...
            unsafe
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    {
                        this.quantizer.AddColor(Color.FromArgb(ptr[RGB.R], ptr[RGB.G], ptr[RGB.B]));
                    }
        ...
        public Bitmap ReduceColors(Bitmap image, int paletteSize)
        {
            ...
            try
            {
                Bitmap result = this.ReduceColors(new UnmanagedImage(data), paletteSize);
        ...
        public Bitmap ReduceColors(Bitmap image, Color[] palette)
        {
            ...
            try
            {
                Bitmap result = this.ReduceColors(new UnmanagedImage(data), palette);
                result.CopyResolutionFrom(image);
                return result;
            }
            finally
            ...
            unsafe
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    for (int x = 0; x < width; x++, ptr += pixelSize)
                    {
                        ...
                        byte colorIndex = (byte)this.GetClosestColor(ptr[RGB.R], ptr[RGB.G], ptr[RGB.B]);
        ...
        private int GetClosestColor(int red, int green, int blue)
        {
            ...
            for (int i = 0, n = this.paletteToUse.Length; i < n; i++)
            {
                int dr = red - this.paletteToUse[i].R;
                int dg = green - this.paletteToUse[i].G;
                int db = blue - this.paletteToUse[i].B;
