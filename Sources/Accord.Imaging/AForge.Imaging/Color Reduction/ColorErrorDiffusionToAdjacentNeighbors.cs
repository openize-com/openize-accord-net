...
namespace Openize.Accord.Imaging.AForge.Imaging.Color_Reduction
{
    ...
    public class ColorErrorDiffusionToAdjacentNeighbors : ErrorDiffusionColorDithering
    {
        ...
        protected override unsafe void Diffuse( int rError, int gError, int bError, byte* ptr )
        {
            ...
            for ( int jI = 1, jP = this.pixelSize, jC = 0, k = coefficientsRow.Length; jC < k; jI++, jC++, jP += this.pixelSize )
            {
                if ( this.x + jI >= this.width )
                    break;

                edR = ptr[jP + RGB.R] + ( rError * coefficientsRow[jC] ) / this.coefficientsSum;
                edR = ( edR < 0 ) ? 0 : ( ( edR > 255 ) ? 255 : edR );
                ptr[jP + RGB.R] = (byte) edR;

                edG = ptr[jP + RGB.G] + ( gError * coefficientsRow[jC] ) / this.coefficientsSum;
                edG = ( edG < 0 ) ? 0 : ( ( edG > 255 ) ? 255 : edG );
                ptr[jP + RGB.G] = (byte) edG;

                edB = ptr[jP + RGB.B] + ( bError * coefficientsRow[jC] ) / this.coefficientsSum;
            ...
            for ( int i = 1, n = this.coefficients.Length; i < n; i++ )
            {
                ...
                // process the row
                for ( int jC = 0, k = coefficientsRow.Length, jI = -( k >> 1 ), jP = -( k >> 1 ) * this.pixelSize; jC < k; jI++, jC++, jP += this.pixelSize )
                {
                    if ( this.x + jI >= this.width )
                        break;
                    if ( this.x + jI < 0 )
