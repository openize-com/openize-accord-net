...
namespace FileFormat.Accord.Imaging.AForge.Imaging.Color_Reduction
{
    ...
    public abstract class ErrorDiffusionColorDithering
    {
        ...
        public Color[] ColorTable
        {
            ...
            set
            {
                if ((this.colorTable.Length < 2) || (this.colorTable.Length > 256))
        ...
        {
            get { return this.useCaching; }
            set { this.useCaching = value; }
        }
        ...
        public Bitmap Apply(Bitmap sourceImage)
        {
            ...
            try
            {
                result = this.Apply(new UnmanagedImage(data));
        ...
        public Bitmap Apply(UnmanagedImage sourceImage)
        {
            ...
            this.pixelSize = Bitmap.GetPixelFormatSize(sourceImage.PixelFormat) / 8;

            int offset = this.stride - this.width * this.pixelSize;

            // create destination image
            Bitmap destImage = new Bitmap(this.width, this.height, (this.colorTable.Length > 16) ?
                PixelFormat.Format8bppIndexed : PixelFormat.Format4bppIndexed);
            // and init its palette
            ColorPalette cp = destImage.Palette;
            for (int i = 0, n = this.colorTable.Length; i < n; i++)
            {
                cp.Entries[i] = this.colorTable[i];
            }
            destImage.Palette = cp;

            // lock destination image
            BitmapData destData = destImage.LockBits(new Rectangle(0, 0, this.width, this.height),
            ...
            unsafe
            {
                ...
                bool is8bpp = (this.colorTable.Length > 16);

                // for each line
                for (this.y = 0; this.y < this.height; this.y++)
                {
                    byte* dst = dstBase + this.y * destData.Stride;

                    // for each pixels
                    for (this.x = 0; this.x < this.width; this.x++, ptr += this.pixelSize)
                    {
                        r = ptr[RGB.R];
                        g = ptr[RGB.G];
                        b = ptr[RGB.B];
                        ...
                        Color closestColor = this.GetClosestColor(r, g, b, out colorIndex);

                        // do error diffusion
                        this.Diffuse(r - closestColor.R, g - closestColor.G, b - closestColor.B, ptr);
        ...
        private Color GetClosestColor(int red, int green, int blue, out byte colorIndex)
        {
            ...
            if ((this.useCaching) && (this.cache.ContainsKey(color)))
            {
                colorIndex = this.cache[color];
            }
            else
            {
                colorIndex = 0;
                ...
                for (int i = 0, n = this.colorTable.Length; i < n; i++)
                {
                    int dr = red - this.colorTable[i].R;
                    int dg = green - this.colorTable[i].G;
                    int db = blue - this.colorTable[i].B;
                ...
                if (this.useCaching)
                {
                    this.cache.Add(color, colorIndex);
                }
            }
