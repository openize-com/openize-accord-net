    ...
    public class BilateralSmoothing : BaseUsingCopyPartialFilter
    {
        ...
        private void InitSpatialFunc()
        {
                 ...
                 (this.spatialPropertiesChanged))
            {
                ...
                int kernelRadius = this.kernelSize / 2;
                ...
                for (int i = 0; i < this.kernelSize; i++)
                {
                    ...
                    for (int k = 0; k < this.kernelSize; k++)
                    {
                        ...
                        this.spatialFunc[i, k] = Math.Exp(-0.5 * Math.Pow(Math.Sqrt((ti2 + tk2) / this.spatialFactor), this.spatialPower));
                    }
                }
        ...
        private void InitColorFunc()
        {
            if ((this.colorFunc == null) || (this.colorPropertiesChanged))
            {
                ...
                for (int i = 0; i < colorsCount; i++)
                {
                    ...
                    {
                        this.colorFunc[i, k] = Math.Exp(-0.5 * (Math.Pow(Math.Abs(i - k) / this.colorFactor, this.colorPower)));
                    }
                }
        ...
        {
            int kernelHalf = this.kernelSize / 2;

            this.InitFilter();

            if ((rect.Width <= this.kernelSize) || (rect.Height <= this.kernelSize))
            {
                this.ProcessWithEdgeChecks(sourceData, destinationData, rect);
            }
            else
            {
                ...
                {
                    this.ProcessWithoutChecksParallel(sourceData, destinationData, safeArea);
                }
                else
                {
                    this.ProcessWithoutChecks(sourceData, destinationData, safeArea);
                }

                // top
                this.ProcessWithEdgeChecks(sourceData, destinationData,
        ...
        private unsafe void ProcessWithoutChecksParallel(UnmanagedImage source, UnmanagedImage destination, Rectangle rect)
        {
            ...
            int kernelHalf = this.kernelSize / 2;
            int bytesInKernelRow = this.kernelSize * pixelSize;
