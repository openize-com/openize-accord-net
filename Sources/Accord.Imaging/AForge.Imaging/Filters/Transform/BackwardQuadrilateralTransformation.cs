...
namespace FileFormat.Accord.Imaging.AForge.Imaging.Filters.Transform
{
    ...
    public class BackwardQuadrilateralTransformation : BaseInPlaceFilter
    {
        ...
        {
            get { return this.sourceUnmanagedImage; }
            set
            {
        ...
        {
            get { return this.destinationQuadrilateral; }
            set { this.destinationQuadrilateral = value; }
        }
        ...
        protected override unsafe void ProcessFilter(UnmanagedImage image)
        {
            ...
            if (this.sourceImage != null)
            {
                ...
                // lock source image
                BitmapData srcData = this.sourceImage.LockBits(
                    new Rectangle(0, 0, this.sourceImage.Width, this.sourceImage.Height),
                    ImageLockMode.ReadOnly, this.sourceImage.PixelFormat);

                try
                {
                    this.ProcessFilter(image, new UnmanagedImage(srcData));
                }
                finally
                {
                    // unlock source image
                    this.sourceImage.UnlockBits(srcData);
                }
            }
            else if (this.sourceUnmanagedImage != null)
            {
                // source and destination images must have same pixel format
        ...
        private unsafe void ProcessFilter(UnmanagedImage dstImage, UnmanagedImage srcImage)
        {
            ...
            PointsCloud.GetBoundingRectangle(this.destinationQuadrilateral, out minXY, out maxXY);
            ...
            double[,] matrix = QuadTransformationCalcs.MapQuadToQuad(this.destinationQuadrilateral, srcRect);
