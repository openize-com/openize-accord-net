...
namespace Openize.Accord.Imaging.AForge.Imaging.Filters.Base_classes
{
    ...
    public abstract class BaseTransformationFilter : IFilter, IFilterInformation
    {
        ...
        public Bitmap Apply(Bitmap image, Bitmap destination)
        {
            ...
            {
                // apply the filter
                dstImage = this.Apply(srcData, destination);
                dstImage.CopyResolutionFrom(image);
            }
            finally
            {
        ...
        {
            return this.Apply(imageData, null);
        }
        ...
        public Bitmap Apply(BitmapData imageData, Bitmap destination)
        {
            ...
            PixelFormat dstPixelFormat = this.FormatTranslations[imageData.PixelFormat];

            // get new image size
            Size newSize = this.CalculateNewImageSize(new UnmanagedImage(imageData));
            ...
            {
                // process the filter
                this.ProcessFilter(new UnmanagedImage(imageData), new UnmanagedImage(dstData));
            }
            finally
            {
        ...
        public UnmanagedImage Apply(UnmanagedImage image)
        {
            ...
            // get new image size
            Size newSize = this.CalculateNewImageSize(image);

            // create new destination image
            UnmanagedImage dstImage = UnmanagedImage.Create(newSize.Width, newSize.Height, this.FormatTranslations[image.PixelFormat]);

            // process the filter
            this.ProcessFilter(image, dstImage);
        ...
        public void Apply(UnmanagedImage sourceImage, UnmanagedImage destinationImage)
        {
            ...
            if (destinationImage.PixelFormat != this.FormatTranslations[sourceImage.PixelFormat])
            {
                throw new InvalidImagePropertiesException("Destination pixel format is specified incorrectly.");
            }

            // get new image size
            Size newSize = this.CalculateNewImageSize(sourceImage);
