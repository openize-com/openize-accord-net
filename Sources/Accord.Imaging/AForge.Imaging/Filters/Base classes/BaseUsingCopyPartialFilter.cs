...
namespace FileFormat.Accord.Imaging.AForge.Imaging.Filters.Base_classes
{
    ...
    public abstract class BaseUsingCopyPartialFilter : IFilter, IInPlaceFilter, IInPlacePartialFilter, IFilterInformation
    {
        ...
        public Bitmap Apply(BitmapData imageData)
        {
            ...
            {
                // process the filter
                this.ProcessFilter(new UnmanagedImage(imageData), new UnmanagedImage(dstData), new Rectangle(0, 0, width, height));
            }
            finally
            {
        ...
        public UnmanagedImage Apply(UnmanagedImage image)
        {
            ...
            UnmanagedImage dstImage = UnmanagedImage.Create(image.Width, image.Height, this.FormatTranslations[image.PixelFormat]);

            // process the filter
            this.ProcessFilter(image, dstImage, new Rectangle(0, 0, image.Width, image.Height));
        ...
        public void ApplyInPlace(Bitmap image, Rectangle rect)
        {
            ...
            {
                // apply the filter
                this.ApplyInPlace(new UnmanagedImage(data), rect);
            }
            finally
            {
                // unlock image
