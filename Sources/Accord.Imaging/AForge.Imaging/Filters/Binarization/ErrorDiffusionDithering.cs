...
namespace Openize.Accord.Imaging.AForge.Imaging.Filters.Binarization
{
    ...
    public abstract class ErrorDiffusionDithering : BaseInPlacePartialFilter
    {
        ...
        protected override unsafe void ProcessFilter(UnmanagedImage image, Rectangle rect)
        {
            ...
            ptr += (this.startY * this.stride + this.startX);

            // for each line
            for (this.y = this.startY; this.y < this.stopY; this.y++)
            {
                // for each pixels
                for (this.x = this.startX; this.x < this.stopX; this.x++, ptr++)
                {
                    v = *ptr;
