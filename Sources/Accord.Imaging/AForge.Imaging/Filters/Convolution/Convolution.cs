...
namespace Openize.Accord.Imaging.AForge.Imaging.Filters.Convolution
{
    ...
    public class Convolution : BaseUsingCopyPartialFilter
    {
        ...
        {
            get { return this.dynamicDivisorForEdges; }
            set { this.dynamicDivisorForEdges = value; }
        }
        ...
        {
            get { return this.processAlpha; }
            set { this.processAlpha = value; }
        }
        ...
        protected override unsafe void ProcessFilter(UnmanagedImage sourceData, UnmanagedImage destinationData, Rectangle rect)
        {
            ...
            if ((pixelSize <= 4) && (pixelSize != 2))
            {
                ...
                {
                    // grayscale image
                    this.Process8bppImage(src, dst, srcStride, dstStride, srcOffset, dstOffset, startX, startY, stopX, stopY);
                }
                else
                {
                    // RGB image
                    if ((pixelSize == 3) || (!this.processAlpha))
                    {
                        this.Process24bppImage(src, dst, srcStride, dstStride, srcOffset, dstOffset, startX, startY, stopX, stopY, pixelSize);
                    }
                    else
                    {
                        this.Process32bppImage(src, dst, srcStride, dstStride, srcOffset, dstOffset, startX, startY, stopX, stopY);
                    }
                }
            }
            else
            {
                ...
                if (sourceData.PixelFormat == PixelFormat.Format16bppGrayScale)
                {
                    ...
                    this.Process16bppImage(baseSrc, baseDst, srcStride, dstStride, startX, startY, stopX, stopY);
                }
                else
                {
                    // RGB image
                    if ((pixelSize == 3) || (!this.processAlpha))
                    {
                        this.Process48bppImage(baseSrc, baseDst, srcStride, dstStride, startX, startY, stopX, stopY, pixelSize);
                    }
                    else
                    {
                        this.Process64bppImage(baseSrc, baseDst, srcStride, dstStride, startX, startY, stopX, stopY);
                    }
                }
            }
        }
            ...
            for (int y = startY; y < stopY; y++)
            {
                ...
                for (int x = startX; x < stopX; x++, src++, dst++)
                {
                    ...
                    else
                    {
                        ...
                        if (!this.dynamicDivisorForEdges)
                        {
                            // do
                            div = this.divisor;
                        }
