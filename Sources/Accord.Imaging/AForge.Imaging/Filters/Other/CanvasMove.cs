...
namespace Openize.Accord.Imaging.AForge.Imaging.Filters.Other
{
    ...
    public class CanvasMove : BaseInPlaceFilter
    {
        ...
        {
            get { return Color.FromArgb(this.fillAlpha, this.fillRed, this.fillGreen, this.fillBlue); }
            set
            {
                this.fillRed = value.R;
                this.fillGreen = value.G;
                this.fillBlue = value.B;
                this.fillAlpha = value.A;
            }
        }
        ...
        {
            get { return this.fillGray; }
            set { this.fillGray = value; }
        }
        ...
        protected override void ProcessFilter(UnmanagedImage image)
        {
            ...
            switch (pixelSize)
            {
                ...
                case 3:
                case 4:
                    this.ProcessFilter8bpc(image);
                    break;
                case 2:
                case 6:
                case 8:
                    this.ProcessFilter16bpc(image);
                    break;
            }
        }
        ...
        private unsafe void ProcessFilter8bpc(UnmanagedImage image)
        {
            ...
            int movePointX = this.movePoint.X;
            int movePointY = this.movePoint.Y;
            ...
            else
            {
                ...
                for (int y = yStart; y != yStop; y += yStep)
                {
                    for (int x = xStart; x != xStop; x += xStep)
                    {
                        ...
                        {
                            pixel[RGB.R] = this.fillRed;
                            pixel[RGB.G] = this.fillGreen;
                            pixel[RGB.B] = this.fillBlue;

                            if (is32bpp)
                            {
                                pixel[RGB.A] = this.fillAlpha;
                            }
                        }
                    }
                }
