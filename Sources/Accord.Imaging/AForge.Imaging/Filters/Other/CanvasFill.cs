...
namespace Openize.Accord.Imaging.AForge.Imaging.Filters.Other
{
    ...
    public class CanvasFill : BaseInPlaceFilter
    {
        ...
        {
            get { return Color.FromArgb(this.fillRed, this.fillGreen, this.fillBlue); }
            set
            {
        ...
        protected override unsafe void ProcessFilter(UnmanagedImage image)
        {
            ...
            int startX = Math.Max(0, this.region.X);
            int startY = Math.Max(0, this.region.Y);
            ...
            int stopX = Math.Min(width, this.region.Right);
            int stopY = Math.Min(height, this.region.Bottom);
            ...
            else
            {
                ...
                for (int y = startY; y < stopY; y++)
                {
                    ...
                    {
                        ptr[RGB.R] = this.fillRed;
                        ptr[RGB.G] = this.fillGreen;
                        ptr[RGB.B] = this.fillBlue;
                    }
                    ptr += offset;
                }
            }
