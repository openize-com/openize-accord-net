...
namespace Openize.Accord.Imaging.AForge.Imaging.Filters.Other
{
    ...
    public class Pixellate : BaseInPlacePartialFilter
    {
        ...
        public int PixelWidth
        {
            ...
            set { this.pixelWidth = Math.Max(2, Math.Min(32, value)); }
        }
        ...
        public int PixelHeight
        {
            ...
            set { this.pixelHeight = Math.Max(2, Math.Min(32, value)); }
        }
        ...
        {
            set { this.pixelWidth = this.pixelHeight = Math.Max(2, Math.Min(32, value)); }
        }
        ...
        protected override unsafe void ProcessFilter(UnmanagedImage image, Rectangle rect)
        {
            ...
            int len = (int)((width - 1) / this.pixelWidth) + 1;
            // reminder
            int rem = ((width - 1) % this.pixelWidth) + 1;
            ...
            if (image.PixelFormat == PixelFormat.Format8bppIndexed)
            {
                ...
                for (int y1 = startY, y2 = startY; y1 < stopY; )
                {
                    ...
                    // calculate
                    for (i = 0; (i < this.pixelHeight) && (y1 < stopY); i++, y1++)
                    {
                        // for each pixel
                        for (x = 0; x < width; x++, src++)
                        {
                            tmp[(int)(x / this.pixelWidth)] += (int)*src;
                        }
                        src += offset;
                    }

                    // get average values
                    t1 = i * this.pixelWidth;
                    t2 = i * rem;
                    ...
                    for (i = 0; (i < this.pixelHeight) && (y2 < stopY); i++, y2++)
                    {
                        // for each pixel
                        for (x = 0; x < width; x++, dst++)
                        {
                            *dst = (byte)tmp[(int)(x / this.pixelWidth)];
                        }
                        dst += offset;
                    }
                }
