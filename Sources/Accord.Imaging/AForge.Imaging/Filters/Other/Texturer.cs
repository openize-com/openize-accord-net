...
namespace FileFormat.Accord.Imaging.AForge.Imaging.Filters.Other
{
    ...
    public class Texturer : BaseInPlacePartialFilter
    {
        ...
        protected override unsafe void ProcessFilter(UnmanagedImage image, Rectangle rect)
        {
            ...
            {
                widthToProcess = Math.Min(width, this.texture.GetLength(1));
                heightToProcess = Math.Min(height, this.texture.GetLength(0));
            }
            ...
            for (int y = 0; y < heightToProcess; y++)
            {
                ...
                {
                    double t = this.texture[y, x];
                    // process each pixel
                    for (int i = 0; i < pixelSize; i++, ptr++)
                    {
                        *ptr = (byte)Math.Min(255.0f, (this.preserveLevel * (*ptr)) + (this.filterLevel * (*ptr)) * t);
                    }
                }
                ptr += offset;
            }
