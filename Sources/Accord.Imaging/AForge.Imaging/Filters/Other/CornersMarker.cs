...
namespace FileFormat.Accord.Imaging.AForge.Imaging.Filters.Other
{
    ...
    public class CornersMarker : BaseInPlaceFilter
    {
        ...
        protected override unsafe void ProcessFilter( UnmanagedImage image )
        {
            ...
            List<IntPoint> corners = this.detector.ProcessImage( image );
            // mark all corners
            foreach ( IntPoint corner in corners )
            {
                Drawing.FillRectangle( image, new Rectangle( corner.X - 1, corner.Y - 1, 3, 3 ), this.markerColor );
            }
        }
    }
}
