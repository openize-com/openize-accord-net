...
namespace Openize.Accord.Imaging.AForge.Imaging.Filters.Other
{
    ...
    public class TexturedFilter : BaseFilter
    {
        ...
        public double FilterLevel
        {
            ...
            set { this.filterLevel = Math.Max(0.0, Math.Min(1.0, value)); }
        }
        ...
        {
            get { return this.preserveLevel; }
            set { this.preserveLevel = Math.Max(0.0, Math.Min(1.0, value)); }
        }
        ...
        {
            get { return this.textureGenerator; }
            set { this.textureGenerator = value; }
        }
        ...
        protected override unsafe void ProcessFilter(UnmanagedImage sourceData, UnmanagedImage destinationData)
        {
            ...
            else
            {
                ...
                if ((this.texture.GetLength(0) != height) || (this.texture.GetLength(1) != width))
                {
            ...
            // apply first filter
            UnmanagedImage filteredImage1 = this.filter1.Apply(sourceData);
            ...
            {
                filteredImage2 = this.filter2.Apply(sourceData);
                // check size of the result image
            ...
            unsafe
            {
                ...
                if (this.preserveLevel != 0.0)
                {
                    ...
                    for (int y = 0; y < height; y++)
                    {
                        ...
                        {
                            double t1 = this.texture[y, x];
                            double t2 = 1 - t1;

                            for (int i = 0; i < 3; i++, src1++, src2++, dst++)
                            {
                                *dst = (byte)Math.Min(255.0f,
                                    this.filterLevel * (t1 * (*src1) + t2 * (*src2)) +
                                    this.preserveLevel * (*src2));
                            }
                        }
                        src1 += src1Offset;
                        src2 += src2Offset;
