...
namespace FileFormat.Accord.Imaging.AForge.Imaging.Filters.Other
{
    ...
    public class WaterWave : BaseFilter
    {
        ...
        {
            get { return this.xWavesCount; }
            set { this.xWavesCount = Math.Max(1, Math.Min(10000, value)); }
        }
        ...
        {
            get { return this.yWavesCount; }
            set { this.yWavesCount = Math.Max(1, Math.Min(10000, value)); }
        }
        ...
        {
            get { return this.xWavesAmplitude; }
            set { this.xWavesAmplitude = Math.Max(0, Math.Min(10000, value)); }
        }
        ...
        {
            get { return this.yWavesAmplitude; }
            set { this.yWavesAmplitude = Math.Max(0, Math.Min(10000, value)); }
        }
        ...
        protected override unsafe void ProcessFilter(UnmanagedImage sourceData, UnmanagedImage destinationData)
        {
            ...
            double xFactor = 2 * Math.PI * this.xWavesCount / width;
            double yFactor = 2 * Math.PI * this.yWavesCount / height;

            // for each line
            for (int y = 0; y < height; y++)
            {
                double yPart = Math.Sin(yFactor * y) * this.yWavesAmplitude;

                // for each pixel
                for (int x = 0; x < width; x++)
                {
                    ox = x + yPart;
                    oy = y + Math.Cos(xFactor * x) * this.xWavesAmplitude;
