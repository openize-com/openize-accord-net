...
namespace FileFormat.Accord.Imaging.AForge.Imaging.Filters.Flood_Fillers
{
    ...
    public unsafe class PointedMeanFloodFill : BaseInPlacePartialFilter
    {
        ...
        protected override unsafe void ProcessFilter( UnmanagedImage image, Rectangle rect )
        {
            ...
            {
                byte startColor= *( (byte*) this.CoordsToPointerGray( this.startingPoint.X, this.startingPoint.Y ) );
                ...
                int offset = this.stride - rect.Width;

                // for each line	
                for ( int y = this.startY; y <= this.stopY; y++ )
                {
                    // for each pixel
                    for ( int x = this.startX; x <= this.stopX; x++, src++ )
                    {
            ...
            {
                byte* startColor= (byte*) this.CoordsToPointerRGB( this.startingPoint.X, this.startingPoint.Y );
                ...
                byte fillR = (byte) this.meanR;
                byte fillG = (byte) this.meanG;
                byte fillB = (byte) this.meanB;
                ...
                int offset = this.stride - rect.Width * 3;

                // for each line	
                for ( int y = this.startY; y <= this.stopY; y++ )
                {
                    // for each pixel
                    for ( int x = this.startX; x <= this.stopX; x++, src += 3 )
                    {
                        if ( this.checkedPixels[y, x] )
                        {
                            src[RGB.R] = fillR;
        ...
        private unsafe void LinearFloodFill4Gray( int x, int y )
        {
            ...
            byte* p = (byte*) this.CoordsToPointerGray( x, y );
        ...
        {
            // get image pointer for current (X, Y)
            byte* p = (byte*) this.CoordsToPointerRGB( x, y );
