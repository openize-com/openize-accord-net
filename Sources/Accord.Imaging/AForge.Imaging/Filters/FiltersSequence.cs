...
namespace Openize.Accord.Imaging.AForge.Imaging.Filters
{
    ...
    public class FiltersSequence : CollectionBase, IFilter
    {
        ...
        {
            this.InnerList.AddRange(filters);
        }
        ...
        {
            get { return ((IFilter)this.InnerList[index]); }
        }
        ...
        public Bitmap Apply(BitmapData imageData)
        {
            ...
            UnmanagedImage dstUnmanagedImage = this.Apply(new UnmanagedImage(imageData));
        ...
        public UnmanagedImage Apply(UnmanagedImage image)
        {
            ...
            dstImg = ((IFilter)this.InnerList[0]).Apply(image);

            // apply other filters
            for (int i = 1; i < n; i++)
            {
                tmpImg = dstImg;
                dstImg = ((IFilter)this.InnerList[i]).Apply(tmpImg);
                tmpImg.Dispose();
            }
        ...
        public void Apply(UnmanagedImage sourceImage, UnmanagedImage destinationImage)
        {
            ...
            if (n == 1)
            {
                ((IFilter)this.InnerList[0]).Apply(sourceImage, destinationImage);
            }
            else
            {
                ...
                // apply the first filter
                tmpImg1 = ((IFilter)this.InnerList[0]).Apply(sourceImage);
                ...
                {
                    tmpImg2 = tmpImg1;
                    tmpImg1 = ((IFilter)this.InnerList[i]).Apply(tmpImg2);
                    tmpImg2.Dispose();
                }

                ((IFilter)this.InnerList[n]).Apply(tmpImg1, destinationImage);
            }
        }
    }
}
