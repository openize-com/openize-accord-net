...
namespace Openize.Accord.Imaging.AForge.Imaging.Filters.Color_Filters
{
    ...
    public class ContrastCorrection : BaseInPlacePartialFilter
    {
        ...
        public int Factor
        {
            ...
            set
            {
                this.factor = Math.Max(-127, Math.Min(127, value));

                if (this.factor > 0)
                {
                    this.baseFilter.InRed = this.baseFilter.InGreen = this.baseFilter.InBlue = this.baseFilter.InGray = new IntRange(this.factor, 255 - this.factor);
                    this.baseFilter.OutRed = this.baseFilter.OutGreen = this.baseFilter.OutBlue = this.baseFilter.OutGray = new IntRange(0, 255);
                }
                else
                {
                    this.baseFilter.OutRed = this.baseFilter.OutGreen = this.baseFilter.OutBlue = this.baseFilter.OutGray = new IntRange(-this.factor, 255 + this.factor);
                    this.baseFilter.InRed = this.baseFilter.InGreen = this.baseFilter.InBlue = this.baseFilter.InGray = new IntRange(0, 255);
                }
            }
        }
