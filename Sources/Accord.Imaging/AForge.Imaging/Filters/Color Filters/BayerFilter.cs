...
namespace FileFormat.Accord.Imaging.AForge.Imaging.Filters.Color_Filters
{
    ...
    public class BayerFilter : BaseFilter
    {
        ...
        {
            get { return this.performDemosaicing; }
            set { this.performDemosaicing = value; }
        }
            ...
            if ( !this.performDemosaicing )
            {
                ...
                for ( int y = 0; y < height; y++ )
                {
                    ...
                    for ( int x = 0; x < width; x++, src++, dst += 3 )
                    {
                        ...
                        dst[this.bayerPattern[y & 1, x & 1]] = *src;
                    }
                    src += srcOffset;
                    dst += dstOffset;
                }
            ...
            else
            {
                ...
                for ( int y = 0; y < height; y++ )
                {
                    ...
                    for ( int x = 0; x < width; x++, src++, dst += 3 )
                    {
                        ...
                        int bayerIndex = this.bayerPattern[y & 1, x & 1];
                        ...
                        if ( x != 0 )
                        {
                            bayerIndex = this.bayerPattern[y & 1, ( x - 1 ) & 1];
                        ...
                        if ( y != 0 )
                        {
                            bayerIndex = this.bayerPattern[( y - 1 ) & 1, x & 1];
                            ...
                            if ( x != 0 )
                            {
                                bayerIndex = this.bayerPattern[( y - 1 ) & 1, ( x - 1 ) & 1];
