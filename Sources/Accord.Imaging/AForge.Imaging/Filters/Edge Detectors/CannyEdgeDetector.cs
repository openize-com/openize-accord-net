...
namespace FileFormat.Accord.Imaging.AForge.Imaging.Filters.Edge_Detectors
{
    using System;
    ...
    public class CannyEdgeDetector : BaseUsingCopyPartialFilter
    {
        ...
        {
            get { return this.gaussianFilter.Sigma; }
            set { this.gaussianFilter.Sigma = value; }
        }
        ...
        {
            get { return this.gaussianFilter.Size; }
            set { this.gaussianFilter.Size = value; }
        }
        ...
        protected override unsafe void ProcessFilter(UnmanagedImage sourceData, UnmanagedImage destinationData, Rectangle rect)
        {
            ...
            // STEP 1 - blur image
            UnmanagedImage blurredImage = this.gaussianFilter.Apply(sourceData);

            // orientation array
            byte[] orients = new byte[width * height];
            // gradients array
            float[,] gradients = new float[sourceData.Width, sourceData.Height];
            ...
            for (int y = startY; y < stopY; y++)
            {
                ...
                {
                    if (*dst < this.highThreshold)
                    {
                        if (*dst < this.lowThreshold)
                        {
                            // non edge
                            *dst = 0;
                        }
                        else
                        {
                            ...
                            if ((dst[-1] < this.highThreshold) &&
                                (dst[1] < this.highThreshold) &&
                                (dst[-dstStride - 1] < this.highThreshold) &&
                                (dst[-dstStride] < this.highThreshold) &&
                                (dst[-dstStride + 1] < this.highThreshold) &&
                                (dst[dstStride - 1] < this.highThreshold) &&
                                (dst[dstStride] < this.highThreshold) &&
                                (dst[dstStride + 1] < this.highThreshold))
