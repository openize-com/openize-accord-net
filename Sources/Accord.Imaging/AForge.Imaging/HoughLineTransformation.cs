...
namespace FileFormat.Accord.Imaging.AForge.Imaging
{
    ...
    public class HoughLine : IComparable<HoughLine>
    {
        ...
        public int CompareTo(HoughLine other)
        {
            ...
            return -(this.Intensity.CompareTo(other.Intensity));
        }
        ...
        {
            return "Theta: {0}, radius: {1}, intensity: {2}".Format(this.Theta, this.Radius, this.Intensity);
        }
    ...
    public class HoughLineTransformation
    {
        ...
        public int StepsPerDegree
        {
            ...
            set
            {
                ...
                this.thetaStep = Math.PI / this.houghHeight;
        ...
        {
            get { return this.minLineIntensity; }
            set { this.minLineIntensity = value; }
        }
                ...
                for (int y = startY; y < stopY; y++)
                {
                    ...
                    for (int x = startX; x < stopX; x++, src++)
                    {
                        if (*src != 0)
                        {
                            ...
                            {
                                int radius = (int)Math.Round(this.cosMap[theta] * x - this.sinMap[theta] * y) + halfHoughWidth;
        ...
        public Bitmap ToBitmap()
        {
            ...
            int width = this.houghMap.GetLength(1);
            ...
            float scale = 255.0f / this.maxMapIntensity;
        ...
        {
            // lines count
            int n = Math.Min(count, this.lines.Count);
        ...
        {
            int count = 0, n = this.lines.Count;

            while ((count < n) && (((HoughLine)this.lines[count]).RelativeIntensity >= minRelativeIntensity))
                count++;
        ...
        {
            int maxTheta = this.houghMap.GetLength(0);
            int maxRadius = this.houghMap.GetLength(1);
            ...
            for (int theta = 0; theta < maxTheta; theta++)
            {
                ...
                for (int radius = 0; radius < maxRadius; radius++)
                {
                    ...
                    for (int tt = theta - this.localPeakRadius, ttMax = theta + this.localPeakRadius; tt < ttMax; tt++)
                    {
                        ...
                        for (int tr = cycledRadius - this.localPeakRadius, trMax = cycledRadius + this.localPeakRadius; tr < trMax; tr++)
                        {
                            ...
                            if (this.houghMap[cycledTheta, tr] > intensity)
                            {
                                foundGreater = true;
                                break;
                            }
                    ...
                    if (!foundGreater)
                    {
                        ...
                        this.lines.Add(new HoughLine(theta / (double)this.stepsPerDegree, (short)(radius - halfHoughWidth), intensity, (double)intensity / this.maxMapIntensity));
                    }
                }
            }
