    ...
    public class IntegralImage : ICloneable
    {
        ...
        public uint[,] InternalData
        {
            get
            {
                ...
                {
                    this.integralImage = new uint[this.height + 1, this.width + 1];
                    for (int y = 0; y <= this.height; y++)
                    {
                        for (int x = 0; x <= this.width; x++)
                        {
                            this.integralImage[y, x] = this.matrix[y][x];
                        }
                    }
                }
        ...
        public uint GetRectangleSum(int x1, int y1, int x2, int y2)
        {
            ...
            return this.matrix[y2][x2] + this.matrix[y1][x1] - this.matrix[y2][x1] - this.matrix[y1][x2];
        }
        ...
        public int GetHaarXWavelet(int x, int y, int radius)
        {
            ...
            long a = this.GetRectangleSum(x, y1, x + radius - 1, y2);
            long b = this.GetRectangleSum(x - radius, y1, x - 1, y2);
        ...
        public float GetRectangleMean(int x1, int y1, int x2, int y2)
        {
            ...
            return (float)((double)(this.matrix[y2][x2] + this.matrix[y1][x1] - this.matrix[y2][x1] - this.matrix[y1][x2]) /
                (double)((x2 - x1) * (y2 - y1)));
        }
        ...
        public object Clone()
        {
            var clone = new IntegralImage(this.width, this.height);
            this.integralImage.CopyTo(clone.integralImage, 0);
            return clone;
        }
    }
}
