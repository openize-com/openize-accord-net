...
namespace Openize.Accord.Imaging.AForge.Imaging
{
    ...
    public class UnmanagedImage : IDisposable
    {
        ...
        public int Offset
        {
            get { return this.stride - this.width * this.PixelSize; }
        }
        ...
        {
            get { return System.Drawing.Bitmap.GetPixelFormatSize(this.pixelFormat) / 8; }
        }
        ...
        {
            this.init(imageData, width, height, stride, pixelFormat);
        }
        ...
        {
            this.init(bitmapData.Scan0, bitmapData.Width, bitmapData.Height, bitmapData.Stride, bitmapData.PixelFormat);
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            {
                System.Runtime.InteropServices.Marshal.FreeHGlobal(this.imageData);
                System.GC.RemoveMemoryPressure(this.stride * this.height);
        ...
        public UnmanagedImage Clone()
        {
            ...
            IntPtr newImageData = System.Runtime.InteropServices.Marshal.AllocHGlobal(this.stride * this.height);
            System.GC.AddMemoryPressure(this.stride * this.height);
        ...
        internal unsafe void CheckBounds(byte* address)
        {
            ...
            byte* end = src + this.stride * this.height;
        ...
        {
            if (
                (this.width != destImage.width) || (this.height != destImage.height) ||
                (this.pixelFormat != destImage.pixelFormat))
            {
        ...
        public Bitmap ToManagedImage(bool makeCopy)
        {
            ...
            try
            {
                if (!makeCopy)
                {
                    ...
                    dstImage = new Bitmap(this.width, this.height, this.pixelFormat);
                    var data = dstImage.LockBits(new Rectangle(0, 0, this.width, this.height), ImageLockMode.WriteOnly, this.pixelFormat);
                    data.Scan0 = this.ImageData;
        ...
        {
            int pixelSize = Bitmap.GetPixelFormatSize(this.pixelFormat) / 8;

            if ((this.pixelFormat == PixelFormat.Format16bppGrayScale) || (pixelSize > 4))
            {
            ...
            byte[] pixelValues = new byte[points.Count * ((this.pixelFormat == PixelFormat.Format8bppIndexed) ? 1 : 3)];

            unsafe
            {
                byte* basePtr = (byte*)this.imageData.ToPointer();
                byte* ptr;
                ...
                if (this.pixelFormat == PixelFormat.Format8bppIndexed)
                {
                    ...
                    {
                        ptr = basePtr + this.stride * point.Y + point.X;
                        pixelValues[i++] = *ptr;
                    }
                }
                else
                {
                    ...
                    foreach (IntPoint point in points)
                    {
                        ptr = basePtr + this.stride * point.Y + point.X * pixelSize;
        ...
        {
            return this.CollectActivePixels(new Rectangle(0, 0, this.width, this.height));
        }
        ...
        public List<IntPoint> CollectActivePixels(Rectangle rect)
        {
            ...
            unsafe
            {
                ...
                if ((this.pixelFormat == PixelFormat.Format16bppGrayScale) || (pixelSize > 4))
                {
                    ...
                    {
                        ushort* ptr = (ushort*)(basePtr + y * this.stride + startX * pixelSize);
                ...
                else
                {
                    for (int y = startY; y < stopY; y++)
                    {
                        byte* ptr = basePtr + y * this.stride + startX * pixelSize;
        ...
        public void SetPixels(List<IntPoint> coordinates, Color color)
        {
            unsafe
            {
                ...
                switch (this.pixelFormat)
                {
                    case PixelFormat.Format8bppIndexed:
                        {
                            ...
                            foreach (IntPoint point in coordinates)
                            {
                                ...
                                {
                                    byte* ptr = basePtr + point.Y * this.stride + point.X;
                                    *ptr = grayValue;
                                }
                            }
                        }
                    ...
                    case PixelFormat.Format32bppRgb:
                        {
                            ...
                            foreach (IntPoint point in coordinates)
                            {
                                if ((point.X >= 0) && (point.Y >= 0) && (point.X < this.width) && (point.Y < this.height))
                                {
                                    byte* ptr = basePtr + point.Y * this.stride + point.X * pixelSize;
                                    ptr[RGB.R] = red;
                                    ptr[RGB.G] = green;
                                    ptr[RGB.B] = blue;
                                }
                    ...
                    case PixelFormat.Format16bppGrayScale:
                        {
                            ...
                            foreach (IntPoint point in coordinates)
                            {
                                if ((point.X >= 0) && (point.Y >= 0) && (point.X < this.width) && (point.Y < this.height))
                                {
                                    ushort* ptr = (ushort*)(basePtr + point.Y * this.stride) + point.X;
                                    *ptr = grayValue;
                                }
                            }
                        }
                    ...
                    case PixelFormat.Format48bppRgb:
                        {
                            ...
                            foreach (IntPoint point in coordinates)
                            {
                                if ((point.X >= 0) && (point.Y >= 0) && (point.X < this.width) && (point.Y < this.height))
                                {
                                    ushort* ptr = (ushort*)(basePtr + point.Y * this.stride + point.X * pixelSize);
                                    ptr[RGB.R] = red16;
                                    ptr[RGB.G] = green16;
                                    ptr[RGB.B] = blue16;
                                }
                    ...
                    default:
                        throw new UnsupportedImageFormatException("The pixel format is not supported: " + this.pixelFormat);
                }
            }
        }
        ...
        private void SetPixel(int x, int y, byte r, byte g, byte b, byte a)
        {
            if ((x >= 0) && (y >= 0) && (x < this.width) && (y < this.height))
            {
                ...
                {
                    int pixelSize = Bitmap.GetPixelFormatSize(this.pixelFormat) / 8;
                    byte* ptr = (byte*)this.imageData.ToPointer() + y * this.stride + x * pixelSize;
        ...
        public ushort[] Collect16bppPixelValues(List<IntPoint> points)
        {
            ...
            if ((this.pixelFormat == PixelFormat.Format8bppIndexed) || (pixelSize == 3) || (pixelSize == 4))
            {
            ...
            ushort[] pixelValues = new ushort[points.Count * ((this.pixelFormat == PixelFormat.Format16bppGrayScale) ? 1 : 3)];
            ...
            unsafe
            {
                ...
                if (this.pixelFormat == PixelFormat.Format16bppGrayScale)
                {
                    ...
                    foreach (IntPoint point in points)
                    {
                        ptr = (ushort*)(basePtr + this.stride * point.Y + point.X * pixelSize);
                        pixelValues[i++] = *ptr;
                    }
                }
                else
        ...
        public byte[] ToByteArray()
        {
            ...
            int actualNumberOfBytes = this.width * this.height * pixelSize;
            ...
            unsafe
            {
                ...
                for (int y = 0, k = 0; y < this.height; y++)
                {
