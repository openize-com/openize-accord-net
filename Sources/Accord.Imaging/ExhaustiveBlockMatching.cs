...
namespace FileFormat.Accord.Imaging
{
    ...
    public class ExhaustiveBlockMatching : IBlockMatching
    {
        ...
        public List<BlockMatch> ProcessImage(Bitmap sourceImage, List<IntPoint> coordinates, Bitmap searchImage)
        {
            ...
            {
                // process the image
                matchings = this.ProcessImage(new UnmanagedImage(sourceImageData),
        ...
        public List<BlockMatch> ProcessImage(UnmanagedImage sourceImage, List<IntPoint> coordinates, UnmanagedImage searchImage)
        {
            ...
            int blockRadius = this.blockSize / 2;
            int searchWindowSize = 2 * this.searchRadius;
            int blockLineSize = this.blockSize * pixelSize;
            int blockOffset = stride - (this.blockSize * pixelSize);

            // maximum possible difference of blocks
            int maxDiff = this.blockSize * this.blockSize * pixelSize * 255;

            // integer similarity threshold
            int threshold = (int)(this.similarityThreshold * maxDiff);
            ...
            unsafe
            {
                ...
                for (int iPoint = 0; iPoint < pointsCount; iPoint++)
                {
                    ...
                    int searchStartX = refPointX - blockRadius - this.searchRadius;
                    int searchStartY = refPointY - blockRadius - this.searchRadius;
                    ...
                    {
                        if ((searchStartY + searchWindowRow < 0) || (searchStartY + searchWindowRow + this.blockSize >= height))
                        {
                            // skip row
                            continue;
                        }
                        ...
                        for (int searchWindowCol = 0; searchWindowCol < searchWindowSize; searchWindowCol++)
                        {
                            ...
                            if ((blockSearchX < 0) || (blockSearchX + this.blockSize >= width))
                            {
                                // skip column
                                continue;
                            }
                            ...
                            int error = 0;
                            for (int blockRow = 0; blockRow < this.blockSize; blockRow++)
                            {
